...
export function PlanEditor({ plan, proposal, validation, comments = [], selectedCommentId = null, onSelectComment = () => {}, onEdit, onMeasure, layoutPresets = [] }) {
  ...
  return <div className="plan-editor">
    ...
    {panels.library && <aside className="object-library"><header><b>LIBRARY</b><Button variant="ghost" size="icon-xs" type="button" onClick={() => setPanels((value) => ({ ...value, library: false }))} aria-label="Close library">×</Button></header>{candidate.objects.length === 0 && layoutPresets.map((preset) => <Button className="is-layout" variant="outline" type="button" key={preset.id} onClick={() => { apply({ operation: "apply-layout", roomBoundary: preset.roomBoundary, objects: clone(preset.objects) }); setPanels((value) => ({ ...value, library: false })); }}><span>LAY</span><strong>{preset.label}</strong><small>{preset.objects.length}</small></Button>)}{venueTemplateCatalog.inventoryTemplates.map((item) => <Button variant="outline" type="button" key={item.id} onClick={() => { setPlaceTemplate(item); setTool("inventory"); setPanels((value) => ({ ...value, library: false })); }}><span>{item.category.slice(0,3).toUpperCase()}</span><strong>{item.name}</strong><small>{item.availability.total - item.availability.unavailable}</small></Button>)}</aside>}
