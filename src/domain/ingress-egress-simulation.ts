...
interface MobilityProfileInput {
  id?: string;
  label?: string;
  share: number;
  speedFactor?: number;
...
export interface IngressEgressInput {
  mode?: "normal" | "emergency";
  curves?: { arrival?: CurvePoint[]; departure?: CurvePoint[] };
  mobilityProfiles?: MobilityProfileInput[];
  assumptions?: { normal?: Partial<FlowAssumption>; emergency?: Partial<FlowAssumption> };
}
interface FlowAssumption {
  responseDelaySeconds: number;
  flowFactor: number;
  elevatorsAvailable: boolean;
}
const isIngressEgressInput = (value: unknown): value is IngressEgressInput | null | undefined =>
...
const footprintArea = (footprint: Footprint): number => {
  ...
  if (footprint?.kind === "polygon")
    return Math.abs(
      footprint.points.reduce((sum, point, index) => {
...
const normalizeCurve = (
  source: readonly CurvePoint[] | undefined,
  label: string,
  horizonSeconds: number,
  fallback: readonly CurvePoint[],
): CurvePoint[] => {
  ...
  if (points.some((point) => !Number.isFinite(point.second) || !Number.isFinite(point.cumulativeShare)))
    throw new Error(`${label} curve values must be finite`);
  if (
    points.some(
      (point) =>
        point.second < 0 || point.second > horizonSeconds || point.cumulativeShare < 0 || point.cumulativeShare > 1,
    )
  )
    throw new Error(`${label} curve must fit inside the Scenario horizon`);
  const withEnds = [
    ...(points[0]?.second === 0 ? [] : [{ second: 0, cumulativeShare: 0, _index: -1 }]),
    ...points,
    ...(points.at(-1)?.second === horizonSeconds
      ? []
      : [{ second: horizonSeconds, cumulativeShare: 1, _index: points.length }]),
  ];
  let previousShare = -1;
  ...
  return withEnds.map(({ _index, ...point }) => ({
    ...point,
    cumulativeShare: round(point.cumulativeShare / finalShare, 6),
  }));
};

const timeAtShare = (curve: readonly CurvePoint[], share: number): number => {
  const target = clamp(share, 0, 1);
  for (let index = 1; index < curve.length; index += 1) {
    const left = curve[index - 1];
    const right = curve[index];
    if (!left || !right || right.cumulativeShare < target) continue;
...
const peakCurveRate = (curve: readonly CurvePoint[], population: number): number =>
  curve.slice(1).reduce((peak, point, index) => {
    const previous = curve[index];
    if (!previous) return peak;
    const seconds = point.second - previous.second;
    return seconds > 0
      ? Math.max(peak, ((point.cumulativeShare - previous.cumulativeShare) * population * 60) / seconds)
      : peak;
  }, 0);

const normalizeMobilityProfiles = (source: readonly MobilityProfileInput[] | undefined) => {
  const profiles: readonly MobilityProfileInput[] = source?.length
    ? source
    : [
  ...
  return normalized.map((profile) => ({
    ...profile,
    share: round(profile.share / total, 6),
    speedFactor: round(profile.speedFactor, 4),
  }));
};

export function normalizeIngressEgressInputs(input: unknown, horizonSeconds: number) {
  if (!isIngressEgressInput(input)) throw new Error("Ingress/egress input must be an object");
  const value = input;
...
const footprintWidth = (footprint: Footprint): number =>
  footprint.kind === "rectangle"
    ...
    : footprint.kind === "line"
      ...
      : footprint.kind === "circle"
        ? footprint.radius * 2
        : 1;
const infrastructureSnapshot = (plan: VenuePlan) => {
  const analysis = analyzeSpatialPlan({ plan });
  const objects = new Map(plan.objects.map((object) => [object.id, object]));
  const capacitySections = new Map(
    analysis.evidence.capacity.sectionCapacities.map((section) => [section.objectId, section]),
  );
  ...
  const routes = analysis.evidence.circulation.graphEdges.map((edge) => {
    ...
    return {
      id: edge.objectId,
      kind: object?.kind ?? "corridor",
      label: object?.label ?? edge.objectId,
      widthM: round(edge.widthM),
      lengthM: round(edge.lengthM),
      accessible: object?.route?.accessible === true,
      blockedByObjectIds: clone(edge.blockedByObjectIds),
      point: object ? footprintCenter(object.footprint) : { x: 0, y: 0 },
    };
  });
  const typed = <T extends { id: string }>(
    predicate: (object: VenueObject) => boolean,
    map: (object: VenueObject) => T,
  ): T[] =>
    plan.objects
      .filter(predicate)
      .map(map)
      .sort((left, right) => left.id.localeCompare(right.id));
  const entrances = typed(
    (object) => ["entrance", "accessible_entrance"].includes(object.kind),
    (object) => ({
      id: object.id,
      kind: object.kind,
      label: object.label,
      clearWidthM: object.entrance?.clearWidthM ?? footprintWidth(object.footprint),
      accessible: object.kind === "accessible_entrance" || object.accessibility?.accessible === true,
      point: footprintCenter(object.footprint),
    }),
  );
  const exits = typed(
    (object) => object.kind === "fire_exit",
    (object) => ({
      id: object.id,
      kind: object.kind,
      label: object.label,
      clearWidthM: object.exit?.clearWidthM ?? footprintWidth(object.footprint),
      ratedCapacityPersons: object.exit?.capacityPersons ?? null,
      emergency: object.exit?.emergency === true,
      point: footprintCenter(object.footprint),
    }),
  );
  const doors = typed(
    ...
    (object) => ({
      ...
      kind: object.kind,
      label: object.label,
      clearWidthM: object.door?.clearWidthM ?? footprintWidth(object.footprint),
      accessible: object.door?.accessible === true,
      point: footprintCenter(object.footprint),
    }),
  );
  const checkpoints = typed(
    (object) => object.kind === "checkpoint" || object.circulation?.role === "checkpoint",
    (object) => ({
      id: object.id,
      kind: "checkpoint",
      label: object.label,
      capacityPersonsPerMinute:
        object.circulation?.capacityPersonsPerMinute ?? object.circulation?.capacityPersons ?? 30,
      servesZoneIds: clone(object.circulation?.servesZoneIds ?? []),
      point: footprintCenter(object.footprint),
    }),
  );
  const stairs = typed(
    (object) => ["stair", "stairs"].includes(object.kind),
    (object) => ({
      id: object.id,
      kind: "stairs",
      label: object.label,
      clearWidthM: object.circulation?.clearWidthM ?? footprintWidth(object.footprint),
      accessible: false,
      servesZoneIds: clone(object.circulation?.servesZoneIds ?? []),
      point: footprintCenter(object.footprint),
    }),
  );
  const elevators = typed(
    (object) => object.kind === "elevator",
    (object) => ({
      id: object.id,
      kind: "elevator",
      label: object.label,
      carCapacityPersons: object.circulation?.carCapacityPersons ?? 12,
      cycleSeconds: object.circulation?.cycleSeconds ?? 60,
      accessible: object.accessibility?.accessible !== false,
      servesZoneIds: clone(object.circulation?.servesZoneIds ?? []),
      point: footprintCenter(object.footprint),
    }),
  );
  const sections = plan.objects
    .filter(
      (object) =>
        object.kind === "seating_section" && (capacitySections.get(object.id)?.capacity ?? object.capacity ?? 0) > 0,
    )
    .map((object) => {
      const path =
        analysis.evidence.circulation.shortestExitPaths.find((item) => item.occupiedObjectId === object.id) ?? null;
      const routeObjects = (path?.routeObjectIds ?? []).flatMap((id) => {
      ...
      return {
        ...
        capacity: capacitySections.get(object.id)?.capacity ?? object.capacity ?? 0,
  ...
  const snapshot = {
    ...
    corridors: routes,
    sections,
    graphFingerprint: analysis.evidence.circulation.graphFingerprint,
  };
  return { ...snapshot, fingerprint: stableFingerprint("simulation-infrastructure", snapshot) };
};

type Infrastructure = ReturnType<typeof infrastructureSnapshot>;
type InfrastructureSection = Infrastructure["sections"][number];
type IngressEgressConfiguration = ReturnType<typeof normalizeIngressEgressInputs>;
interface IngressEgressScenario {
  id: string;
  seed: number;
  horizonSeconds: number;
  sampleCount: number;
  inputs: { population: number };
  confidence: object;
  phases: object[];
  ingressEgress: IngressEgressConfiguration;
}
const pathCapacity = (
  section: InfrastructureSection,
  infrastructure: Infrastructure,
  mode: "normal" | "emergency",
  assumptions: FlowAssumption,
) => {
  ...
  const components: Array<{
    ...
    kind: string;
    accessible: boolean;
    capacityPersonsPerSecond: number;
    memberObjectIds?: string[];
  }> = section.routeObjectIds.flatMap((id) => {
      ...
      ? [
          {
            ...
            capacityPersonsPerSecond: route.blockedByObjectIds.length ? 0 : route.widthM * 1.3,
          },
        ]
      : [];
  });
  components.push(
    ...infrastructure.exits
      .filter((exit) => mode !== "emergency" || exit.emergency)
      .map((exit) => ({
        id: exit.id,
        kind: "exit",
        accessible: true,
        capacityPersonsPerSecond: Math.min(
          exit.clearWidthM * 1.3,
          exit.ratedCapacityPersons ? exit.ratedCapacityPersons / 180 : Number.MAX_SAFE_INTEGER,
        ),
      })),
  );
  if (mode === "normal")
    components.push(
      ...infrastructure.doors.map((door) => ({
        id: door.id,
        kind: "door",
        accessible: door.accessible,
        capacityPersonsPerSecond: door.clearWidthM * 1.3,
      })),
    );
  const servesSection = (item: { servesZoneIds: string[] }): boolean =>
    !item.servesZoneIds.length || item.servesZoneIds.includes(section.zoneId);
  ...
  const vertical = [
    ...
    ...elevators.map((item) => ({
      id: item.id,
      capacityPersonsPerSecond: item.carCapacityPersons / item.cycleSeconds,
  ...
  if (vertical.length) {
    ...
    if (primary)
      components.push({
        ...
        kind: "vertical",
        accessible,
        memberObjectIds: vertical.map((item) => item.id).sort(),
        capacityPersonsPerSecond: vertical.reduce((sum, item) => sum + item.capacityPersonsPerSecond, 0),
      });
  }
  if (mode === "normal")
    components.push(
      ...infrastructure.checkpoints.filter(servesSection).map((checkpoint) => ({
        id: checkpoint.id,
        kind: "checkpoint",
        accessible: true,
        capacityPersonsPerSecond: checkpoint.capacityPersonsPerMinute / 60,
      })),
    );
  const traversable = components
    .filter((component) => component.capacityPersonsPerSecond > 0)
    .sort(
      (left, right) =>
        left.capacityPersonsPerSecond - right.capacityPersonsPerSecond || left.id.localeCompare(right.id),
    );
  return {
    components,
    bottleneck: traversable[0] ?? { id: "unavailable", kind: "route", accessible: false, capacityPersonsPerSecond: 0 },
  };
};

interface AssumptionSimulationInput {
  scenario: IngressEgressScenario;
  infrastructure: Infrastructure;
  mode: "normal" | "emergency";
  random: () => number;
  sampleCount: number;
}
const simulateAssumption = ({ scenario, infrastructure, mode, random, sampleCount }: AssumptionSimulationInput) => {
  ...
  const weightedSpeedFactor = scenario.ingressEgress.mobilityProfiles.reduce(
    (sum, profile) => sum + profile.share * profile.speedFactor,
    0,
  );
  const accessibleProfileShare = scenario.ingressEgress.mobilityProfiles
    .filter((profile) => profile.accessibleRouteRequired)
    .reduce((sum, profile) => sum + profile.share, 0);
  const sampleOutputs: Array<{
    zones: Array<{
      section: InfrastructureSection;
      demandPersons: number;
      path: ReturnType<typeof pathCapacity>;
      travelSeconds: number;
      queueSeconds: number;
      clearanceSeconds: number;
      accessibleDemand: number;
      accessibleAvailable: boolean;
      accessibleClearanceSeconds: number;
    }>;
  ...
  for (let sampleIndex = 0; sampleIndex < sampleCount; sampleIndex += 1) {
    ...
    const zones = infrastructure.sections.map((section) => {
      ...
      const travelSeconds =
        section.pathDistanceM == null
          ? scenario.horizonSeconds
          : section.pathDistanceM / Math.max(0.2, 1.25 * weightedSpeedFactor * speedVariation);
      const queueSeconds = flowCapacity > 0 ? demandPersons / flowCapacity : scenario.horizonSeconds;
      const clearanceSeconds = Math.min(
        scenario.horizonSeconds * 2,
        assumptions.responseDelaySeconds * responseVariation + release95Seconds + travelSeconds + queueSeconds,
      );
      const accessibleDemand = Math.max(section.accessibleSeats, demandPersons * accessibleProfileShare);
      const accessibleAvailable =
        section.pathAccessible &&
        path.components.filter((component) => component.kind !== "stairs").every((component) => component.accessible);
      const accessibleFlow = accessibleAvailable ? Math.max(0.01, flowCapacity * 0.72) : 0;
      const accessibleClearanceSeconds = accessibleAvailable
        ? assumptions.responseDelaySeconds * responseVariation +
          release95Seconds +
          (section.pathDistanceM ?? 0) / Math.max(0.2, 1.25 * 0.68 * speedVariation) +
          accessibleDemand / accessibleFlow
        : scenario.horizonSeconds * 2;
      return {
        section,
        demandPersons,
        ...
        accessibleDemand,
        accessibleAvailable,
        accessibleClearanceSeconds,
      };
    });
    sampleOutputs.push({
      zones,
      totalClearanceSeconds: Math.max(0, ...zones.map((zone) => zone.clearanceSeconds)),
      accessibleClearanceSeconds: Math.max(0, ...zones.map((zone) => zone.accessibleClearanceSeconds)),
    });
  }
  const zoneResults = infrastructure.sections.map((section) => {
    const values = sampleOutputs.flatMap((sample) => {
      const value = sample.zones.find((zone) => zone.section.objectId === section.objectId);
      return value ? [value] : [];
    });
    return {
      ...
      p95ClearanceSeconds: round(
        percentile(
          values.map((value) => value.clearanceSeconds),
          0.95,
        ),
        1,
      ),
  ...
  const bottleneckCandidates = infrastructure.sections
    .map((section) => {
      const path = pathCapacity(section, infrastructure, mode, assumptions);
      ...
      const durationSeconds = Math.max(
        0,
        (zone?.p95ClearanceSeconds ?? 0) - (assumptions.responseDelaySeconds + release95Seconds),
      );
      return {
        objectId: path.bottleneck.id,
        kind: path.bottleneck.kind,
        durationSeconds: round(durationSeconds, 1),
        affectedOccupancyPersons: round(affected, 1),
        capacityPersonsPerSecond: round(path.bottleneck.capacityPersonsPerSecond * assumptions.flowFactor, 3),
        loadRatio: round(
          affected /
            Math.max(
              1,
              path.bottleneck.capacityPersonsPerSecond * assumptions.flowFactor * Math.max(1, durationSeconds),
            ),
          3,
        ),
      };
    ...
    .sort(
      (left, right) =>
        ...
        right.affectedOccupancyPersons - left.affectedOccupancyPersons ||
        left.objectId.localeCompare(right.objectId),
    );
  const accessibleRouteIds = [
    ...new Set(infrastructure.sections.flatMap((section) => (section.pathAccessible ? section.routeObjectIds : []))),
  ].sort();
  const accessibleAvailable =
    infrastructure.sections.every((section) => section.pathAccessible) && infrastructure.sections.length > 0;
  return {
    mode,
    sampleCount,
    ...
    p95ClearanceSeconds: round(
      percentile(
        sampleOutputs.map((sample) => sample.totalClearanceSeconds),
        0.95,
      ),
      1,
    ),
    ...
    accessibleRoutePerformance: {
      ...
      p95ClearanceSeconds: round(
        percentile(
          sampleOutputs.map((sample) => sample.accessibleClearanceSeconds),
          0.95,
        ),
        1,
      ),
      relativeDelayPercent: round(
        (mean(sampleOutputs.map((sample) => sample.accessibleClearanceSeconds)) /
          Math.max(1, mean(sampleOutputs.map((sample) => sample.totalClearanceSeconds))) -
          1) *
          100,
        1,
      ),
...
const densityLevel = (density: number): "critical" | "high" | "medium" | "low" =>
  density >= 3 ? "critical" : density >= 1.5 ? "high" : density >= 0.5 ? "medium" : "low";

type AssumptionResult = ReturnType<typeof simulateAssumption>;
const densityFrames = (scenario: IngressEgressScenario, infrastructure: Infrastructure, result: AssumptionResult) => {
  const clearance = Math.ceil(
    Math.max(1, result.p95ClearanceSeconds, ...result.zones.map((zone) => zone.p95ClearanceSeconds)),
  );
  const seconds = [
    ...new Set([0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1].map((ratio) => Math.round(clearance * ratio))),
  ];
  return seconds.map((second, index) => {
    ...
    const sectionCells = infrastructure.sections.map((section) => {
      ...
      const remaining = Math.max(
        0,
        (zone?.occupancyPersons ?? 0) * (1 - clamp(second / Math.max(1, zone?.p95ClearanceSeconds ?? clearance), 0, 1)),
      );
      const density = remaining / Math.max(1, footprintArea(section.footprint));
      return {
        id: `density-${section.objectId}`,
        objectId: section.objectId,
        kind: "zone",
        point: section.point,
        occupancyPersons: round(remaining, 1),
        densityPersonsPerM2: round(density, 3),
        level: densityLevel(density),
      };
    });
    const routeCells = infrastructure.corridors.map((route) => {
      const affected = result.bottlenecks
        .filter((bottleneck) => bottleneck.objectId === route.id)
        .reduce((sum, bottleneck) => sum + bottleneck.affectedOccupancyPersons, 0);
      const activeShare = Math.sin(Math.PI * progress);
      const occupancy =
        (affected || scenario.inputs.population / Math.max(1, infrastructure.corridors.length)) * activeShare * 0.08;
      ...
      return {
        ...
        kind: "route",
        point: route.point,
        occupancyPersons: round(occupancy, 1),
        densityPersonsPerM2: round(density, 3),
        level: densityLevel(density),
      };
    });
    ...
    return {
      id: `density-frame-${index + 1}`,
      second,
      progress: round(progress, 3),
      cells,
      peakDensityPersonsPerM2: round(Math.max(0, ...cells.map((cell) => cell.densityPersonsPerM2)), 3),
    };
  });
};

const simulateIngress = (scenario: IngressEgressScenario, infrastructure: Infrastructure) => {
  const components = [
    ...infrastructure.entrances.map((entrance) => ({
      objectId: entrance.id,
      kind: "entrance",
      capacityPersonsPerSecond: (entrance.clearWidthM ?? 1.2) * 1.3,
    })),
    ...infrastructure.doors.map((door) => ({
      objectId: door.id,
      kind: "door",
    ...
    ...infrastructure.checkpoints.map((checkpoint) => ({
      objectId: checkpoint.id,
      kind: "checkpoint",
      capacityPersonsPerSecond: checkpoint.capacityPersonsPerMinute / 60,
    })),
    ...infrastructure.corridors
      .filter((route) => route.kind === "corridor" || route.id.includes("main"))
      .map((route) => ({
        objectId: route.id,
        kind: route.kind,
        capacityPersonsPerSecond: route.blockedByObjectIds.length ? 0 : route.widthM * 1.3,
    ...
    (left, right) =>
      left.capacityPersonsPerSecond - right.capacityPersonsPerSecond || left.objectId.localeCompare(right.objectId),
  );
  const bottleneck = components.find((component) => component.capacityPersonsPerSecond > 0) ?? {
    objectId: "unavailable",
    kind: "entrance",
    capacityPersonsPerSecond: 0,
  };
  const capacityPersonsPerSecond = bottleneck.capacityPersonsPerSecond;
  let backlogPersons = 0;
  let peakBacklogPersons = 0;
  let previous = scenario.ingressEgress.curves.arrival[0] ?? { second: 0, cumulativeShare: 0 };
  ...
  const totalAdmissionSeconds =
    capacityPersonsPerSecond > 0
      ? curveEndSecond + backlogPersons / capacityPersonsPerSecond
      : scenario.horizonSeconds * 2;
  return {
    peakArrivalRatePerMinute: round(
      peakCurveRate(scenario.ingressEgress.curves.arrival, scenario.inputs.population),
      1,
    ),
    ...
    p95AdmissionSeconds: round(
      Math.max(
        timeAtShare(scenario.ingressEgress.curves.arrival, 0.95),
        totalAdmissionSeconds - (scenario.inputs.population * 0.05) / Math.max(0.01, capacityPersonsPerSecond),
      ),
      1,
    ),
...
interface IngressSimulationInput {
  scenario: IngressEgressScenario;
  plan: VenuePlan;
...
export function simulateIngressEgress({
  scenario,
  plan,
  branchId = null,
  inputFingerprint,
  ...
  const result = {
    ...
    assumptionComparison: {
      ...
      emergencyMinusNormalAccessibleSeconds: round(
        emergency.accessibleRoutePerformance.p95ClearanceSeconds -
          normal.accessibleRoutePerformance.p95ClearanceSeconds,
        1,
      ),
    },
...
export const INGRESS_EGRESS_BENCHMARKS = Object.freeze([
  ...
  },
  {
    ...
    scenario: {
      model: "ingress-egress",
      id: "benchmark-summit-emergency",
      name: "Summit emergency",
      seed: 73421,
      horizonSeconds: 1800,
      sampleCount: 128,
      inputs: { population: 400, arrivalRatePerMinute: 40, serviceRatePerMinute: 30, servers: 2, mobilityFactor: 1 },
      ingressEgress: { mode: "emergency" },
    },
