...
import { getRoomTemplate } from "./venue-templates.js";

const clone = (value) => value === undefined ? undefined : JSON.parse(JSON.stringify(value));
const same = (left, right) => JSON.stringify(left) === JSON.stringify(right);
const overridden = (paths, path) => paths.some((item) => item === path || path.startsWith(`${item}.`) || item.startsWith(`${path}.`));
const record = (value) => value && typeof value === "object" && !Array.isArray(value);

const mergeTemplateValue = (before, after, current, path, overrides, preserved) => {
  ...
  if (record(before) && record(after)) {
    const value = { ...(record(current) ? clone(current) : {}) };
    let applied = false;
    for (const key of [...new Set([...Object.keys(before), ...Object.keys(after)])].sort()) {
      const childPath = `${path}.${key}`;
      const child = mergeTemplateValue(before[key], after[key], current?.[key], childPath, overrides, preserved);
...
const metadataFor = (object) => Object.fromEntries(Object.entries(object).filter(([key]) => !["id", "footprint"].includes(key)));

export function createRoomTemplateUpdateProposal(plan, { templateId, toVersion, actor = "agent" }) {
  ...
  const projectObjects = new Map((plan.objects ?? []).filter((object) => object.templateRef?.kind === "room-template" && object.templateRef.templateId === templateId).map((object) => [object.templateRef.templateObjectId, object]));
  const changes = [];
  const skipped = [];
  ...
  for (const templateObjectId of [...new Set([...fromObjects.keys(), ...toObjects.keys()])].sort()) {
    ...
    const preservedOverrides = new Set();
    for (const path of overrides) preservedOverrides.add(path);
    if (!same(before.footprint, after.footprint)) {
      const merged = mergeTemplateValue(before.footprint, after.footprint, instance.footprint, "footprint", overrides, preservedOverrides);
    ...
    for (const key of [...new Set([...Object.keys(beforeMetadata), ...Object.keys(afterMetadata)])].sort()) {
      ...
      const merged = mergeTemplateValue(beforeMetadata[key], afterMetadata[key], instance[key], key, overrides, preservedOverrides);
      if (merged.applied) values[key] = merged.value;
    }
    proposalOverrides.push(...[...preservedOverrides].map((path) => ({ projectObjectId: instance.id, templateObjectId, path })));
    ...
    if (spatialEffects.length) {
      const semantic = { templateId, fromVersion: binding.version, toVersion, templateObjectId, projectObjectId: instance.id, spatialEffects };
      changes.push({
  ...
  const proposalSeed = { planId: plan.id, baseVersion: plan.version, templateId, fromVersion: binding.version, toVersion, changeIds: changes.map((change) => change.id) };
  return {
    ...
    validation: null,
    waivers: [],
    templateUpdate: { kind: "room-template", templateId, fromVersion: binding.version, toVersion, actor, skipped, preservedOverrides: proposalOverrides.sort((left, right) => left.projectObjectId.localeCompare(right.projectObjectId) || left.path.localeCompare(right.path)) },
  };
}

export function applyApprovedTemplateBinding(plan, proposal) {
