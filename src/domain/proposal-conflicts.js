...
import { footprintsIntersect, materializeSpatialPlan } from "./spatial-analysis.js";

const clone = (value) => JSON.parse(JSON.stringify(value));

const acceptedPlanForVersion = (ledger, version) => ledger
  .slice()
  .reverse()
  .find((entry) => entry.details?.acceptedPlan?.version === version)
  ?.details.acceptedPlan ?? null;

const conflict = (branchId, type, suffix, fields) => ({
  id: `conflict-${branchId}-${type}-${suffix}`,
  type,
  ...fields,
});

const conflictPriority = { "stale-base": 0, "deleted-dependency": 1, "lock-conflict": 2, "same-object-edit": 3, "geometry-overlap": 4, "constraint-regression": 5 };
const collisionObject = (object) => object?.placement?.collisionMode === "solid" || object?.restriction?.blocksPlacement === true;

const spatialEffectsFor = (change, objectId) => (change.spatialEffects ?? []).filter((effect) => effect.objectId === objectId);
const spatialEffectsAlreadyApplied = (change, objectId, currentObject) => {
  const effects = spatialEffectsFor(change, objectId);
  if (effects.length === 0) return false;
  return effects.every((effect) => {
    if (effect.operation === "update_footprint") return Object.entries(effect.footprint).every(([key, value]) => stableFingerprint("value", currentObject.footprint[key]) === stableFingerprint("value", value));
    if (effect.operation === "update_metadata") return Object.entries(effect.values).every(([key, value]) => stableFingerprint("value", currentObject[key]) === stableFingerprint("value", value));
    return false;
  });
};

export function detectProposalConflicts(state, branchId = state.activeBranchId) {
  ...
  const lockConflictsByChangeAndObject = new Map(lockConflicts.map((item) => [`${item.changeId}:${item.objectId}`, item]));

  for (const change of proposal.changes) {
    const addedObjectIds = new Set((change.spatialEffects ?? []).filter((effect) => effect.operation === "add_object").map((effect) => effect.object?.id));
  ...
  const overlapChanges = proposal.changes.map((change) => ({ ...change, spatialEffects: (change.spatialEffects ?? []).filter((effect) => ["add_object", "update_room_boundary"].includes(effect.operation) || currentObjects.has(effect.objectId)) }));
  const candidatePlan = materializeSpatialPlan(state.plan, overlapChanges, { projectLocks: state.projectLocks ?? [], allowLockConflicts: true });
  ...
  for (const change of overlapChanges) {
    for (const objectId of [...new Set((change.spatialEffects ?? []).map((effect) => effect.objectId))].sort()) {
      const candidateObject = candidateObjects.get(objectId);
      if (!collisionObject(candidateObject)) continue;
      for (const other of candidatePlan.objects.filter((object) => object.id !== objectId && collisionObject(object)).sort((left, right) => left.id.localeCompare(right.id))) {
        ...
        const overlapIsNew = !previousObject || !previousOther || !footprintsIntersect(previousObject.footprint, previousOther.footprint);
  ...
  for (const check of validation.checks.filter((item) => item.status === "fail" && item.id !== "check-locked-objects")) {
  ...
  conflicts.sort((left, right) => (conflictPriority[left.type] ?? 99) - (conflictPriority[right.type] ?? 99) || left.id.localeCompare(right.id));
  return {
    ...
    baseVersion: proposal.baseVersion,
    currentVersion: state.plan.version,
    conflicts: clone(conflicts.map((item) => ({ ...item, blocking: item.severity === "error" && item.type !== "stale-base" }))),
