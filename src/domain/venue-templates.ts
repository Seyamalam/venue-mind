...
const rectangle: any = (x: any, y: any, width: any, depth: any, rotationDegrees: any = 0) => ({ kind: "rectangle", center: { x, y }, width, depth, rotationDegrees });
const line: any = (x1: any, y1: any, x2: any, y2: any, width: any) => ({ kind: "line", start: { x: x1, y: y1 }, end: { x: x2, y: y2 }, width });

const roomObject: any = (id: any, kind: any, label: any, layer: any, footprint: any, metadata: any = {}) => ({ id, kind, label, layer, elevationM: 0, footprint, ...metadata });

const harborviewObjects: any = (routeWidthM: any) => [
...
const genericBoundary: any = (width: any, depth: any) => ({ outer: [{ x: 0, y: 0 }, { x: width, y: 0 }, { x: width, y: depth }, { x: 0, y: depth }], holes: [] });

const starterRoom: any = (slug: any, name: any, useCase: any, width: any, depth: any, capacity: any) => ({
...
const inventory: any = (slug: any, name: any, category: any, dimensions: any, availability: any, metadata: any = {}) => ({
  schemaVersion: TEMPLATE_SCHEMA_VERSION,
...
const venueTemplates: any = [
  ...
  ...["conference", "concert", "banquet", "exhibition", "classroom", "community-event"].map((slug: any) => ({ schemaVersion: TEMPLATE_SCHEMA_VERSION, kind: "venue-template", id: `venue-template-${slug}-starter`, version: "1.0.0", name: `${slug.replaceAll("-", " ")} starter venue`, roomTemplateIds: [`room-template-${slug}`] })),
];

const inventoryTemplates: any = [
...
const findVersion: any = (items: any, id: any, version: any) => items.find((item: any) => item.id === id && item.version === version);

export const listVenueTemplates = () => clone(venueTemplateCatalog);

export const getRoomTemplate = (id: any, version: any) => {
  const template: any = findVersion(roomTemplates, id, version);
  if (!template) throw venueError("TEMPLATE_VERSION_NOT_FOUND", { templateKind: "room-template", templateId: id, version });
  return clone(template);
};

export const getInventoryTemplate = (id: any, version: any) => {
  const template: any = findVersion(inventoryTemplates, id, version);
  if (!template) throw venueError("TEMPLATE_VERSION_NOT_FOUND", { templateKind: "inventory-item-template", templateId: id, version });
  return clone(template);
};

export function assertCurrentTemplateDocument(document: any) {
...
export function evaluateInventoryAvailability(plan: any) {
  const demand: any = new Map();
  for (const object of plan.objects ?? []) {
    const reference: any = object.templateRef;
    if (reference?.kind !== "inventory-item-template") continue;
    const key: any = `${reference.templateId}@${reference.version}`;
    demand.set(key, (demand.get(key) ?? 0) + (object.inventoryCount ?? 1));
  }
  return [...demand.entries()].map(([key, requested]: any) => {
    const separator: any = key.lastIndexOf("@");
    const templateId: any = key.slice(0, separator);
    const version: any = key.slice(separator + 1);
    const template: any = getInventoryTemplate(templateId, version);
    const available: any = Math.max(0, template.availability.total - template.availability.unavailable);
  ...
  }).sort((left: any, right: any) => left.templateId.localeCompare(right.templateId));
}
