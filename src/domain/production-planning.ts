...
const angleBetween = (origin: Point, aim: Point, point: Point): number => {
...
const rayFootprint = (start: Point, end: Point): Footprint => ({ kind: "line", start, end, width: 0.01 });
type ProductionObject = VenueObject & { production: ProductionMetadata };
type PowerUtilityObject = VenueObject & { utility: UtilityMetadata };
const hasProduction = (object: VenueObject): object is ProductionObject => Boolean(object.production?.equipmentType);
const isPowerUtility = (object: VenueObject): object is PowerUtilityObject =>
  object.kind === "utility_point" && object.utility?.type === "power";

export const PRODUCTION_EQUIPMENT_TYPES = Object.freeze([
  "screen",
  "projector",
  "speaker",
  "camera",
...
export function normalizeProductionPolicy(value: Partial<ProductionPolicy> = {}): ProductionPolicy {
  const number = (
    field: keyof ProductionPolicy,
    fallback: number,
    minimum = 0,
    ...
    if (!Number.isFinite(normalized) || normalized < minimum || normalized > maximum)
      throw new Error(`Production policy ${field} is invalid`);
    return normalized;
  };
  return {
    schemaVersion: 1,
    minimumScreenVisibilityRatio: round(number("minimumScreenVisibilityRatio", 0.7, 0, 1)),
    minimumSpeakerCoverageRatio: round(number("minimumSpeakerCoverageRatio", 0.95, 0, 1)),
    minimumControlSightlineRatio: round(number("minimumControlSightlineRatio", 1, 0, 1)),
    allowedAccessibleCrossingTreatments: [
      ...new Set((value.allowedAccessibleCrossingTreatments ?? ["overhead", "cable-ramp"]).map(String)),
    ].sort(),
  };
}

const blockersForRay = (
  plan: VenuePlan,
  ...
  plan.objects
    .filter(
      (object) =>
        object.id !== sourceObjectId && object.id !== targetObjectId && Number(object.sightline?.opacity ?? 0) > 0,
    )
    .filter((object) => footprintsIntersect(rayFootprint(start, end), object.footprint))
    .map((object) => object.id)
    .sort();

export function analyzeProductionPlan(plan: VenuePlan) {
  const policy = normalizeProductionPolicy(plan.productionPolicy);
  const byId = new Map(plan.objects.map((object) => [object.id, object]));
  const equipment = plan.objects.filter(hasProduction).sort((left, right) => left.id.localeCompare(right.id));
  ...
  const throwDistanceChecks = projectors.map((projector) => {
    const targetObjectId = projector.production.targetObjectId;
    const target = targetObjectId ? byId.get(targetObjectId) : undefined;
    ...
    const throwRatio =
      typeof widthM === "number" && widthM > 0 && throwDistanceM != null ? round(throwDistanceM / widthM) : null;
    ...
    const status =
      ...
      throwRatio <= maximum
        ...
        : "fail";
    return {
      projectorObjectId: projector.id,
      screenObjectId: target?.id ?? projector.production.targetObjectId ?? null,
      throwDistanceM,
      viewableWidthM: widthM ?? null,
      throwRatio,
      minimumThrowRatio: minimum ?? null,
      maximumThrowRatio: maximum ?? null,
      status,
    };
  });
  ...
  const screenVisibility = screens.map((screen) => {
    ...
    const rays = seating.flatMap((section) =>
      (section.sightline?.samples ?? []).map((sample) => {
        const blockedByObjectIds = blockersForRay(plan, section.id, screen.id, sample.point, target);
        return {
          id: `production-ray-${sample.id}-${screen.id}`,
          sampleId: sample.id,
          seatingObjectId: section.id,
          screenObjectId: screen.id,
          viewingDistanceM: round(distance(sample.point, target), 2),
          blockedByObjectIds,
          status: blockedByObjectIds.length ? "blocked" : "clear",
        };
      }),
    );
    ...
    return {
      screenObjectId: screen.id,
      sampledSeats: rays.length,
      coverageRatio,
      minimumCoverageRatio: policy.minimumScreenVisibilityRatio,
      status: coverageRatio >= policy.minimumScreenVisibilityRatio ? "pass" : "fail",
      rays,
    };
  });
  ...
  const speakerCoverage = seating.map((section) => {
    ...
    const sampleResults = samples.map((sample) => {
      const coveringSpeakerObjectIds = speakers
        .filter((speaker) => {
          const origin = centerOf(speaker.footprint);
          const aim = speaker.production.aimPoint ?? origin;
          return (
            (speaker.production.targetZoneObjectIds ?? []).includes(section.id) &&
            distance(origin, sample.point) <= (speaker.production.coverageRangeM ?? 0) &&
            angleBetween(origin, aim, sample.point) <= (speaker.production.coverageAngleDegrees ?? 0) / 2
          );
        })
      ...
      return {
        sampleId: sample.id,
        coveringSpeakerObjectIds,
        status: coveringSpeakerObjectIds.length ? "covered" : "gap",
      };
    });
    const coverageRatio = sampleResults.length
      ? round(sampleResults.filter((sample) => sample.status === "covered").length / sampleResults.length)
      : 0;
    return {
      seatingObjectId: section.id,
      coverageRatio,
      minimumCoverageRatio: policy.minimumSpeakerCoverageRatio,
      status: coverageRatio >= policy.minimumSpeakerCoverageRatio ? "pass" : "fail",
      samples: sampleResults,
    };
  });

  const cameraChecks = cameras.map((camera) => {
    const targetObjectId = camera.production.targetObjectId;
    const target = targetObjectId ? byId.get(targetObjectId) : undefined;
    ...
    const blockers = target
      ? blockersForRay(plan, camera.id, target.id, centerOf(camera.footprint), centerOf(target.footprint))
      : [];
    const status =
      target &&
      cameraDistanceM !== null &&
      cameraDistanceM >= (camera.production.minimumDistanceM ?? 0) &&
      cameraDistanceM <= (camera.production.maximumDistanceM ?? 0) &&
      blockers.length === 0
        ? "pass"
        : "fail";
    return {
      cameraObjectId: camera.id,
      targetObjectId: target?.id ?? camera.production.targetObjectId ?? null,
      distanceM: cameraDistanceM,
      minimumDistanceM: camera.production.minimumDistanceM,
      maximumDistanceM: camera.production.maximumDistanceM,
      blockedByObjectIds: blockers,
      status,
    };
  });

  const controlSightlines = controls.flatMap((control) =>
    (control.production.targetObjectIds ?? []).map((targetObjectId) => {
      const target = byId.get(targetObjectId);
      const blockers = target
        ? blockersForRay(plan, control.id, target.id, centerOf(control.footprint), centerOf(target.footprint))
        : [];
      return {
        controlObjectId: control.id,
        targetObjectId,
        viewingDistanceM: target ? round(distance(centerOf(control.footprint), centerOf(target.footprint)), 2) : null,
        blockedByObjectIds: blockers,
        status: target && blockers.length === 0 ? "pass" : "fail",
      };
    }),
  );
  const controlSightlineRatio = controlSightlines.length
    ? round(controlSightlines.filter((check) => check.status === "pass").length / controlSightlines.length)
    : 0;

  const accessibleRoutes = plan.objects.filter(
    (object) =>
      ["accessible_route", "corridor", "aisle", "service_lane"].includes(object.kind) &&
      object.route?.accessible === true,
  );
  const cableCrossings = cables
    .flatMap((cable) =>
      accessibleRoutes
        .filter((route) => footprintsIntersect(cable.footprint, route.footprint))
        .map((route) => {
          const treatment = cable.production.crossingTreatment ?? "none";
          return {
            cableObjectId: cable.id,
            routeObjectId: route.id,
            treatment,
            status: policy.allowedAccessibleCrossingTreatments.includes(treatment) ? "pass" : "fail",
          };
        }),
    )
    .sort(
      (left, right) =>
        left.cableObjectId.localeCompare(right.cableObjectId) || left.routeObjectId.localeCompare(right.routeObjectId),
    );

  const poweredEquipment = equipment.filter((object) => Number(object.production.powerWatts ?? 0) > 0);
  const circuits = plan.objects
    .filter(isPowerUtility)
    .map((utility) => {
      const connected = poweredEquipment.filter((object) => object.production.circuitId === utility.utility.circuitId);
      const demandWatts = connected.reduce(
        (sum, object) => sum + (object.production.powerWatts ?? 0) * (object.inventoryCount ?? 1),
        0,
      );
      const capacityWatts = utility.utility.maxWatts ?? (utility.utility.powerKw ?? 0) * 1000;
      return {
        circuitId: utility.utility.circuitId,
        utilityObjectId: utility.id,
        capacityWatts,
        demandWatts,
        spareWatts: capacityWatts - demandWatts,
        connectedObjectIds: connected.map((object) => object.id).sort(),
        status: demandWatts <= capacityWatts ? "pass" : "fail",
      };
    })
  ...
  const unpoweredObjectIds = poweredEquipment
    .filter((object) => !object.production.circuitId || !knownCircuitIds.has(object.production.circuitId))
    .map((object) => object.id)
    .sort();

  const rigging = plan.objects
    .filter((object) => object.kind === "rigging_point")
    .map((point) => {
      const suspended = equipment.filter((object) => object.production.riggingPointId === point.id);
      const demandKg = round(
        suspended.reduce((sum, object) => sum + (object.production.weightKg ?? 0) * (object.inventoryCount ?? 1), 0),
        2,
      );
      const capacityKg = point.rigging?.safeWorkingLoadKg ?? 0;
      return {
        riggingPointObjectId: point.id,
        capacityKg,
        demandKg,
        spareKg: round(capacityKg - demandKg, 2),
        suspendedObjectIds: suspended.map((object) => object.id).sort(),
        status: demandKg <= capacityKg ? "pass" : "fail",
      };
    })
  ...
  const unresolvedRiggingObjectIds = equipment
    .filter(
      (object) =>
        object.production.requiresRigging === true &&
        (!object.production.riggingPointId || !riggingPointIds.has(object.production.riggingPointId)),
    )
    .map((object) => object.id)
    .sort();
  ...
  const result = {
    ...
    productionLayerObjectIds: plan.objects
      .filter((object) => object.layer === "production")
      .map((object) => object.id)
      .sort(),
    backstageZoneObjectIds: plan.objects
      .filter((object) => object.kind === "backstage_zone")
      .map((object) => object.id)
      .sort(),
    throwDistanceChecks,
    screenVisibility,
...
export function createProductionScheduleCsv(
  plan: VenuePlan,
  result: ReturnType<typeof analyzeProductionPlan> = analyzeProductionPlan(plan),
): string {
  const inventory = new Map(result.inventory.map((item) => [`${item.templateId}@${item.version}`, item]));
  const rows = plan.objects
    .filter(hasProduction)
    ...
    .map((object) => {
      ...
      return [
        ...
        object.label,
        object.kind,
        object.production.equipmentType,
        object.inventoryCount ?? 1,
        object.production.targetObjectId ?? object.production.targetObjectIds?.join("|") ?? "",
        object.production.circuitId ?? "",
        object.production.powerWatts ?? 0,
        object.production.riggingPointId ?? "",
        object.production.weightKg ?? 0,
        object.production.crossingTreatment ?? "",
        object.templateRef?.templateId ?? "",
        templateKey ? (inventory.get(templateKey)?.status ?? "unmeasured") : "unmeasured",
      ];
    });
  const header = [
    "object_id",
...
export function createProductionMapSvg(
  plan: VenuePlan,
  result: ReturnType<typeof analyzeProductionPlan> = analyzeProductionPlan(plan),
): string {
  ...
  const point = ({ x, y }: Point): Point => ({ x: 60 + (x - minX) * scale, y: 590 - (y - minY) * scale });
  const room = boundary
    .map((item) => {
      const p = point(item);
  ...
  const objects = plan.objects
    .filter((object) => object.layer === "production" || object.production?.equipmentType === "cable-route")
    .sort((left, right) => left.id.localeCompare(right.id))
    .map((object, index) => {
