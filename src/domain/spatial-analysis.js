...
import { assertNoLockConflicts } from "./locks.js";
...
const fingerprint = (prefix, value) => {
...
const pointInAxisAlignedRectangle = (point, footprint) => Math.abs(point.x - footprint.center.x) <= footprint.width / 2 + JOIN_TOLERANCE_M
  && Math.abs(point.y - footprint.center.y) <= footprint.depth / 2 + JOIN_TOLERANCE_M;

const pointTouchesFootprint = (point, footprint) => {
...
const polygonIntersectsFootprint = (polygon, footprint) => {
  if (footprint.kind === "circle") {
    if (pointInPolygon(footprint.center, polygon)) return true;
    return polygon.some((point, index) => distancePointToSegment(footprint.center, point, polygon[(index + 1) % polygon.length]) <= footprint.radius);
  }
  const other = footprintPolygon(footprint);
  if (!other.length) return false;
  return polygon.some((point, index) => other.some((otherPoint, otherIndex) => segmentsIntersect(point, polygon[(index + 1) % polygon.length], otherPoint, other[(otherIndex + 1) % other.length])))
    || pointInPolygon(polygon[0], other)
    || pointInPolygon(other[0], polygon);
};

export const footprintsIntersect = (left, right) => {
...
const distancePointToSegment = (point, start, end) => {
  ...
  const projection = Math.max(0, Math.min(1, ((point.x - start.x) * (end.x - start.x) + (point.y - start.y) * (end.y - start.y)) / lengthSquared));
  return distance(point, { x: start.x + projection * (end.x - start.x), y: start.y + projection * (end.y - start.y) });
};

const segmentIntersectsFootprint = (start, end, footprint) => {
...
export function materializeSpatialPlan(plan, changes = [], { projectLocks = [], allowLockConflicts = false } = {}) {
  ...
  for (const change of changes) {
    for (const effect of change.spatialEffects ?? []) {
      ...
      if (effect.operation === "update_footprint") object.footprint = { ...object.footprint, ...clone(effect.footprint) };
        ...
        objects.splice(objects.findIndex((item) => item.id === effect.objectId), 1);
...
const buildRouteGraph = (objects) => {
  const routeObjects = objects.filter((object) => ["accessible_route", "aisle", "corridor", "service_lane"].includes(object.kind) && object.footprint.kind === "line" && object.route?.staffOnly !== true);
  ...
  const nodeFor = (point) => {
    ...
    const node = { id: `node-${pointKey(point)}`, point: { x: round(point.x), y: round(point.y) }, edgeIds: [] };
  ...
  for (const object of routeObjects) {
    ...
    const edge = {
      ...
      blockedByObjectIds: blockers.filter((blocker) => blocker.id !== object.id && segmentIntersectsFootprint(object.footprint.start, object.footprint.end, blocker.footprint)).map((blocker) => blocker.id).sort(),
    };
    edges.push(edge);
    start.edgeIds.push(edge.id);
    end.edgeIds.push(edge.id);
  }
  const sortedNodes = [...nodes.values()].map((node) => ({ ...node, edgeIds: node.edgeIds.sort() })).sort((a, b) => a.id.localeCompare(b.id));
  const sortedEdges = edges.sort((a, b) => a.id.localeCompare(b.id));
  return { nodes: sortedNodes, edges: sortedEdges, routeObjects };
};

const ringArea = (points) => Math.abs(points.reduce((sum, point, index) => {
...
const reachableNodeIds = (graph, seedNodeIds) => {
...
const doorClearancePolygon = (door, side) => {
...
const exitApproachPolygon = (exit, roomBoundary, depthM) => {
  ...
  const inward = normals.sort((left, right) => ((right.x * (roomCenter.x - midpoint.x)) + (right.y * (roomCenter.y - midpoint.y))) - ((left.x * (roomCenter.x - midpoint.x)) + (left.y * (roomCenter.y - midpoint.y))))[0];
  return [
...
const accessibilityEvidence = (plan, sightlines) => {
  ...
  const nodesTouching = (object) => {
    const nodeIds = new Set(graph.nodes.filter((node) => pointTouchesFootprint(node.point, object.footprint)).map((node) => node.id));
  ...
  const reachableDestinationIds = destinations.filter((object) => nodesTouching(object).some((nodeId) => reachedNodes.has(nodeId))).map((object) => object.id).sort();
  const unreachableDestinationIds = destinations.map((object) => object.id).filter((id) => !reachableDestinationIds.includes(id)).sort();
  const junctions = graph.nodes.filter((node) => node.edgeIds.length >= 3);
  const edgeById = new Map(graph.edges.map((edge) => [edge.id, edge]));
  const turningClearanceM = junctions.length === 0 ? 0 : Math.min(...junctions.map((node) => Math.min(...node.edgeIds.map((id) => edgeById.get(id).widthM))));
  const minimumClearWidthM = graph.edges.length === 0 ? 0 : Math.min(...graph.edges.map((edge) => edge.widthM));
  const connected = entranceNodeIds.length > 0 && graph.nodes.length > 0 && graph.nodes.every((node) => reachedNodes.has(node.id)) && unreachableDestinationIds.length === 0;
  const policy = plan.accessibilityPolicy ?? {};
  const accessibleSeatingSections = plan.objects.filter((object) => object.kind === "seating_section" && object.accessibility?.accessibleSeats > 0).map((object) => ({
    objectId: object.id,
  ...
  const blockedAccessibleSeatSampleIds = accessibleSeatSightlineSections.flatMap((section) => section.blockedSampleIds).sort();
  const missingAccessibleSeatSampleSectionIds = accessibleSeatSightlineSections.filter((section) => section.sampleIds.length === 0).map((section) => section.objectId);
  const accessibleSeatSightlineCoverageRatio = accessibleSeatSampleIds.length ? round((accessibleSeatSampleIds.length - blockedAccessibleSeatSampleIds.length) / accessibleSeatSampleIds.length, 3) : 0;
  const accessibleDoors = plan.objects.filter((object) => object.kind === "door" && object.door?.accessible);
  const clearanceExemptKinds = new Set(["door", "fire_exit", "accessible_entrance", "accessible_route", "corridor", "aisle", "service_lane", "temporary_ramp"]);
  const doorClearanceZones = accessibleDoors.flatMap((door) => {
    ...
    const sides = door.door.clearance.side === "both" ? ["left", "right"] : [door.door.clearance.side];
    return sides.map((side) => {
      const points = doorClearancePolygon(door, side);
      const obstructingObjectIds = plan.objects.filter((object) => object.id !== door.id && !clearanceExemptKinds.has(object.kind) && object.accessibility?.clearanceExempt !== true && polygonIntersectsFootprint(points, object.footprint)).map((object) => object.id).sort();
  ...
  const minimumDoorClearWidthM = accessibleDoors.length ? Math.min(...accessibleDoors.map((door) => door.door.clearWidthM)) : 0;
  const obstructedDoorObjectIds = [...new Set(doorClearanceZones.filter((zone) => zone.status !== "clear").map((zone) => zone.doorObjectId))].sort();
  const ramps = plan.objects.filter((object) => object.kind === "temporary_ramp").map((object) => {
  ...
  return {
    ...
    companionAdjacencySatisfied: accessibleSeatingSections.every((section) => section.companionSeats >= section.accessibleSeats),
...
const capacityBand = (capacity, minimumCapacity, maximumCapacity) => {
...
const assertCapacityRange = (scopeKind, scopeId, minimumCapacity, maximumCapacity) => {
...
const capacitySnapshot = (plan, brief) => {
  const roomAreaM2 = ringArea(plan.spatial.roomBoundary.outer) - plan.spatial.roomBoundary.holes.reduce((sum, hole) => sum + ringArea(hole), 0);
  ...
  const seating = plan.objects.filter((object) => object.kind === "seating_section" && Number.isInteger(object.capacity) && object.capacity >= 0);
  const seatingIds = new Set(seating.map((object) => object.id));
  const zoneIds = new Set();
  for (const zone of plan.occupancy?.zones ?? []) {
    if (!zone.id || zoneIds.has(zone.id) || !Array.isArray(zone.sectionObjectIds) || zone.sectionObjectIds.some((objectId) => !seatingIds.has(objectId))) {
  ...
  const sectionCapacities = seating.map((object) => {
  ...
  const zoneCapacities = (plan.occupancy?.zones ?? []).map((zone) => {
  ...
  const densityM2PerAttendee = plan.occupancy?.densityM2PerAttendee ?? DENSITY_M2_PER_ATTENDEE[brief?.occupancyMode ?? "custom"];
  ...
  const explanations = [
    ...sectionCapacities.filter((section) => section.status !== "within-limit").map((section) => ({
    ...
    ...zoneCapacities.filter((zone) => zone.status !== "within-limit").map((zone) => ({
...
const capacityDelta = (baseline, candidate, changeId) => ({
  changeId,
  ...
  sectionDeltas: candidate.sectionCapacities.map((section) => {
  ...
  zoneDeltas: candidate.zoneCapacities.map((zone) => {
...
const capacityEvidence = (basePlan, candidatePlan, changes, brief, projectLocks) => {
  const baseline = capacitySnapshot(basePlan, brief);
  const candidate = capacitySnapshot(candidatePlan, brief);
  return {
    ...candidate,
    changeDeltas: changes.map((change) => capacityDelta(
      baseline,
...
const shortestPath = (graph, sourceNodeIds, targetNodeIds, excludedEdgeIds = new Set()) => {
  ...
  while (queue.size) {
    const currentId = [...queue].sort((left, right) => distances.get(left) - distances.get(right) || left.localeCompare(right))[0];
    ...
    for (const edgeId of node.edgeIds) {
      ...
      const candidate = distances.get(currentId) + edge.lengthM;
...
const circulationSnapshot = (plan, graph, capacity) => {
  const occupiedObjects = plan.objects.filter((object) => object.kind === "seating_section" && object.capacity > 0);
  const exits = plan.objects.filter((object) => object.kind === "fire_exit");
  const nodesTouching = (object) => graph.nodes.filter((node) => pointTouchesFootprint(node.point, object.footprint)).map((node) => node.id);
  const exitNodeEntries = exits.flatMap((exit) => nodesTouching(exit).map((nodeId) => [nodeId, exit.id]));
  const exitNodes = exitNodeEntries.map(([nodeId]) => nodeId);
  const exitByNodeId = new Map(exitNodeEntries);
  const shortestExitPaths = [];
  const disconnectedOccupiedObjectIds = [];
  ...
  const approachBlockers = plan.objects.filter((object) => !["fire_exit", "door", "accessible_route", "aisle", "corridor", "service_lane"].includes(object.kind) && object.circulation?.blocksExitApproach !== false);
  const exitApproachZones = exits.map((exit) => {
    const points = exitApproachPolygon(exit, plan.spatial.roomBoundary, exitApproachDepthM);
    const obstructingObjectIds = approachBlockers.filter((object) => polygonIntersectsFootprint(points, object.footprint)).map((object) => object.id).sort();
  ...
  const obstructedExitObjectIds = exitApproachZones.filter((zone) => zone.status === "blocked").map((zone) => zone.exitObjectId);
  ...
  const hardBlock = blockedEdges.length > 0 || disconnectedOccupiedObjectIds.length > 0 || obstructedExitObjectIds.length > 0;
  const congestionIndex = (demand) => hardBlock ? 1000 : bottleneckWidthM > 0 ? round(demand / (bottleneckWidthM * 3.6), 1) : 1000;
  const phaseProfiles = [
  ...
  const criticalRouteEdges = traversableEdges.map((edge) => {
    const impactedOccupiedObjectIds = occupiedObjects.filter((occupied) => {
  ...
  }).filter((edge) => edge.impactedOccupiedObjectIds.length > 0).sort((left, right) => left.edgeId.localeCompare(right.edgeId));
  ...
  const routeLoads = traversableEdges.map((edge) => {
    const demand = shortestExitPaths.filter((path) => path.routeObjectIds.includes(edge.objectId)).reduce((sum, path) => sum + (capacityByObjectId.get(path.occupiedObjectId) ?? 0), 0);
  ...
  const assignedCapacityByExit = new Map(exits.map((exit) => [exit.id, shortestExitPaths
  ...
  const exitLoads = exits.map((exit) => {
    ...
    const demand = assignedCapacityTotal > 0 ? round(capacity.operationalLoad * assignedCapacity / assignedCapacityTotal, 1) : 0;
  ...
  const junctionLoads = graph.nodes.filter((node) => node.edgeIds.length >= 3).map((node) => {
    const demand = shortestExitPaths.filter((path) => path.routeObjectIds.some((objectId) => node.edgeIds.some((edgeId) => graph.edges.find((edge) => edge.id === edgeId)?.objectId === objectId))).reduce((sum, path) => sum + (capacityByObjectId.get(path.occupiedObjectId) ?? 0), 0);
    const widthM = Math.min(...node.edgeIds.map((edgeId) => graph.edges.find((edge) => edge.id === edgeId)?.widthM ?? Number.MAX_SAFE_INTEGER));
  ...
  const controlLoads = plan.objects.filter((object) => ["queue", "checkpoint"].includes(object.circulation?.role)).map((object) => {
  ...
  const bottleneckLoads = [...routeLoads, ...exitLoads, ...junctionLoads, ...controlLoads].sort((left, right) => right.loadIndex - left.loadIndex || left.id.localeCompare(right.id));
  return {
    source: "canonical-geometry",
    graphFingerprint: fingerprint("graph", { nodes: graph.nodes, edges: graph.edges }),
...
const circulationDelta = (baseline, candidate, changeId) => {
  const baselinePaths = new Map(baseline.shortestExitPaths.map((path) => [path.occupiedObjectId, path.routeObjectIds]));
  const candidatePaths = new Map(candidate.shortestExitPaths.map((path) => [path.occupiedObjectId, path.routeObjectIds]));
  const changedPathObjectIds = [...new Set([...baselinePaths.keys(), ...candidatePaths.keys()])].filter((objectId) => stableStringify(baselinePaths.get(objectId) ?? null) !== stableStringify(candidatePaths.get(objectId) ?? null)).sort();
  return {
    changeId,
    peakCongestionIndexDelta: round(candidate.peakCongestionIndex - baseline.peakCongestionIndex, 1),
    changedPathObjectIds,
    newlyDisconnectedOccupiedObjectIds: candidate.disconnectedOccupiedObjectIds.filter((id) => !baseline.disconnectedOccupiedObjectIds.includes(id)),
    resolvedDisconnectedOccupiedObjectIds: baseline.disconnectedOccupiedObjectIds.filter((id) => !candidate.disconnectedOccupiedObjectIds.includes(id)),
    blockedRouteObjectIds: candidate.blockedRouteObjectIds,
  };
};

const circulationEvidence = (basePlan, candidatePlan, candidateGraph, capacity, changes, brief, projectLocks) => {
...
const sightlineEvidence = (plan) => {
  const focalPoints = plan.objects.flatMap((object) => (object.sightline?.focalPoints ?? []).map((focal) => ({ ...focal, objectId: object.id })));
  const focal = focalPoints.find((item) => item.priority === "primary") ?? focalPoints[0] ?? null;
  const samples = plan.objects.flatMap((object) => (object.sightline?.samples ?? []).map((sample) => ({ ...sample, seatingObjectId: object.id })));
  ...
  const rays = samples.map((sample) => {
    const viewingDistanceM = distance(sample.point, focal.point);
    const blockers = obstructions.filter((object) => object.id !== sample.seatingObjectId
    ...
    const horizontalAngleDegrees = round((Math.atan2(focal.point.y - sample.point.y, focal.point.x - sample.point.x) * 180) / Math.PI, 1);
    const verticalAngleDegrees = round((Math.atan2(focal.elevationM - sample.eyeHeightM, viewingDistanceM) * 180) / Math.PI, 1);
    return {
  ...
  const blockedSampleIds = rays.filter((ray) => ray.status === "blocked").map((ray) => ray.sampleId);
  const sectionSummaries = plan.objects.filter((object) => object.kind === "seating_section").map((object) => {
...
export function analyzeSpatialPlan({ plan, changes = [], brief = null, projectLocks = [] }) {
