...
const DEFAULT_PHASE_FRACTIONS: any = Object.freeze([0, 0.25, 0.3125, 0.625, 0.6875, 0.875, 1]);
...
const sourceObjectIds: any = (plan: any, candidates: any) => candidates.filter((id: any) => plan.objects.some((object: any) => object.id === id));
const owner: any = (plan: any, roleId: any, shiftId: any = null, staffPostObjectId: any = null) => ({
  roleId: plan.staffing?.roles?.some((role: any) => role.id === roleId) ? roleId : null,
  shiftId: plan.staffing?.shifts?.some((shift: any) => shift.id === shiftId) ? shiftId : null,
  staffPostObjectId: plan.objects.some((object: any) => object.id === staffPostObjectId) ? staffPostObjectId : null,
  assigneeId: null,
});

const phaseInstant: any = (schedule: any, fraction: any) => {
  const start: any = Date.parse(schedule.startAt);
  const end: any = Date.parse(schedule.endAt);
  return new Date(start + ((end - start) * fraction)).toISOString();
};

export function deriveRunbookPhases(scheduleInput: any) {
  const schedule: any = normalizeEventSchedule(scheduleInput, { label: "Event Day Runbook schedule" });
  return RUNBOOK_PHASE_KINDS.map((kind: any, order: any) => ({
...
const defaultTaskInputs: any = (plan: any) => [
...
export function deriveRunbookTasks(plan: any, phases: any) {
  const phaseByKind: any = new Map(phases.map((phase: any) => [phase.kind, phase]));
  const inputs: any = defaultTaskInputs(plan);
  const idByKey: any = new Map(inputs.map((task: any) => [task.key, `runbook-task-${task.key}`]));
  const counters: any = new Map();
  return inputs.map((task: any) => {
    const order: any = counters.get(task.phase) ?? 0;
    ...
    return {
      ...
      owner: task.owner,
      dependencyTaskIds: (task.after ?? []).map((key: any) => idByKey.get(key)).sort(),
...
const failDefinition: any = (reason: any, details: any = {}) => {
  throw venueError("RUNBOOK_DEFINITION_INVALID", { reason, ...details });
};

const assertUnique: any = (values: any, reason: any) => {
  if (values.some((value: any) => typeof value !== "string" || !value) || new Set(values).size !== values.length) failDefinition(reason);
};

export function validateRunbookDefinition({ plan, phases, tasks }: any) {
  ...
  assertUnique(phases.map((phase: any) => phase.id), "phase-id-invalid");
  if (phases.some((phase: any, order: any) => phase.kind !== RUNBOOK_PHASE_KINDS[order] || phase.order !== order || Date.parse(phase.endAt) <= Date.parse(phase.startAt))) failDefinition("phase-order-invalid");
  assertUnique(tasks.map((task: any) => task.id), "task-id-invalid");
  assertUnique(tasks.map((task: any) => task.key), "task-key-invalid");
  const phaseIds: any = new Set(phases.map((phase: any) => phase.id));
  const taskIds: any = new Set(tasks.map((task: any) => task.id));
  const objectIds: any = new Set(plan.objects.map((object: any) => object.id));
  const roleIds: any = new Set((plan.staffing?.roles ?? []).map((role: any) => role.id));
  const shiftIds: any = new Set((plan.staffing?.shifts ?? []).map((shift: any) => shift.id));
  if (tasks.some((task: any) => !phaseIds.has(task.phaseId) || !RUNBOOK_WORKSTREAMS.includes(task.workstream) || !RUNBOOK_TASK_STATUSES.includes(task.status))) failDefinition("task-enum-invalid");
  if (tasks.some((task: any) => task.dependencyTaskIds.some((id: any) => !taskIds.has(id)) || task.planObjectIds.some((id: any) => !objectIds.has(id)))) failDefinition("task-reference-missing");
  if (tasks.some((task: any) => (task.owner.roleId && !roleIds.has(task.owner.roleId)) || (task.owner.shiftId && !shiftIds.has(task.owner.shiftId)) || (task.owner.staffPostObjectId && !objectIds.has(task.owner.staffPostObjectId)))) failDefinition("task-owner-reference-missing");
  const visiting: any = new Set();
  const visited: any = new Set();
  const byId: any = new Map(tasks.map((task: any) => [task.id, task]));
  ...
  tasks.forEach((task: any) => visit(task.id));
  return true;
}

const createLedgerEntry: any = (runbook: any, type: any, details: any, metadata: any, committedAt: any) => {
  const sequence: any = runbook.ledger.length + 1;
  const previousHash: any = runbook.ledger.at(-1)?.hash ?? runbook.source.sourceLedgerHeadHash;
  const entry: any = {
...
export function createEventDayRunbook({ projectId, plan, brief = plan?.brief, validation, sourceLedgerHeadHash, approvalLedgerEntryId, frozenAt, frozenBy, version = 1, phases: suppliedPhases, tasks: suppliedTasks }: any) {
  if (!brief?.schedule) throw venueError("RUNBOOK_SCHEDULE_REQUIRED", { projectId, planId: plan?.id ?? null, planVersion: plan?.version ?? null });
  const schedule: any = normalizeEventSchedule(brief.schedule, { label: "Accepted Event Brief schedule" });
  ...
  const acceptedPlan: any = clone(plan);
  const acceptedBrief: any = clone(brief);
  const staffingEvidence: any = analyzeStaffingOperations(acceptedPlan);
  const phases: any = clone(suppliedPhases ?? deriveRunbookPhases(schedule));
  const tasks: any = clone(suppliedTasks ?? deriveRunbookTasks(acceptedPlan, phases));
  ...
  const baseline: any = { acceptedPlan, acceptedBrief, staffingEvidence };
  baseline.fingerprint = stableFingerprint("runbook-baseline", baseline);
  const definitionFingerprint: any = stableFingerprint("runbook-definition", { source, baselineFingerprint: baseline.fingerprint, phases, tasks: tasks.map(({ status: _status, revision: _revision, evidence: _evidence, ...task }: any) => task) });
  const id: any = `runbook-${projectId}-${acceptedPlan.id}`;
  const versionId: any = `${id}-${definitionFingerprint.slice(-8)}`;
  const createdAt: any = frozenAt ?? new Date().toISOString();
  const runbook: any = {
    schemaVersion: 1,
    id,
    versionId,
...
export function taskReadiness(runbook: any, taskId: any) {
  const task: any = runbook.tasks.find((candidate: any) => candidate.id === taskId);
  if (!task) throw venueError("RUNBOOK_TASK_NOT_FOUND", { runbookVersionId: runbook.versionId, taskId });
  const byId: any = new Map(runbook.tasks.map((candidate: any) => [candidate.id, candidate]));
  const completedDependencyIds: any = task.dependencyTaskIds.filter((id: any) => byId.get(id)?.status === "completed");
...
const normalizeEvidence: any = (evidence: any = []) => {
  if (!Array.isArray(evidence)) throw venueError("RUNBOOK_EVIDENCE_REQUIRED", { reason: "evidence-not-array" });
  const normalized: any = evidence.map((item: any) => {
    if (!item || typeof item.code !== "string" || !item.code || typeof item.ref !== "string" || !item.ref || Object.keys(item).some((key: any) => !["code", "ref"].includes(key))) throw venueError("RUNBOOK_EVIDENCE_REQUIRED", { reason: "evidence-invalid" });
    return { code: item.code, ref: item.ref };
  }).sort((left: any, right: any) => left.code.localeCompare(right.code) || left.ref.localeCompare(right.ref));
  return [...new Map(normalized.map((item: any) => [`${item.code}\u0000${item.ref}`, item])).values()];
};

const transitionInput: any = (command: any) => ({
...
export function transitionRunbookTask(runbook: any, command: any, { committedAt = new Date().toISOString() }: any = {}) {
  ...
  const input: any = transitionInput(command);
  const inputFingerprint: any = stableFingerprint("runbook-command", input);
  const existing: any = runbook.receipts.find((receipt: any) => receipt.idempotencyKey === command.idempotencyKey);
  if (existing) {
  ...
  const task: any = runbook.tasks.find((candidate: any) => candidate.id === command.taskId);
  ...
  const isReopen: any = ["completed", "skipped"].includes(task.status) && command.toStatus === "pending";
  const ordinary: any = TRANSITIONS[task.status]?.includes(command.toStatus);
  ...
  if (command.toStatus === "in-progress") {
    const readiness: any = taskReadiness(runbook, task.id);
  ...
  const evidence: any = normalizeEvidence(command.evidence);
  if (command.toStatus === "completed") {
    const present: any = new Set(evidence.map((item: any) => item.code));
    const missingEvidenceCodes: any = task.requiredEvidenceCodes.filter((code: any) => !present.has(code));
    if (missingEvidenceCodes.length) throw venueError("RUNBOOK_EVIDENCE_REQUIRED", { taskId: task.id, missingEvidenceCodes });
  }
  const nextTask: any = { ...clone(task), status: command.toStatus, revision: task.revision + 1, evidence };
  const transitionSequence: any = runbook.transitions.length + 1;
  ...
  const eventType: any = isReopen ? "runbook.task_reopened" : `runbook.task_${command.toStatus.replace("in-progress", "started")}`;
  const next: any = {
    ...clone(runbook),
    tasks: runbook.tasks.map((candidate: any) => candidate.id === task.id ? nextTask : clone(candidate)),
...
export function deriveRunbookHandoff(runbook: any, { at, outgoingAssignmentId, incomingAssignmentId, roleId = null }: any) {
  const instant: any = Date.parse(at);
  if (!Number.isFinite(instant)) failDefinition("handoff-instant-invalid");
  const phaseById: any = new Map(runbook.phases.map((phase: any) => [phase.id, phase]));
  const filtered: any = runbook.tasks.filter((task: any) => !roleId || task.owner.roleId === roleId);
  const ids: any = (predicate: any) => filtered.filter(predicate).map((task: any) => task.id).sort();
  const handoff: any = {
    schemaVersion: 1,
    runbookVersionId: runbook.versionId,
    ...
    roleId,
    taskIds: {
      pending: ids((task: any) => task.status === "pending"),
      active: ids((task: any) => task.status === "in-progress"),
      blocked: ids((task: any) => task.status === "blocked"),
      completed: ids((task: any) => task.status === "completed"),
      skipped: ids((task: any) => task.status === "skipped"),
      overdue: ids((task: any) => !["completed", "skipped"].includes(task.status) && Date.parse(phaseById.get(task.phaseId).endAt) < instant),
      evidenceGap: ids((task: any) => task.status === "completed" && task.requiredEvidenceCodes.some((code: any) => !task.evidence.some((item: any) => item.code === code))),
    },
  };
  return freeze({ ...handoff, fingerprint: stableFingerprint("runbook-handoff", handoff) });
}

export function verifyRunbookLedger(runbook: any) {
  let previousHash: any = runbook.source.sourceLedgerHeadHash;
