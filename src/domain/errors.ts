const define: any = (code: any, message: any, remediation: any) => Object.freeze({ code, message, remediation });
...
export const errorCatalog = Object.freeze({
  ...
  OCCUPANCY_LEDGER_INTEGRITY_FAILED: define("OCCUPANCY_LEDGER_INTEGRITY_FAILED", "Occupancy Monitor Ledger integrity failed.", "Restore the last verified monitor state before inspection or export."),
  ...
  INCIDENT_BASELINE_INVALID: define("INCIDENT_BASELINE_INVALID", "Incident Register baseline is invalid.", "Create the register from one active Event Day Runbook with its frozen accepted Plan and Emergency Plan evidence."),
  INCIDENT_REGISTER_NOT_FOUND: define("INCIDENT_REGISTER_NOT_FOUND", "Incident Register is unavailable.", "Create the register from the active Event Day Runbook before inspecting or reporting incidents."),
  INCIDENT_NOT_FOUND: define("INCIDENT_NOT_FOUND", "Operational Incident is unavailable.", "Inspect the Incident Register and retry with an existing stable Incident ID."),
  INCIDENT_ID_CONFLICT: define("INCIDENT_ID_CONFLICT", "Operational Incident ID conflicts with an existing record.", "Use the original semantic report or generate a new stable Incident ID."),
  INCIDENT_INVALID: define("INCIDENT_INVALID", "Operational Incident input is invalid.", "Use the published severity, category, owner, reason, reference, and transition fields."),
  INCIDENT_PRIVACY_REJECTED: define("INCIDENT_PRIVACY_REJECTED", "Incident input contains prohibited person-level data.", "Remove identities, contacts, ticket, device, attendee, medical, and free-form person records."),
  INCIDENT_LOCATION_INVALID: define("INCIDENT_LOCATION_INVALID", "Incident Location Context is invalid.", "Anchor the incident to one object or in-room coordinate in the frozen accepted Plan."),
  INCIDENT_REVISION_CONFLICT: define("INCIDENT_REVISION_CONFLICT", "Operational Incident revision conflict.", "Refresh the incident and retry only if the transition is still required."),
  INCIDENT_TRANSITION_INVALID: define("INCIDENT_TRANSITION_INVALID", "Operational Incident transition is invalid.", "Use a published state transition and supply any required structured reason or resolution code."),
  INCIDENT_OWNER_INVALID: define("INCIDENT_OWNER_INVALID", "Incident owner evidence is invalid.", "Assign one structured Runbook role, shift, Staff Post, or assignment reference."),
  INCIDENT_ACKNOWLEDGEMENT_INVALID: define("INCIDENT_ACKNOWLEDGEMENT_INVALID", "Incident acknowledgement is invalid.", "Use an authenticated human operator and an unacknowledged open incident."),
  INCIDENT_ESCALATION_INVALID: define("INCIDENT_ESCALATION_INVALID", "Incident escalation is invalid.", "Increase the structured escalation level without skipping required authority."),
  INCIDENT_HANDOFF_INVALID: define("INCIDENT_HANDOFF_INVALID", "Incident handoff is invalid.", "Use the current owner, a different structured next owner, and bounded open action codes."),
  INCIDENT_EMERGENCY_ACTION_INVALID: define("INCIDENT_EMERGENCY_ACTION_INVALID", "Emergency Action Invocation is invalid.", "Use an approved action, scenario, and object reference from the frozen Emergency Plan."),
  INCIDENT_ATTACHMENT_INVALID: define("INCIDENT_ATTACHMENT_INVALID", "Incident Attachment is invalid.", "Upload one allowlisted bounded file whose declared type, signature, and checksum agree."),
  INCIDENT_ATTACHMENT_NOT_FOUND: define("INCIDENT_ATTACHMENT_NOT_FOUND", "Incident Attachment is unavailable.", "Refresh the incident and retry with an available attachment ID."),
  INCIDENT_ATTACHMENT_UNAVAILABLE: define("INCIDENT_ATTACHMENT_UNAVAILABLE", "Incident Attachment storage is unavailable.", "Retry when the authenticated private evidence store is available."),
  INCIDENT_HUMAN_REQUIRED: define("INCIDENT_HUMAN_REQUIRED", "This Incident transition requires a human operator.", "Ask an authorized human operator to acknowledge, manage, hand off, attach evidence, or record emergency action."),
  INCIDENT_LEDGER_INTEGRITY_FAILED: define("INCIDENT_LEDGER_INTEGRITY_FAILED", "Incident Register ledger integrity failed.", "Restore the last verified register state before inspection or export."),
  INCIDENT_TOOL_UNAVAILABLE: define("INCIDENT_TOOL_UNAVAILABLE", "Incident tool Adapter is unavailable.", "Open VenueMind Studio or connect an MCP Project session with Incident Register operations enabled."),
...
export class VenueError extends Error {
  ...
  constructor(code: any, details: any = {}, message: any = null) {
    const entry: any = (errorCatalog as Record<string, any>)[code];
...
export const venueError = (code: any, details: any = {}, message: any = null) => new VenueError(code, details, message);

export const errorPayload = (error: any) => {
  const normalized: any = error instanceof VenueError ? error : new VenueError("VENUE_INTERNAL_ERROR");
