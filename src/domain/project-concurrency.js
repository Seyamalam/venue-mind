...
const changed = (base, value) => comparable(base) !== comparable(value);
...
export function projectEtag(projectId, revision) {
...
export function parseProjectEtag(value, projectId) {
...
export function reconcileProjectRecords({ base, local, remote }) {
  ...
  const localFields = PROJECT_MUTABLE_FIELDS.filter((field) => changed(base[field], local[field]));
  const remoteFields = PROJECT_MUTABLE_FIELDS.filter((field) => changed(base[field], remote[field]));
  const overlappingFields = localFields.filter((field) => remoteFields.includes(field) && changed(local[field], remote[field]));
  if (overlappingFields.length) {
    return {
      status: "conflict",
      kind: overlappingFields.some((field) => field === "snapshot" || field === "activePlanId") ? "planning" : "metadata",
      overlappingFields,
      localFields,
      remoteFields,
    };
...
export function projectConflictData({ base, local, remote, analysis }) {
  return {
    kind: analysis.kind,
    projectId: local.id,
