...
import { analyzeSpatialPlan, footprintsIntersect } from "./spatial-analysis.js";
...
const pointTouchesFootprint = (point, footprint, tolerance = .2) => {
  ...
  if (footprint?.kind === "polygon") {
    let inside = false;
    for (let current = 0, previous = footprint.points.length - 1; current < footprint.points.length; previous = current, current += 1) {
...
export function normalizeEmergencyPlan(value = {}) {
  ...
  const scenarioDefinitions = (value.scenarioDefinitions?.length ? value.scenarioDefinitions : defaultScenarios).map((scenario) => ({
  ...
  if (new Set(scenarioDefinitions.map((item) => item.id)).size !== scenarioDefinitions.length || scenarioDefinitions.some((item) => !item.id || !EMERGENCY_SCENARIO_TYPES.includes(item.type) || !Number.isInteger(item.durationMinutes) || item.durationMinutes <= 0)) throw new Error("Emergency scenarios require unique IDs, supported types, and positive durations");
  const requiredTypes = new Set(EMERGENCY_SCENARIO_TYPES);
  if ([...requiredTypes].some((type) => !scenarioDefinitions.some((item) => item.type === type))) throw new Error("Emergency Plan requires blocked-exit, unavailable-corridor, and power-loss scenarios");
  const authorizedReviewerRoles = uniqueStrings(value.authorizedReviewerRoles ?? EMERGENCY_REVIEWER_ROLES);
  if (!authorizedReviewerRoles.length || authorizedReviewerRoles.some((role) => !EMERGENCY_REVIEWER_ROLES.includes(role))) throw new Error("Emergency Plan requires supported authorized reviewer roles");
  const numeric = (field, fallback, integer = false) => {
...
const shortestPath = (graph, sourceNodeIds, targetNodeIds, excludedRouteObjectIds = new Set()) => {
...
const pathSet = (path) => `${path?.exitObjectId ?? "none"}:${(path?.routeObjectIds ?? []).join("|")}`;

const evaluateDegradedScenario = (plan, emergencyPlan, spatialEvidence, definition) => {
  ...
  const occupiedZones = plan.objects.filter((object) => object.kind === "seating_section" && object.capacity > 0);
  const unavailable = new Set(definition.unavailableObjectIds);
  const availableExits = exits.filter((exit) => !unavailable.has(exit.id));
  const exitNodes = new Map(availableExits.map((exit) => [exit.id, graph.nodes.filter((node) => pointTouchesFootprint(node.point, exit.footprint)).map((node) => node.id)]));
  const allExitNodeIds = [...exitNodes.values()].flat();
  const exitForNode = (nodeId) => availableExits.find((exit) => exitNodes.get(exit.id).includes(nodeId))?.id ?? null;
  const pathsFor = (excluded = new Set()) => occupiedZones.map((zone) => {
    const sourceNodeIds = graph.nodes.filter((node) => pointTouchesFootprint(node.point, zone.footprint)).map((node) => node.id);
  ...
  const baselineExitNodes = new Map(exits.map((exit) => [exit.id, graph.nodes.filter((node) => pointTouchesFootprint(node.point, exit.footprint)).map((node) => node.id)]));
  const baselineTargets = [...baselineExitNodes.values()].flat();
  const baselineExitForNode = (nodeId) => exits.find((exit) => baselineExitNodes.get(exit.id).includes(nodeId))?.id ?? null;
  const baselinePaths = occupiedZones.map((zone) => {
    const sources = graph.nodes.filter((node) => pointTouchesFootprint(node.point, zone.footprint)).map((node) => node.id);
  ...
  const excludedRoutes = new Set(definition.unavailableObjectIds.filter((id) => graph.edges.some((edge) => edge.objectId === id)));
  const scenarioPaths = pathsFor(excludedRoutes);
  const affectedZoneObjectIds = scenarioPaths.filter((path) => pathSet(path) !== pathSet(baselinePaths.find((item) => item.zoneObjectId === path.zoneObjectId))).map((path) => path.zoneObjectId).sort();
  const alternativeRoutes = scenarioPaths.filter((path) => affectedZoneObjectIds.includes(path.zoneObjectId) && path.status === "available").map((path) => ({ zoneObjectId: path.zoneObjectId, exitObjectId: path.exitObjectId, routeObjectIds: path.routeObjectIds, distanceM: path.distanceM }));
  const unreachableZoneObjectIds = scenarioPaths.filter((path) => path.status === "unavailable").map((path) => path.zoneObjectId).sort();
  const reachableExitObjectIds = [...new Set(scenarioPaths.map((path) => path.exitObjectId).filter(Boolean))].sort();
  const baselineExitCapacityPersons = exits.reduce((sum, exit) => sum + exit.exit.capacityPersons, 0);
  const availableExitCapacityPersons = reachableExitObjectIds.reduce((sum, id) => sum + objects.get(id).exit.capacityPersons, 0);
  const operationalLoadPersons = spatialEvidence.capacity.operationalLoad;
  const powerFailures = plan.objects.filter((object) => object.emergency?.powerSourceCircuitId && definition.unavailableCircuitIds.includes(object.emergency.powerSourceCircuitId) && Number(object.emergency.backupPowerMinutes ?? 0) < definition.durationMinutes).map((object) => ({ objectId: object.id, circuitId: object.emergency.powerSourceCircuitId, backupPowerMinutes: Number(object.emergency.backupPowerMinutes ?? 0), requiredMinutes: definition.durationMinutes })).sort((left, right) => left.objectId.localeCompare(right.objectId));
  const hardFailures = [
    ...
    ...powerFailures.map((failure) => ({ code: "BACKUP_POWER_SHORTFALL", affectedObjectIds: [failure.objectId], actual: failure.backupPowerMinutes, required: failure.requiredMinutes })),
  ];
  const result = {
    schemaVersion: 1,
...
export function analyzeEmergencyPlan(plan) {
  const emergencyPlan = normalizeEmergencyPlan(plan.emergencyPlan);
  const spatialEvidence = analyzeSpatialPlan({ plan }).evidence;
  const exits = plan.objects.filter((object) => object.kind === "fire_exit").sort((left, right) => left.id.localeCompare(right.id));
  const assemblyPoints = plan.objects.filter((object) => object.kind === "assembly_point").sort((left, right) => left.id.localeCompare(right.id));
  const accessLanes = plan.objects.filter((object) => object.kind === "emergency_access_lane").sort((left, right) => left.id.localeCompare(right.id));
  const fireEquipment = plan.objects.filter((object) => object.kind === "fire_equipment").sort((left, right) => left.id.localeCompare(right.id));
  const firstAidPosts = plan.objects.filter((object) => object.kind === "first_aid").sort((left, right) => left.id.localeCompare(right.id));
  const commandPosts = plan.objects.filter((object) => object.kind === "command_post").sort((left, right) => left.id.localeCompare(right.id));
  const operationalLoadPersons = spatialEvidence.capacity.operationalLoad;
  const totalExitCapacityPersons = exits.reduce((sum, exit) => sum + exit.exit.capacityPersons, 0);
  const totalAssemblyCapacityPersons = assemblyPoints.reduce((sum, point) => sum + point.emergency.capacityPersons, 0);
  const laneBlockers = plan.objects.filter((object) => object.occupancy?.excludesUsableArea && !["emergency_access_lane", "fire_equipment", "first_aid", "command_post"].includes(object.kind));
  const accessLaneChecks = accessLanes.map((lane) => {
    const obstructingObjectIds = laneBlockers.filter((object) => footprintsIntersect(lane.footprint, object.footprint)).map((object) => object.id).sort();
  ...
  const fireCoverage = occupiedZones.map((zone) => {
    const coveringEquipmentObjectIds = fireEquipment.filter((equipment) => distance(centerOf(zone.footprint), centerOf(equipment.footprint)) <= Number(equipment.emergency.coverageRadiusM ?? 0)).map((equipment) => equipment.id).sort();
    return { zoneObjectId: zone.id, coveringEquipmentObjectIds, status: coveringEquipmentObjectIds.length ? "covered" : "gap" };
  });
  const fireEquipmentCoverageRatio = fireCoverage.length ? round(fireCoverage.filter((item) => item.status === "covered").length / fireCoverage.length) : 0;
  const assemblyChecks = assemblyPoints.map((point) => ({ assemblyPointObjectId: point.id, capacityPersons: point.emergency.capacityPersons, designatedExitObjectIds: uniqueStrings(point.emergency.designatedExitObjectIds), missingExitObjectIds: uniqueStrings(point.emergency.designatedExitObjectIds).filter((id) => !exits.some((exit) => exit.id === id)), status: uniqueStrings(point.emergency.designatedExitObjectIds).length > 0 && uniqueStrings(point.emergency.designatedExitObjectIds).every((id) => exits.some((exit) => exit.id === id)) ? "pass" : "fail" }));
  const degradedScenarios = emergencyPlan.scenarioDefinitions.map((definition) => evaluateDegradedScenario(plan, emergencyPlan, spatialEvidence, definition));
  const structuralFailures = [
    ...(exits.length < emergencyPlan.minimumExitCount ? [{ code: "EXIT_COUNT", affectedObjectIds: exits.map((exit) => exit.id) }] : []),
    ...(totalExitCapacityPersons < emergencyPlan.minimumExitCapacityPersons || totalExitCapacityPersons < operationalLoadPersons ? [{ code: "EXIT_CAPACITY", affectedObjectIds: exits.map((exit) => exit.id) }] : []),
    ...(totalAssemblyCapacityPersons < emergencyPlan.minimumAssemblyCapacityPersons || assemblyChecks.some((check) => check.status === "fail") ? [{ code: "ASSEMBLY_CAPACITY", affectedObjectIds: assemblyPoints.map((point) => point.id) }] : []),
    ...accessLaneChecks.filter((check) => check.status === "fail").map((check) => ({ code: "EMERGENCY_ACCESS", affectedObjectIds: [check.laneObjectId, ...check.obstructingObjectIds] })),
    ...(fireEquipmentCoverageRatio < emergencyPlan.minimumFireEquipmentCoverageRatio ? [{ code: "FIRE_COVERAGE", affectedObjectIds: [...fireEquipment.map((item) => item.id), ...fireCoverage.filter((item) => item.status === "gap").map((item) => item.zoneObjectId)] }] : []),
    ...(firstAidPosts.length < emergencyPlan.requiredFirstAidPosts ? [{ code: "FIRST_AID", affectedObjectIds: firstAidPosts.map((item) => item.id) }] : []),
    ...(commandPosts.length < emergencyPlan.requiredCommandPosts ? [{ code: "COMMAND_POST", affectedObjectIds: commandPosts.map((item) => item.id) }] : []),
  ];
  const result = {
    schemaVersion: 1,
    ...
    structuralFailures,
    degradedScenarios,
    summary: { status: structuralFailures.length ? "fail" : "pass", structuralFailures: structuralFailures.length, degradedScenarioFailures: degradedScenarios.filter((scenario) => scenario.status === "fail").length, exitCount: exits.length, totalExitCapacityPersons, totalAssemblyCapacityPersons, accessibleEmergencyRoutes: spatialEvidence.accessibility.connected },
  };
  result.evidenceFingerprint = stableFingerprint("emergency-planning", result);
  return result;
}

export const emergencyChangeObjectIds = (plan, changes) => {
  const emergencyKinds = new Set(["fire_exit", "assembly_point", "emergency_access_lane", "fire_equipment", "first_aid", "command_post"]);
  const objects = new Map(plan.objects.map((object) => [object.id, object]));
  const changedEmergencyIds = (changes ?? []).flatMap((change) => {
    ...
    const addedObjects = (change.spatialEffects ?? [])
      .filter((effect) => effect.operation === "add_object" && effect.object)
      .map((effect) => effect.object)
