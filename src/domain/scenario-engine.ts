...
import { normalizeQueueInputs, simulateQueue } from "./queue-simulation.ts";
...
const positive: any = (value: any, label: any) => {
  const result: any = finite(value, label);
  if (result <= 0) throw new Error(`${label} must be greater than zero`);
  return result;
};

const normalizePhases: any = (phases: any, horizonSeconds: any) => {
  const source: any = phases?.length ? phases : [{ id: "phase-full", label: "Full horizon", startSecond: 0, endSecond: horizonSeconds, demandShare: 1 }];
  const ids: any = new Set();
  const normalized: any = source.map((phase: any, index: any) => {
    const id: any = String(phase.id ?? `phase-${index + 1}`).trim();
    if (!id || ids.has(id)) throw new Error("Scenario phases require unique stable IDs");
    ids.add(id);
    const startSecond: any = finite(phase.startSecond, `${id}.startSecond`);
    const endSecond: any = finite(phase.endSecond, `${id}.endSecond`);
  ...
  }).sort((left: any, right: any) => left.startSecond - right.startSecond || left.id.localeCompare(right.id));
  for (let index: any = 1; index < normalized.length; index += 1) {
    if (normalized[index].startSecond < normalized[index - 1].endSecond) throw new Error("Scenario phases cannot overlap");
  }
  const totalShare: any = normalized.reduce((sum: any, phase: any) => sum + phase.demandShare, 0);
  return normalized.map((phase: any) => ({ ...phase, demandShare: round(phase.demandShare / totalShare, 6) }));
};

export function normalizeScenarioDefinition(value: any) {
  const horizonSeconds: any = positive(value?.horizonSeconds ?? 3600, "horizonSeconds");
  const sampleCount: any = Math.trunc(positive(value?.sampleCount ?? 256, "sampleCount"));
  if (sampleCount > 10_000) throw new Error("sampleCount cannot exceed 10000");
  const seed: any = Math.trunc(finite(value?.seed ?? 1, "seed")) >>> 0;
  const inputs: any = {
  ...
  const model: any = value?.model ?? "operations";
  ...
  return {
    ...
    inputs: Object.fromEntries(Object.entries(inputs).map(([key, input]: any) => [key, round(input, 6)])),
...
const mulberry32: any = (seed: any) => () => {
  let value: any = seed += 0x6d2b79f5;
  value = Math.imul(value ^ value >>> 15, value | 1);
  value ^= value + Math.imul(value ^ value >>> 7, value | 61);
  return ((value ^ value >>> 14) >>> 0) / 4294967296;
};

const percentile: any = (values: any, ratio: any) => values[Math.min(values.length - 1, Math.max(0, Math.ceil(values.length * ratio) - 1))];

const geometryFactors: any = (plan: any) => {
  const routes: any = plan.objects.filter((object: any) => ["accessible_route", "corridor", "aisle", "service_lane"].includes(object.kind));
  const exits: any = plan.objects.filter((object: any) => object.kind === "fire_exit");
  const minimumRouteWidthM: any = routes.length ? Math.min(...routes.map((route: any) => route.footprint.width)) : 0.1;
  const exitCapacity: any = exits.reduce((sum: any, exit: any) => sum + (exit.exit?.capacityPersons ?? 0), 0) || 1;
  return { routeCount: routes.length, exitCount: exits.length, minimumRouteWidthM: round(minimumRouteWidthM), exitCapacity };
};

const simulatePhase: any = (scenario: any, phase: any, plan: any, random: any) => {
  const durationMinutes: any = (phase.endSecond - phase.startSecond) / 60;
  const demand: any = scenario.inputs.population * phase.demandShare;
  const geometry: any = geometryFactors(plan);
  const samples: any = Array.from({ length: scenario.sampleCount }, () => {
    const arrivalVariation: any = 0.84 + random() * 0.32;
    const serviceVariation: any = 0.88 + random() * 0.24;
    const mobilityVariation: any = (0.92 + random() * 0.16) * scenario.inputs.mobilityFactor;
    const arrivals: any = Math.min(demand * arrivalVariation, scenario.inputs.arrivalRatePerMinute * durationMinutes * arrivalVariation);
    const serviceCapacity: any = scenario.inputs.serviceRatePerMinute * scenario.inputs.servers * durationMinutes * serviceVariation;
    const routeCapacity: any = geometry.minimumRouteWidthM * 55 * durationMinutes / mobilityVariation;
    const exitCapacity: any = geometry.exitCapacity * Math.max(1, durationMinutes / 10);
    const processed: any = Math.min(arrivals, serviceCapacity, routeCapacity, exitCapacity);
    const backlog: any = Math.max(0, demand - processed);
    const utilization: any = demand / Math.max(1, Math.min(serviceCapacity, routeCapacity, exitCapacity));
    return { processed, backlog, utilization };
  });
  const sortedBacklog: any = samples.map((sample: any) => sample.backlog).sort((left: any, right: any) => left - right);
  const sortedUtilization: any = samples.map((sample: any) => sample.utilization).sort((left: any, right: any) => left - right);
  const mean: any = (key: any) => samples.reduce((sum: any, sample: any) => sum + sample[key], 0) / samples.length;
  return {
    phaseId: phase.id,
...
export function scenarioDefinitionFingerprint(scenario: any) {
  return stableFingerprint("scenario-definition", normalizeScenarioDefinition(scenario));
}

const normalizeResult: any = ({ scenario, plan, branchId, inputFingerprint, phaseResults }: any) => ({
  schemaVersion: 1,
  ...
  phases: phaseResults,
  summary: {
    meanProcessedPersons: round(phaseResults.reduce((sum: any, phase: any) => sum + phase.meanProcessedPersons, 0), 1),
    maximumP95BacklogPersons: round(Math.max(...phaseResults.map((phase: any) => phase.p95BacklogPersons)), 1),
    maximumP95Utilization: round(Math.max(...phaseResults.map((phase: any) => phase.p95Utilization)), 3),
  },
});

export function scenarioInputFingerprint(scenario: any, plan: any, branchId: any = null, engineVersion: any = SIMULATION_ENGINE_VERSION) {
...
export function createScenarioRunner({ engineVersion = SIMULATION_ENGINE_VERSION, scheduler = () => new Promise((resolve: any) => setTimeout(resolve, 0)) }: any = {}) {
  ...
  const cancelActive: any = (reason: any = "superseded") => {
    if (!active) return false;
    active.controller.abort(reason);
    return true;
  };

  const run: any = ({ scenario: sourceScenario, plan, branchId = null }: any, { onProgress = () => {} }: any = {}) => {
    const scenario: any = normalizeScenarioDefinition(sourceScenario);
    const inputFingerprint: any = scenarioInputFingerprint(scenario, plan, branchId, engineVersion);
    ...
    const promise: any = (async () => {
      ...
      if (["ingress-egress", "queue"].includes(scenario.model)) {
        const stages: any = scenario.model === "ingress-egress" ? ["infrastructure", "normal-egress", "emergency-egress"] : ["arrivals", "service", "spatial-preflight"];
        let partialResult: any = null;
        for (let index: any = 0; index < stages.length; index += 1) {
          await scheduler();
          ...
          const simulate: any = scenario.model === "ingress-egress" ? simulateIngressEgress : simulateQueue;
      ...
      const random: any = mulberry32(scenario.seed);
      for (let index: any = 0; index < scenario.phases.length; index += 1) {
        await scheduler();
        if (controller.signal.aborted) {
          const partialResult: any = phaseResults.length ? normalizeResult({ scenario, plan, branchId, inputFingerprint, phaseResults }) : null;
        ...
        onProgress({ runId, status: "running", progress: round((index + 1) / scenario.phases.length, 3), completedPhaseIds: phaseResults.map((phase: any) => phase.phaseId), partialResult: normalizeResult({ scenario, plan, branchId, inputFingerprint, phaseResults }) });
      }
      const result: any = normalizeResult({ scenario, plan, branchId, inputFingerprint, phaseResults });
...
export function compareSimulationResults(left: any, right: any) {
  ...
  const modelDeltas: any = left.model === "ingress-egress" ? {
...
export function exportSimulationRun(scenario: any, runResult: any) {
  if (runResult?.status !== "completed" || !runResult.result) throw new Error("Only a completed Simulation Run can be exported");
  const payload: any = { format: "venuemind-simulation", formatVersion: 1, scenario: normalizeScenarioDefinition(scenario), result: clone(runResult.result) };
