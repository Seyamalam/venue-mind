...
export const stableFingerprint = (prefix, value) => {
...
export function verifyActivityLedger(entries) {
  ...
  const valid = entries.length === expected.length && entries.every((entry, index) => (
...
export function normalizeActivityLedger(entries) {
  ...
  for (let index = 1; index < transitions.length; index += 1) {
    ...
    for (const object of (before.plan.objects ?? []).filter((item) => item.locked)) {
      ...
      if (!next) spatialEffects.push({ operation: "delete_object", objectId: object.id });
        ...
        const metadata = (value) => Object.fromEntries(Object.entries(value).filter(([key]) => !["footprint", "locks", "locked"].includes(key)));
      ...
      const conflicts = detectLockConflicts(before.plan, [{ id: `replay-${before.planVersion}-${after.planVersion}-${object.id}`, targetObjectIds: [], spatialEffects }]);
      const lockMetadataChanged = next && stableFingerprint("locks", object.locks ?? []) !== stableFingerprint("locks", next.locks ?? []);
