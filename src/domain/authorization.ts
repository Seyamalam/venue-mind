...
const viewer: any = ["project.read", "plan.read", "proposal.read", "comment.read", "simulation.read", "occupancy.read", "incident.read"];
const planner: any = [...viewer, "proposal.create", "proposal.manage", "comment.write", "simulation.run", "occupancy.write", "incident.report", "incident.manage", "incident.attachment", "export.plan"];
const reviewer: any = [...viewer, "proposal.review", "comment.write", "incident.export", "export.audit", "audit.read"];
const approver: any = [...reviewer, "approval.approve", "approval.waive"];
const safetyOfficer: any = [...new Set([...viewer, "incident.report", "incident.manage", "incident.attachment", "incident.emergency-act", "incident.export", "audit.read"])] ;
const venueAdministrator: any = [...new Set([...planner, ...reviewer, ...approver, "incident.emergency-act", "project.manage", "plan.update", "lock.manage"])] ;
...
const COMMAND_PERMISSION: any = Object.freeze({
...
const TOOL_PERMISSION: any = Object.freeze({
...
export const permissionForCommand = (commandType: any) => COMMAND_PERMISSION[commandType] ?? null;

export const permissionForTool = (toolName: any, requiredScope: any = null) => TOOL_PERMISSION[toolName]
...
const normalizedRoles: any = (roles: any) => [...new Set((roles ?? []).map(String))].sort();
const normalizedScopes: any = (scopes: any) => [...new Set((scopes ?? []).map(String))].sort();

export function createAgentGrant({ id, agentId, organizationId, projectId, scopes, issuedBy, issuedAt, expiresAt }: any, { clock = () => new Date().toISOString() }: any = {}) {
  const normalizedIssuedAt: any = issuedAt ?? clock();
  const normalizedExpiresAt: any = expiresAt;
  const issuedMs: any = Date.parse(normalizedIssuedAt);
  const expiresMs: any = Date.parse(normalizedExpiresAt);
  ...
  if (normalized.scopes.length === 0 || normalized.scopes.some((scope: any) => !AGENT_SCOPES.includes(scope))) throw venueError("AGENT_GRANT_INVALID", { reason: "unsupported-scope" });
...
export const createHumanPrincipal = ({ id, organizationId = null, roles, operationalRoles = [] }: any) => Object.freeze({
  type: "human",
  id: String(id ?? "").trim(),
...
export const createAgentPrincipal = ({ id }: any) => Object.freeze({ type: "agent", id: String(id ?? "").trim() });

export function createShortLivedAgentAuthorization({ agentId, organizationId = "org-local", projectId, scopes = AGENT_SCOPES, issuedBy = "local-host", delegatedBy = null, ttlMs = 15 * 60 * 1000, clock = () => new Date().toISOString() }: any) {
  const issuedAt: any = clock();
  const expiresAt: any = new Date(Date.parse(issuedAt) + ttlMs).toISOString();
  const principal: any = createAgentPrincipal({ id: agentId });
  const grant: any = createAgentGrant({
...
const decision: any = ({ status, reason, permission, principal, organizationId, projectId, grant, evaluatedAt }: any) => {
  const result: any = {
    status,
    reason,
    permission,
...
export function evaluateVenuePermission({ permission, principal, grant = null, delegatedBy = null, organizationId = null, projectId = null, approvalPolicy = DEFAULT_APPROVAL_POLICY, clock = () => new Date().toISOString() }: any) {
  const evaluatedAt: any = clock();
  ...
  if (principal.type === "human") {
    const roles: any = normalizedRoles(principal.roles);
    ...
    if (roles.some((role: any) => !HUMAN_ROLES.includes(role))) return decision({ status: "deny", reason: "role-unknown", permission, principal, organizationId, projectId, grant, evaluatedAt });
    const permissions: any = new Set(roles.flatMap((role: any) => (HUMAN_ROLE_PERMISSIONS as Record<string, readonly string[]>)[role] ?? []));
    ...
    if (permission === "approval.approve" && !roles.some((role: any) => approvalPolicy.requiredReviewerRoles.includes(role))) return decision({ status: "deny", reason: "approval-reviewer-role-required", permission, principal, organizationId, projectId, grant, evaluatedAt });
  ...
  const evaluatedMs: any = Date.parse(evaluatedAt);
  ...
  const permissions: any = new Set(grant.scopes.flatMap((scope: any) => (AGENT_SCOPE_PERMISSIONS as Record<string, readonly string[]>)[scope] ?? []));
  ...
  if (delegatedBy) {
    const delegatedDecision: any = evaluateVenuePermission({ permission, principal: delegatedBy, organizationId, projectId, approvalPolicy, clock: () => evaluatedAt });
    if (delegatedDecision.status !== "allow") return decision({ status: "deny", reason: "delegating-human-insufficient", permission, principal, organizationId, projectId, grant, evaluatedAt });
  }
  return decision({ status: "allow", reason: "agent-grant", permission, principal, organizationId, projectId, grant, evaluatedAt });
}

export function assertVenuePermission(input: any) {
  const result: any = evaluateVenuePermission(input);
  if (result.status === "deny") throw venueError("AUTHORIZATION_DENIED", clone(result));
  return result;
}

export function assertVenueCommand({ command, ...context }: any) {
  const permission: any = permissionForCommand(command?.type);
  if (!permission) throw venueError("COMMAND_UNSUPPORTED", { commandType: command?.type ?? null });
  const result: any = assertVenuePermission({ ...context, permission });
  const actor: any = command?.actor;
  const principalType: any = context.principal?.type;
  if (actor && ["human", "agent"].includes(principalType) && actor !== principalType) {
    const { id: _priorDecisionId, ...prior } = result;
    const mismatch: any = { ...prior, status: "deny", reason: "actor-principal-mismatch" };
