...
const uniqueRoles = (roles) => {
  const normalized = [...new Set((roles ?? []).map(String))].sort();
  if (!normalized.length || normalized.some((role) => !ORGANIZATION_ROLES.includes(role))) {
    throw new TypeError("Membership roles must contain published organization roles");
  }
  return Object.freeze(normalized);
};

export function createAuthenticatedIdentity({ provider, subject, email, displayName = null }) {
...
export function createOrganization({ id, name, slug, createdBy, createdAt }) {
  return Object.freeze({
...
export function createOrganizationMembership({ organizationId, userId, roles, status = "active", createdAt, updatedAt = createdAt }) {
...
export function createOrganizationInvitation({ id, organizationId, email, roles, invitedBy, createdAt, expiresAt, acceptedAt = null, revokedAt = null }) {
...
export function invitationStatus(invitation, at = new Date().toISOString()) {
...
export function createUserSession({ id, userId, createdAt, expiresAt, revokedAt = null, lastSeenAt = createdAt }) {
...
export const sessionStatus = (session, at = new Date().toISOString()) => session.revokedAt
  ? "revoked"
  : Date.parse(at) >= Date.parse(session.expiresAt) ? "expired" : "active";

export const canAdministerOrganization = (membership) => membership?.status === "active"
  && membership.roles.includes("organization-administrator");

export const createAccountAuditEvent = ({ id, organizationId, type, actorUserId, targetUserId = null, details = {}, occurredAt }) => Object.freeze({
...
export const personalOrganizationSlug = (identity) => `personal-${stableFingerprint("organization", {
