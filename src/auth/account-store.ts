...
const safeJson = async (response: any) => {
  ...
  if (!response.ok) {
    const error: any = new Error(value.error ?? "Account request failed");
...
export function createAccountStore({ fetchImpl = globalThis.fetch?.bind(globalThis), storage = typeof window === "undefined" ? undefined : window.localStorage }: any = {}) {
  ...
  const emit = (next: any) => { snapshot = Object.freeze(next); listeners.forEach((listener: any) => listener()); };
  const localFallback = () => {
    const organization = Object.freeze({ id: "org-local", name: "LOCAL", slug: "local", roles: Object.freeze(["organization-administrator", "venue-administrator"]) });
  ...
  const headers = (organizationId: any, extra: any = {}) => ({ ...extra, ...(organizationId ? { "x-venuemind-organization-id": organizationId } : {}) });
  ...
  return Object.freeze({
    ...
    async load() {
      ...
      loading = (async () => {
        try {
          ...
          const activeOrganizationId = first.organizations.some((organization: any) => organization.id === preferred) ? preferred : first.activeOrganizationId;
    ...
    selectOrganization(organizationId: any) {
      if (!snapshot.organizations.some((organization: any) => organization.id === organizationId)) throw new Error("ORGANIZATION_ACCESS_DENIED");
      storage?.setItem(ACTIVE_ORGANIZATION_KEY, organizationId);
      emit({ ...snapshot, activeOrganizationId: organizationId });
    },
    async createOrganization({ name, slug }: any) {
    ...
    async inviteMember({ email, roles, expiresAt }: any) {
    ...
    async acceptInvitation(token: any) {
    ...
    async setMembershipRoles(userId: any, roles: any) {
    ...
    async removeMembership(userId: any) {
