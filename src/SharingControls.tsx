...
const json = async (response: Response): Promise<DomainRecord> => {
...
const optionalJson = async (response: Response, fallback: DomainRecord): Promise<DomainRecord> => (response.headers.get("content-type") ?? "").includes("application/json") ? json(response) : fallback;
const headers = (organizationId: string, extra: Record<string, string> = {}) => ({ "x-venuemind-organization-id": organizationId, accept: "application/json", ...extra });
...
export function SharingControls({ projectId, organizationId, proposalId, canManage = false }: SharingControlsProps) {
  ...
  const [links, setLinks] = useState<DomainList>([]);
  const [scope, setScope] = useState("reviewer");
  const [days, setDays] = useState(7);
  const [createdUrl, setCreatedUrl] = useState("");
  const [notifications, setNotifications] = useState<DomainList>([]);
  const [preferences, setPreferences] = useState<DomainRecord>({ inAppEnabled: true, emailEnabled: false, eventTypes: [] as string[] });
  const [status, setStatus] = useState("READY");

  const run = useCallback(async <T,>(operation: () => Promise<T>): Promise<T | null> => { setStatus("BUSY"); try { const value = await operation(); setStatus("READY"); return value; } catch (error: unknown) { const cause = error as { code?: string; message?: string }; setStatus(cause.code ?? cause.message ?? "ERROR"); return null; } }, []);
  ...
  const revoke = async (id: string) => { await run(async () => { await json(await fetch(`/api/projects/${encodeURIComponent(projectId)}/share-links/${encodeURIComponent(id)}/revoke`, { method: "POST", credentials: "same-origin", headers: headers(organizationId) })); await loadLinks(); }); };
  const savePreferences = async (next: DomainRecord) => { const previous = preferences; setPreferences(next); const saved = await run(async () => json(await fetch("/api/notification-preferences", { method: "PUT", credentials: "same-origin", headers: headers(organizationId, { "content-type": "application/json" }), body: JSON.stringify(next) }))); if (!saved) setPreferences(previous); };
  const toggleEvent = (eventType: string, enabled: boolean) => void savePreferences({ ...preferences, eventTypes: enabled ? [...new Set([...(preferences.eventTypes as string[]), eventType])] : (preferences.eventTypes as string[]).filter((item) => item !== eventType) });
  const markRead = async (notificationId: string) => { await fetch(`/api/notifications/${encodeURIComponent(notificationId)}/read`, { method: "POST", credentials: "same-origin", headers: headers(organizationId) }); await loadNotifications(); };
