...
const stamp = (value) => value ? new Intl.DateTimeFormat("en", { hour: "2-digit", minute: "2-digit", second: "2-digit" }).format(new Date(value)) : "—";
...
function EmptyView({ title, icon: Icon = Radio, action, actionLabel }) {
...
export function OccupancyPanel({ open, monitor, projection, syncState = { state: "offline", pendingCount: 0 }, onClose, onCreate, onIngest, onRefresh, onAcknowledge, onSync, onExport }) {
    ...
    <SheetContent className="occupancy-panel !gap-0 !p-0 sm:!max-w-none" side="right" showOverlay={false} showCloseButton={false} aria-label="Live Occupancy">
      ...
      {!monitor || !projection ? <EmptyView title="RUNBOOK REQUIRED" icon={ShieldCheck} action={onCreate} actionLabel="CREATE" /> : <>
        ...
        <div className="occupancy-live-state" role={projection.overallStatus === "exceeded" ? "alert" : "status"} aria-live={projection.overallStatus === "exceeded" ? "assertive" : "polite"}>
          ...
          <Metric name="SOURCES" value={projection.sources.length} meta={`${projection.sources.filter((source) => source.status === "fresh").length} FRESH`} />
          <Metric name="ALERTS" value={monitor.activeAlerts.length} meta={`${monitor.activeAlerts.filter((alert) => alert.status === "acknowledged").length} ACK`} />
        </div>
        <Tabs defaultValue="scopes" className="occupancy-tabs">
          ...
          <ScrollArea className="occupancy-scroll">
            <TabsContent value="scopes" className="occupancy-list">
              {projection.scopes.map((scope) => <article className="occupancy-scope" key={scope.scopeId}>
            ...
            <TabsContent value="sources" className="occupancy-list">
              {projection.sources.length === 0 ? <EmptyView title="NO SOURCES" /> : projection.sources.map((source) => <article className="occupancy-source" key={`${source.sourceId}:${source.kind}`}>
            ...
            <TabsContent value="alerts" className="occupancy-list">
              {monitor.activeAlerts.length === 0 ? <EmptyView title="NO ALERTS" icon={ShieldCheck} /> : monitor.activeAlerts.map((alert) => <article className={`occupancy-alert is-${alert.severity}`} key={alert.id}>
            ...
            <TabsContent value="ledger" className="occupancy-list">
              {monitor.ledger.slice().reverse().map((entry) => <article className="occupancy-ledger" key={entry.id}>
