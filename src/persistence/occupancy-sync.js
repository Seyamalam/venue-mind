export async function synchronizeOccupancy({ projectId, monitorId, store, remote }) {
  ...
  const result = await remote.sync(projectId, monitorId, pending.map((entry) => entry.command));
  ...
  const conflict = result.acknowledgements.some((item) => ["conflict", "rejected"].includes(item.status));
