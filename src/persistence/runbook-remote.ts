const json = async (response: any) => {
  ...
  const error: any = new Error(payload.error?.message ?? payload.error ?? "Runbook request failed");
...
export function createRunbookRemote({ fetchImpl = globalThis.fetch?.bind(globalThis), organizationId }: any = {}) {
  ...
  const base = (projectId: any) => `/api/projects/${encodeURIComponent(projectId)}/runbooks`;

  return Object.freeze({
    async create(projectId: any, runbook: any) {
    ...
    async get(projectId: any, runbookVersionId: any) {
    ...
    async sync(projectId: any, runbookVersionId: any, commands: any) {
