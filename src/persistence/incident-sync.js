const lastSyncedAt = (register) => register?.updatedAt ?? null;

export async function synchronizeIncidents({ projectId, registerId, store, remote }) {
  ...
  const result = await remote.sync(projectId, registerId, pending.map((entry) => entry.command));
  ...
  const conflict = result.acknowledgements.some((item) => item.status === "conflict" || item.status === "rejected");
  return {
    ...result,
    acknowledgement,
    syncState: {
