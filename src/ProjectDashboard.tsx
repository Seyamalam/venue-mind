...
const projectVersion = (project: DomainRecord) => project.snapshot?.plan?.version ?? "—";
const projectEvent = (project: DomainRecord) => project.snapshot?.plan?.event ?? { attendeeTarget: 0, date: null };
const projectValidation = (project: DomainRecord) => {
  if (!project.snapshot?.plan || !project.snapshot?.proposal) return "SYNC";
  const activeBranch = project.snapshot.branches?.find((branch: DomainRecord) => branch.id === project.snapshot.activeBranchId);
  return (activeBranch?.proposal?.validation?.status ?? project.snapshot.proposal.validation?.status ?? "CHECK").toUpperCase();
};

type ProjectDashboardProps = { organizationId?: string; account?: DomainRecord; accountStore?: DomainRecord; navigate?: (href: string) => void };

export function ProjectDashboard({ organizationId = "org-local", account, accountStore, navigate = browserNavigate }: ProjectDashboardProps) {
  const store = useMemo(() => createProjectStore({ organizationId }), [organizationId]);
  const [projects, setProjects] = useState<DomainList>([]);
  ...
  const [importPreview, setImportPreview] = useState<DomainRecord | null>(null);
  const [importError, setImportError] = useState("");
  const [importing, setImporting] = useState(false);
  const [renameTarget, setRenameTarget] = useState<DomainRecord | null>(null);
  ...
  const replaceProject = (record: DomainRecord) => setProjects((current) => current.map((project) => project.id === record.id ? record : project));
  const openProject = async (project: DomainRecord) => {
  ...
  const openRename = (project: DomainRecord) => {
  ...
  const renameProject = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setImportError((error as { code?: string }).code ?? "RENAME_FAILED");
    } finally {
      setRenaming(false);
    }
  };
  const duplicateProject = async (project: DomainRecord) => {
  ...
  const archiveProject = async (project: DomainRecord, archived: boolean) => replaceProject((await store.archive(project.id, archived)).record);
  const pinProject = async (project: DomainRecord) => replaceProject((await store.pin(project.id, !project.pinned)).record);
  const openDelete = (project: DomainRecord) => {
  ...
  const deleteProject = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      setImportError((error as { code?: string }).code ?? "DELETE_FAILED");
    } finally {
      setDeleting(false);
    }
  };
  const restoreProject = async (project: DomainRecord) => replaceProject((await store.restoreDeleted(project.id)).record);
  ...
  const inspectImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportError((error as { code?: string }).code ?? "IMPORT_INVALID");
    } finally {
      event.target.value = "";
    }
  };
  ...
  const commitImport = async () => {
    ...
    try {
      ...
      setImportError((error as { code?: string }).code ?? "IMPORT_FAILED");
      setImporting(false);
    }
  };
    ...
    <div className="projects-shell">
      ...
      <Sheet open={Boolean(importPreview)} onOpenChange={(open) => { if (!open && !importing) setImportPreview(null); }}>
        {importPreview && <SheetContent className="import-preview !h-auto !gap-0 !p-0 sm:!max-w-none" side="right" showOverlay={false} showCloseButton={false} aria-label="Import Preview">
          ...
          <div className="import-preview-status"><CheckCircle size={24} weight="fill" /><div><strong>{importPreview.idConflict ? "ID CONFLICT" : "READY"}</strong><small>{importPreview.integrity.checksum.toUpperCase()} · {importPreview.integrity.schema.toUpperCase()} · {importPreview.integrity.ledger.toUpperCase()} · {importPreview.integrity.replay.toUpperCase()}</small></div></div>
