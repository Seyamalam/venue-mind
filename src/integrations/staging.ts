...
import { assertPlanningEffectBinding, normalizePlanningEffect } from "../domain/planning-effects.ts";
import { AdapterContractError, assertIsoTimestamp, normalizeAdapterChange, normalizeExternalReference, normalizeSyncCursor, sha256Checksum } from "./contracts.ts";
...
const proposalChange = (change: any, index: any) => {
  ...
  if (change.operation === "update") {
    const protectedFields = ["id", "kind", "locked", "locks", "templateRef", "templateOverrides"].filter((field: any) => Object.hasOwn(values, field));
...
export function createExternalIdMapping({ venueEntityType, venueObjectId, external, batchId, sourceSystem, sourceVersion, synchronizedAt, checksum }: any) {
...
const mappingIntegrityPayload = (mapping: any) => ({
...
export const stagingIntegrityPayload = (batch: any) => ({
...
export async function createAdapterStagingBatch(definition: any, input: any, options: any = {}) {
  if (!input || typeof input !== "object" || Array.isArray(input)) fail("ADAPTER_CONTRACT_INVALID", "Adapter import result must be an object");
  const unknown = Object.keys(input).filter((key: any) => !["sourceSystem", "sourceVersion", "synchronizedAt", "syncCursor", "changes", "mappings", "sourceRecords", "warnings"].includes(key));
  ...
  const changes = input.changes.map((change: any) => normalizeAdapterChange(change, definition));
  ...
  if (changes.some((change: any) => change.external.sourceSystem !== input.sourceSystem)) fail("ADAPTER_SOURCE_MISMATCH", "Every imported Change must belong to the batch source system");
  const proposedIds = changes.filter((change: any) => change.operation === "create").map((change: any) => change.proposedVenueObjectId);
  ...
  const sourceRecords = rawSourceRecords.map((record: any) => {
    ...
    const recordUnknown = Object.keys(record).filter((key: any) => !["external", "synchronizedAt", "descriptive"].includes(key));
    ...
    const descriptiveUnknown = Object.keys(descriptive).filter((key: any) => !["title", "location", "organizer"].includes(key));
    ...
    const locationUnknown = Object.keys(descriptive.location ?? {}).filter((key: any) => key !== "label");
    const organizerUnknown = Object.keys(descriptive.organizer ?? {}).filter((key: any) => !["displayName", "organization", "role"].includes(key));
  ...
  const rawMappingDrafts = input.mappings ?? changes.map((change: any) => ({ venueEntityType: change.venueEntityType, venueObjectId: change.operation === "create" ? change.proposedVenueObjectId : change.venueObjectId, external: change.external }));
  if (!Array.isArray(rawMappingDrafts)) fail("ADAPTER_CONTRACT_INVALID", "Import mappings must be an array");
  const mappingDrafts = rawMappingDrafts.map((mapping: any) => {
    if (!mapping || typeof mapping !== "object" || Array.isArray(mapping)) fail("ADAPTER_CONTRACT_INVALID", "Import mapping must be an object");
    const mappingUnknown = Object.keys(mapping).filter((key: any) => !["venueEntityType", "venueObjectId", "external"].includes(key));
  ...
  const mappingEvidence = mappingDrafts.map((mapping: any) => ({
    schemaVersion: 1,
    venueEntityType: mapping.venueEntityType,
    venueObjectId: mapping.venueObjectId,
  ...
  const draft: any = { schemaVersion: 1, status, adapterId: definition.id, adapterVersion: definition.version, sourceSystem: input.sourceSystem, sourceVersion: input.sourceVersion, synchronizedAt: input.synchronizedAt, basePlanVersion: options.basePlanVersion, proposalRevision: options.proposalRevision, syncCursor, mappings: mappingEvidence, sourceRecords: clone(sourceRecords), warnings: clone(input.warnings ?? []), proposal: proposalDraft };
  ...
  const mappings = mappingEvidence.map((mapping: any) => createExternalIdMapping({ ...mapping, batchId }));
  return deepFreeze({ ...draft, id: batchId, checksum, mappings, proposal });
}

export async function assertStagingBatchIntegrity(batch: any) {
  if (!batch || typeof batch !== "object" || Array.isArray(batch)) fail("ADAPTER_STAGING_INTEGRITY_FAILED", "Staging batch must be an object");
  const allowedBatchFields: any = ["schemaVersion", "id", "status", "adapterId", "adapterVersion", "sourceSystem", "sourceVersion", "synchronizedAt", "basePlanVersion", "proposalRevision", "checksum", "syncCursor", "mappings", "sourceRecords", "warnings", "proposal"];
  const unknownBatchFields = Object.keys(batch).filter((key: any) => !allowedBatchFields.includes(key));
  ...
  if (batch.proposal) {
    const unknownProposalFields = Object.keys(batch.proposal).filter((key: any) => !["id", "revision", "baseVersion", "status", "goal", "changes", "validation", "waivers"].includes(key));
  ...
  for (const mapping of batch.mappings) {
    const unknownMappingFields = Object.keys(mapping ?? {}).filter((key: any) => !["schemaVersion", "venueEntityType", "venueObjectId", "external", "batchId", "sourceSystem", "sourceVersion", "synchronizedAt", "checksum"].includes(key));
    const unknownExternalFields = Object.keys(mapping?.external ?? {}).filter((key: any) => !["adapterId", "sourceSystem", "entityType", "externalId", "sourceVersion", "checksum"].includes(key));
  ...
  if (!Array.isArray(batch.mappings) || batch.mappings.some((mapping: any) => mapping.batchId !== batch.id)) fail("ADAPTER_STAGING_INTEGRITY_FAILED", "External ID mappings must reference the checksum-derived batch ID");
  return true;
}

export function assertAdapterProjectContext(batch: any, context: any) {
  const projectMappings = batch.mappings.filter((mapping: any) => mapping.venueEntityType === "project");
  const planningEffects = batch.proposal?.changes.flatMap((change: any) => change.planningEffects ?? []) ?? [];
...
export async function assertReviewableStagingBatch(batch: any, projectContext: any = null, { requireProjectContext = true }: any = {}) {
  ...
  if (!/^[0-9a-f]{64}$/.test(batch.checksum) || !Array.isArray(batch.mappings) || batch.mappings.some((mapping: any) => mapping.sourceSystem !== batch.sourceSystem || mapping.synchronizedAt !== batch.synchronizedAt || !/^[0-9a-f]{64}$/.test(mapping.checksum ?? ""))) fail("ADAPTER_STAGING_INTEGRITY_FAILED", "Staging batch mapping evidence is invalid");
...
export async function loadAdapterProposalForReview(planner: any, batch: any) {
  ...
  const projectMapping = batch.mappings.find((mapping: any) => mapping.venueEntityType === "project") ?? null;
  if (projectMapping) {
  ...
  for (const change of batch.proposal.changes) {
    ...
    const acceptedObject = snapshot.plan.objects.find((object: any) => object.id === objectId);
  ...
  const activeBranch = snapshot.branches.find((branch: any) => branch.id === snapshot.activeBranchId);
  if (!activeBranch) fail("ADAPTER_ACTIVE_BRANCH_MISSING", "The active Proposal Branch is unavailable");
  const revisions: any = [...(activeBranch.revisions ?? [])];
  if (activeBranch.proposal?.id !== batch.proposal.id) revisions.push(clone(activeBranch.proposal));
  planner.execute({ type: "restore_snapshot", snapshot: { ...snapshot, proposal: clone(batch.proposal), branches: snapshot.branches.map((branch: any) => branch.id === snapshot.activeBranchId ? { ...branch, proposal: clone(batch.proposal), revisions } : branch) } });
