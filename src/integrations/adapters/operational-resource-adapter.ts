...
import { createAdapterStagingBatch } from "../staging.ts";
import { isNonContactLabel } from "../privacy.ts";
import { fingerprintPlan } from "../../domain/activity-ledger.ts";
import { normalizePreparedOperationalResourceInput, reconcileOperationalResources } from "../../domain/operational-resources.ts";
...
const boundedString = (value: any, label: any) => {
...
const records = (value: any, label: any) => {
...
const normalizeSourceBooking = (value: any, lookup: any) => {
...
const normalizeTemplateBinding = (value: any, label: any) => {
...
const normalizeCommonResource = async (record: any, family: any, mapping: any, reservationLookup: any) => {
  const common: any = ["externalId", "sourceVersion", "status", "total", "unavailable", "bookings"];
  ...
  const bookings = records(record.bookings, `Operational ${family} bookings`).map((item: any) => normalizeSourceBooking(item, reservationLookup)).sort((left: any, right: any) => compare(left.startAt, right.startAt) || compare(left.bookingRef, right.bookingRef));
  let capability: any;
  ...
  if (family === "power") {
    ...
    capability = { utilityObjectId: mapping.binding.utilityObjectId, circuitId: mapping.binding.circuitId, voltage: finite(record.voltage, "Operational power voltage", { positive: true }), maxWatts: finite(record.maxWatts, "Operational power maxWatts", { positive: true }), connectors: [...new Set(record.connectors.map((item: any) => boundedString(item, "Operational power connector")))].sort(compare) };
  }
  if (family === "catering") {
  ...
  const sourceRecord: any = { externalId, sourceVersion, status: record.status, total, unavailable, bookings, ...Object.fromEntries(extra.map((field: any) => [field, capability[field]])) };
...
const mapByExternal = (items: any, label: any, valueKey: any) => {
...
const normalizeTrustedContext = (value: any) => {
  ...
  const resourceMappings = records(value.resourceMappings, "Trusted operational resource mappings").map(normalizeMapping).sort((left: any, right: any) => compare(left.family, right.family) || compare(left.externalId, right.externalId));
  const roleMappings = records(value.roleMappings, "Trusted operational role mappings").map((item: any) => { assertExact(item, ["externalId", "roleId"], "Trusted role mapping"); return { externalId: identifier(item.externalId, "Trusted role externalId"), roleId: identifier(item.roleId, "Trusted roleId") }; }).sort((left: any, right: any) => compare(left.externalId, right.externalId));
  const shiftMappings = records(value.shiftMappings, "Trusted operational shift mappings").map((item: any) => { assertExact(item, ["externalId", "shiftId"], "Trusted shift mapping"); return { externalId: identifier(item.externalId, "Trusted shift externalId"), shiftId: identifier(item.shiftId, "Trusted shiftId") }; }).sort((left: any, right: any) => compare(left.externalId, right.externalId));
  const personnelMappings = records(value.personnelMappings, "Trusted personnel mappings").map((item: any) => { assertExact(item, ["externalPersonId", "staffRef", "resourceId"], "Trusted personnel mapping"); return { externalPersonId: identifier(item.externalPersonId, "Trusted external personnel ID"), staffRef: identifier(item.staffRef, "Trusted staffRef"), resourceId: identifier(item.resourceId, "Trusted staff resourceId") }; }).sort((left: any, right: any) => compare(left.externalPersonId, right.externalPersonId));
  const reservationMappings = records(value.reservationMappings, "Trusted reservation mappings").map((item: any) => { assertExact(item, ["externalId", "reservationRef"], "Trusted reservation mapping"); return { externalId: identifier(item.externalId, "Trusted reservation externalId"), reservationRef: identifier(item.reservationRef, "Trusted reservationRef") }; }).sort((left: any, right: any) => compare(left.externalId, right.externalId));
  const demands = clone(records(value.demands, "Trusted operational demands"));
  const demandTargetObjectIds = demands.flatMap((demand: any, index: any) => {
    ...
    return demand.targetObjectIds.map((item: any) => identifier(item, "Trusted demand targetObjectId"));
  });
  if (new Set(resourceMappings.map((item: any) => `${item.family}\u0000${item.externalId}`)).size !== resourceMappings.length
    || new Set(resourceMappings.map((item: any) => item.resourceId)).size !== resourceMappings.length
    || new Set(roleMappings.map((item: any) => item.externalId)).size !== roleMappings.length
    || new Set(roleMappings.map((item: any) => item.roleId)).size !== roleMappings.length
    || new Set(shiftMappings.map((item: any) => item.externalId)).size !== shiftMappings.length
    || new Set(shiftMappings.map((item: any) => item.shiftId)).size !== shiftMappings.length
    || new Set(personnelMappings.map((item: any) => item.externalPersonId)).size !== personnelMappings.length
    || new Set(personnelMappings.map((item: any) => item.staffRef)).size !== personnelMappings.length
    || new Set(personnelMappings.map((item: any) => item.resourceId)).size !== personnelMappings.length
    || new Set(reservationMappings.map((item: any) => item.externalId)).size !== reservationMappings.length
    || new Set(reservationMappings.map((item: any) => item.reservationRef)).size !== reservationMappings.length) {
    fail("ADAPTER_RESOURCE_MAPPING_INVALID", "Trusted operational mappings must be one-to-one");
  }
  if (resourceMappings.some((item: any) => !RESOURCE_ID.test(item.resourceId))
    || personnelMappings.some((item: any) => !RESOURCE_ID.test(item.resourceId) || !STAFF_REF.test(item.staffRef))) fail("ADAPTER_ID_BOUNDARY_VIOLATION", "Operational mappings require server-owned Resource and opaque StaffRef namespaces");
  const externalIds: any = new Set([
    ...resourceMappings.map((item: any) => item.externalId),
    ...roleMappings.map((item: any) => item.externalId),
    ...shiftMappings.map((item: any) => item.externalId),
    ...personnelMappings.map((item: any) => item.externalPersonId),
    ...reservationMappings.map((item: any) => item.externalId),
  ]);
  const stableIds: any = new Set([
    ...
    ...resourceMappings.flatMap((item: any) => [item.resourceId, item.binding.templateRef?.templateId, item.binding.utilityObjectId, item.binding.circuitId].filter(Boolean)),
    ...roleMappings.map((item: any) => item.roleId),
    ...shiftMappings.map((item: any) => item.shiftId),
    ...personnelMappings.flatMap((item: any) => [item.staffRef, item.resourceId]),
    ...reservationMappings.map((item: any) => item.reservationRef),
    ...demandTargetObjectIds,
  ]);
  if ([...externalIds].some((id: any) => stableIds.has(id))) fail("ADAPTER_ID_BOUNDARY_VIOLATION", "External operational IDs must remain globally separate from VenueMind stable IDs");
  return {
...
export async function normalizeOperationalResourceAdapterInput(capability: any, input: any, trustedContext: any) {
  ...
  const mappingLookup: any = new Map(trusted.resourceMappings.map((item: any) => [`${item.family}\u0000${item.externalId}`, item]));
  const normalizedResources: any = [];
  ...
  const personLookup: any = new Map(trusted.personnelMappings.map((item: any) => [item.externalPersonId, item]));
  const roles: any = [];
  for (const role of records(input.staffing.roles, "Operational staffing roles")) {
    ...
    const skills = [...new Set(role.skills.map((item: any) => identifier(item, "Operational staffing skill")))].sort(compare);
    const sourceEvidence: any = { externalId: identifier(role.externalId, "Operational source role externalId"), sourceVersion: boundedString(role.sourceVersion, "Operational staffing role sourceVersion"), availableHeadcount: count(role.availableHeadcount, "Operational staffing availableHeadcount"), skills };
  ...
  for (const shift of records(input.staffing.shifts, "Operational staffing shifts")) {
    ...
    const sourceEvidence: any = { externalId: identifier(shift.externalId, "Operational source shift externalId"), sourceVersion: boundedString(shift.sourceVersion, "Operational staffing shift sourceVersion"), startAt: shift.startAt, endAt: shift.endAt };
    shifts.push({ shiftId, startAt: shift.startAt, endAt: shift.endAt, sourceChecksum: await sha256Checksum(sourceEvidence) });
  }
  const assignments: any = [];
  const staffResourceDrafts: any = new Map();
  for (const assignment of records(input.staffing.assignments, "Operational staffing assignments")) {
    ...
    const bookings = records(assignment.bookings, "Operational staffing assignment bookings").map((item: any) => normalizeSourceBooking(item, reservationLookup)).sort((left: any, right: any) => compare(left.startAt, right.startAt) || compare(left.bookingRef, right.bookingRef));
  ...
  for (const draft of [...staffResourceDrafts.values()].sort((left: any, right: any) => compare(left.resourceId, right.resourceId))) {
    const sourceVersions = [...draft.sourceVersions].sort(compare);
    const sourceChecksums = [...draft.sourceChecksums].sort(compare);
    const normalizedAssignments = [...draft.assignments.values()].sort((left: any, right: any) => compare(left.roleId, right.roleId) || compare(left.shiftId, right.shiftId));
    const allUnavailable = normalizedAssignments.every((item: any) => item.status === "unavailable");
    normalizedResources.push({ resourceId: draft.resourceId, family: "staffing", status: allUnavailable ? "unavailable" : "available", total: 1, unavailable: allUnavailable ? 1 : 0, bookings: [...draft.bookings.values()].sort((left: any, right: any) => compare(left.startAt, right.startAt) || compare(left.bookingRef, right.bookingRef)), capability: { assignments: normalizedAssignments }, source: { entityType: "staff-assignment", sourceVersion: boundedString(input.sourceVersion, "Operational staffing sourceVersion"), checksum: await sha256Checksum({ staffRef: draft.staffRef, sourceVersions, sourceChecksums }) } });
  }
...
const createOperationalSnapshot = async (inputValue: any, synchronizedAt: any) => {
  ...
  const reconciliation: any = await reconcileOperationalResources(input);
  ...
  const content: any = { schemaVersion: 1, adapterId: operationalResourceAdapterDefinition.id, adapterVersion: operationalResourceAdapterDefinition.version, sourceSystem: input.sourceSystem, sourceVersion: input.sourceVersion, synchronizedAt, syncCursor, ...reconciliation };
...
const SNAPSHOT_KEYS: any = ["id", "schemaVersion", "adapterId", "adapterVersion", "sourceSystem", "sourceVersion", "synchronizedAt", "syncCursor", "status", "projectId", "planVersion", "planFingerprint", "eventWindow", "currentReservationRef", "resources", "staffing", "demands", "conflicts", "substitutionOptions", "summary", "privacy", "checksum"];

export async function assertOperationalResourceSnapshot(snapshot: any, context: any = {}) {
...
export async function createOperationalSubstitutionStagingBatch({ snapshot, conflictId, optionId, acceptedPlan, proposalRevision, resolveLatestSnapshot }: any) {
  ...
  const conflict = snapshot.conflicts.find((item: any) => item.id === conflictId);
  const option = snapshot.substitutionOptions.find((item: any) => item.id === optionId && item.conflictId === conflictId);
  ...
  const demand = snapshot.demands.find((item: any) => item.demandId === option.demandId);
  const replacement = snapshot.resources.find((item: any) => item.resourceId === option.replacementResourceId);
  const object = acceptedPlan.objects?.find((item: any) => item.id === option.targetObjectIds[0]);
