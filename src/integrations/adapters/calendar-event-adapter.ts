...
import { normalizeEventSchedule } from "../../domain/event-schedule.ts";
...
const normalizeSchedule = (event: any, { nullable = false }: any = {}) => {
  try {
...
const requirement = ({ id, category, label, constraintIds, evidenceRef }: any) => ({
  id,
  category,
  label,
  priority: "high",
...
const importEvent = async (input: any, definition: any, synchronizedAt: any) => {
  ...
  const external: any = { adapterId: definition.id, sourceSystem: input.sourceSystem, entityType: "calendar-event", externalId: event.externalId, sourceVersion: event.sourceVersion, checksum };
  ...
  const source: any = { ...external, synchronizedAt };
  ...
  const eventSchedule: any = { startAt: event.startAt, endAt: event.endAt, timezone: event.timezone };
