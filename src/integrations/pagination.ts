...
const assertBound = (value: any, label: any, maximum: any) => {
...
const assertCursor = (value: any, label: any) => {
...
const assertNotAborted = (signal: any) => {
  if (signal?.aborted) fail("ADAPTER_PAGINATION_ABORTED", "Adapter pagination was aborted");
};

const normalizePage = (value: any, pageIndex: any) => {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    fail("ADAPTER_PAGINATION_INVALID", "Adapter page must be an object", { pageIndex });
  }
  const unknown = Object.keys(value).filter((key: any) => !["items", "nextCursor", "sourceVersion"].includes(key));
  if (unknown.length) {
...
export async function collectAdapterPages({
  ...
  const seenCursors: any = new Set(cursor === null ? [] : [cursor]);
  const items: any = [];
