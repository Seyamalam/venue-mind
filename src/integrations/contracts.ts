...
const CAPABILITY_SET: any = new Set(ADAPTER_CAPABILITIES);
const IMPORT_RESULT_MODE_SET: any = new Set(ADAPTER_IMPORT_RESULT_MODES);
const CHANGE_OPERATION_SET: any = new Set(ADAPTER_CHANGE_OPERATIONS);
const VENUE_ENTITY_TYPE_SET: any = new Set(VENUE_ENTITY_TYPES);
...
export const canonicalStringify = (value: any): string => {
  ...
  if (value && typeof value === "object") return `{${Object.keys(value).sort().map((key: any) => `${JSON.stringify(key)}:${canonicalStringify(value[key])}`).join(",")}}`;
  return JSON.stringify(value);
};

export async function sha256Checksum(value: any) {
...
export function defineAdapter(input: any) {
  ...
  if (!Array.isArray(input.capabilities) || input.capabilities.length === 0 || input.capabilities.some((item: any) => !CAPABILITY_SET.has(item))) fail("ADAPTER_CONTRACT_INVALID", "Adapter capabilities are invalid");
  const capabilities: any = [...new Set(input.capabilities)].sort();
  ...
  const scopes = Object.fromEntries(capabilities.map((capability: any) => {
    const values = input.scopes[capability];
    if (!Array.isArray(values) || values.some((scope: any) => typeof scope !== "string" || !scope)) fail("ADAPTER_CONTRACT_INVALID", `Scopes for ${capability} must be strings`);
...
export function normalizeRetryPolicy(input: any = {}) {
  ...
  if (policy.retryableCodes.some((code: any) => typeof code !== "string" || !code)) fail("ADAPTER_CONTRACT_INVALID", "retryableCodes must contain non-empty strings");
  return Object.freeze(policy);
}

export function normalizeRateLimit(input: any = {}) {
  assertPlainObject(input, "Rate limit");
  assertExactKeys(input, ["requests", "windowMs"], "Rate limit");
  const limit: any = { requests: input.requests ?? 60, windowMs: input.windowMs ?? 60_000 };
...
export function assertAdapterScope(definition: any, capability: any, grantedScopes: any = []) {
  ...
  const granted: any = new Set(grantedScopes);
  const missingScopes = definition.scopes[capability].filter((scope: any) => !granted.has(scope));
...
export function normalizeExternalReference(input: any, definition: any) {
...
export function normalizeAdapterChange(input: any, definition: any) {
  ...
  if (input.evidence !== undefined) {
    ...
    if (input.evidence.references.some((item: any) => !isNonContactLabel(item))) fail("ADAPTER_PERSONAL_DATA_REJECTED", "Adapter change evidence labels must not contain contact data");
...
export function normalizeSyncCursor(input: any, definition: any) {
...
export async function createSyncCursor(definition: any, { opaque, sourceVersion }: any) {
  assertString(opaque, "Synchronization cursor opaque value");
  assertString(sourceVersion, "Synchronization cursor sourceVersion");
  const payload: any = { adapterId: definition.id, adapterVersion: definition.version, opaque, sourceVersion };
  return Object.freeze({ ...payload, checksum: await sha256Checksum(payload) });
}

export function adapterInvocationId(definition: any, capability: any, input: any) {
