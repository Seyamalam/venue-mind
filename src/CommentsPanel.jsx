...
export function CommentsPanel({ open, state, selectedCommentId, onAdd, onEdit, onStatus, onClose }) {
  ...
  return <Sheet open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }} modal={false}>
    ...
    <header><div><SheetTitle asChild><b>COMMENTS</b></SheetTitle><span>{state.comments.filter((comment) => comment.status === "open").length} OPEN</span></div><button type="button" aria-label="Close comments" onClick={onClose}><X size={17} /></button></header>
