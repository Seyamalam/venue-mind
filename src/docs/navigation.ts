...
export function buildDocsNavigation(pages: readonly PublishedDocsPage[]): DocsNavigationGroup[] {
  return pages
    ...
    .reduce<DocsNavigationGroup[]>((groups, page) => {
...
export function getDocsNeighbors(
  pages: readonly PublishedDocsPage[],
  slug: string,
): { previous: PublishedDocsPage | null; next: PublishedDocsPage | null } {
  const current = pages.find((page) => page.slug === slug);
  const collection = current?.navigation?.collection;
  const sequence =
    collection && current.navigation.hidden
      ? pages.filter(
          (page) =>
            page.navigation !== undefined && page.navigation.collection === collection && page.navigation.hidden,
        )
      : pages.filter((page) => page.navigation === undefined || !page.navigation.hidden);
  const index = sequence.findIndex((page) => page.slug === slug);
  return {
    previous: index > 0 ? (sequence[index - 1] ?? null) : null,
    next: index >= 0 && index < sequence.length - 1 ? (sequence[index + 1] ?? null) : null,
  };
}
