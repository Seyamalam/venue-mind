import {
  ...
  venueToolContracts,
} from "../../contracts/venue-contracts.js";
import {
...
import { bullets, code, links, prose, table } from "../blocks.js";
import {
...
const output = (fields, stableIds, semantics) => ({ fields, stableIds, semantics });
...
const slugToken = (value) => value.replaceAll(".", "-").replaceAll("_", "-");
const schemaType = (schema = {}) => {
  if (schema.const !== undefined) return `const ${JSON.stringify(schema.const)}`;
  if (schema.enum) return schema.enum.map(String).join(" | ");
  if (schema.$ref) return new URL(schema.$ref).pathname.split("/").at(-1).replace(".schema.json", "");
...
const rowsForSchema = (schema, commandType = "") => Object.entries(schema.properties ?? {}).map(([name, property]) => ({
  name,
...
const stringExample = (name, schema) => {
...
const exampleValue = (schema = {}, name = "value", depth = 0) => {
  if (depth > 8) return {};
...
const commandSchemas = venueCommandSchema.oneOf.flatMap((schema) => {
  const types = schema.properties.type.const ? [schema.properties.type.const] : schema.properties.type.enum;
  return types.map((type) => ({
    type,
    schema: { ...schema, properties: { ...schema.properties, type: { const: type } } },
  }));
});

const toolForCommand = new Map();
...
function outputForCommand(type) {
...
const commandErrorCodes = (type, schema) => {
...
export const toolReferencePages = venueToolContracts.map((tool) => {
  ...
  const errorRows = tool.errors.map((errorCode) => errorCatalog[errorCode]).filter(Boolean);
  let commandType = "";
  try {
...
export const commandReferencePages = commandSchemas.map(({ type, schema }) => {
  ...
  const errors = commandErrorCodes(type, schema).map((errorCode) => errorCatalog[errorCode]);
...
const toolsByScope = groupBy(toolReferencePages, (page) => page.reference.name && venueToolContracts.find((tool) => tool.name === page.reference.name).authorization.requiredScope);
...
const persistencePage = {
  ...
  sections: [
    ...
    { id: "sharing", title: "Sharing and notifications", blocks: [bullets("Read-only Share Links expose accepted Plan state; Reviewer Share Links expose exactly one Proposal.", "Bearer tokens are returned once, persisted only as hashes, time-limited, and revocable.", "Notifications use fixed body codes and allowlisted stable references; per-User preferences select channel and event type."), links({ label: "Sharing guide", href: "/guides/sharing-and-notifications.md" })] },
...
const authorizationPage = {
  ...
  sections: [
    { id: "human-roles", title: "Human Roles", blocks: [table([{ key: "role", label: "Role" }, { key: "permissions", label: "Permissions" }], HUMAN_ROLES.map((role) => ({ role, permissions: HUMAN_ROLE_PERMISSIONS[role].join(", ") })))] },
    { id: "agent-scopes", title: "Agent Grant scopes", blocks: [table([{ key: "scope", label: "Scope" }, { key: "permissions", label: "Permissions" }], AGENT_SCOPES.map((scope) => ({ scope, permissions: AGENT_SCOPE_PERMISSIONS[scope].join(", ") })))] },
