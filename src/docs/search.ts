import { blockText, type PublishedDocsPage } from "./blocks.ts";
...
export function buildDocsSearchIndex(pages: readonly PublishedDocsPage[]): DocsSearchEntry[] {
...
function searchScore(entry: DocsSearchEntry, tokens: readonly string[]): number {
  let score = 0;
...
export function searchDocs(index: readonly DocsSearchEntry[], query: string, limit = 10): DocsSearchEntry[] {
...
export function nextSearchSelection(current: number, key: string, resultCount: number): number {
