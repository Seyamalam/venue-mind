...
import { docsPages } from "./content.js";
import { VERSION_REFERENCE } from "./reference-data.js";
...
const publicHref = (href, origin) => href.startsWith("/") && origin ? new URL(href, origin).href : href;
...
function renderBlock(block, origin) {
  ...
  if (block.type === "links") return block.items.map((item) => `- [${item.label}](${publicHref(item.href, origin)})`).join("\n");
  if (block.type === "table") {
    const header = `| ${block.columns.map((column) => column.label).join(" | ")} |`;
    const divider = `| ${block.columns.map(() => "---").join(" | ")} |`;
    const rows = block.rows.map((row) => `| ${block.columns.map((column) => String(row[column.key] ?? "—").replaceAll("|", "\\|").replaceAll("\n", " ")).join(" | ")} |`).join("\n");
...
export function buildAgentDocuments({ origin: originValue = "", skillPackages = [] } = {}) {
  const origin = normalizeOrigin(originValue);
  const publicPages = docsPages.filter((page) => page.public !== false && !page.deprecated);
  const documentationLinks = publicPages.filter((page) => !page.navigation?.hidden)
    .map((page) => `- [${page.slug === "overview" ? "Overview" : page.title}](${publicHref(page.canonicalPath, origin)})`)
    .join("\n");
  const link = (label, href) => `- [${label}](${publicHref(href, origin)})`;
...
${venueToolContracts.map((tool) => `- ${tool.name}: ${tool.description}`).join("\n")}
...
].map(([label, href]) => link(label, href)).join("\n")}
`;
