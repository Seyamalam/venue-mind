...
import { analyzeProductionPlan, createProductionMapSvg, createProductionScheduleCsv } from "../domain/production-planning.js";
import { createReplenishmentScheduleCsv, createServiceStationScheduleCsv } from "../domain/catering-planning.js";
import { analyzeEmergencyPlan } from "../domain/emergency-planning.js";
import { analyzeStaffingOperations, createStaffingPostMapSvg, createStaffingScheduleCsv } from "../domain/staffing-operations.js";
...
const LAYERS = Object.freeze(["architecture", "furniture", "access", "production", "catering", "safety", "annotations"]);
...
const number = (value, digits = 3) => Number(Number(value).toFixed(digits));
const xml = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&apos;");
const slug = (value) => String(value || "venue-plan").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 64) || "venue-plan";
...
const footprintPoints = (footprint, circleSegments = 32) => {
...
const geometryDescriptor = (footprint) => {
...
export function createInventoryScheduleCsv(plan, validation) {
  ...
  const rows = [...placements.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([key, placement]) => {
    ...
    const template = venueTemplateCatalog.inventoryTemplates.find((item) => item.id === templateId && item.version === version);
...
const svgPath = (points, transform) => `${points.map((point, index) => `${index ? "L" : "M"} ${number(transform.x(point.x), 2)} ${number(transform.y(point.y), 2)}`).join(" ")} Z`;

export function createLayeredSvg(plan, validation, { comments = [], exportedAt = new Date().toISOString() } = {}) {
  ...
  const objectMarkup = LAYERS.map((layer) => {
    const objects = plan.objects.filter((object) => object.layer === layer).sort((left, right) => left.id.localeCompare(right.id));
  ...
  const pins = comments.filter((comment) => comment.anchor?.kind === "coordinate" && comment.anchor.planVersion === plan.version).map((comment, index) => {
  ...
  const legend = LAYERS.map((layer, index) => `<g transform="translate(970 ${180 + index * 34})"><rect width="18" height="18" rx="3" fill="${LAYER_STYLE[layer].fill}" stroke="${LAYER_STYLE[layer].stroke}"/><text x="29" y="13" class="legend-label">${xml(layer.toUpperCase())}</text></g>`).join("");
...
const pdfColor = (hex) => [1, 3, 5].map((index) => Number.parseInt(hex.slice(index, index + 2), 16) / 255).map((value) => number(value, 3)).join(" ");

const pdfText = (x, y, text, size = 10, font = "F1", color = "0.18 0.18 0.17") => `BT /${font} ${size} Tf ${color} rg ${number(x, 2)} ${number(y, 2)} Td (${pdfEscape(text)}) Tj ET`;
const pdfLine = (x1, y1, x2, y2, color = "0.4 0.4 0.38", width = 1) => `${color} RG ${width} w ${number(x1, 2)} ${number(y1, 2)} m ${number(x2, 2)} ${number(y2, 2)} l S`;
const pdfPolygon = (points, fill, stroke, width = 0.8) => `${pdfColor(fill)} rg ${pdfColor(stroke)} RG ${width} w ${points.map((point, index) => `${number(point.x, 2)} ${number(point.y, 2)} ${index ? "l" : "m"}`).join(" ")} h B`;
...
export function createPlanPdf(plan, validation, { exportedAt = new Date().toISOString() } = {}) {
  const bounds = planBounds(plan);
  ...
  page1.push(pdfPolygon(plan.spatial.roomBoundary.outer.map((point) => ({ x: transform.x(point.x), y: transform.y(point.y) })), "#ffffff", "#393a37", 1.8));
  for (const layer of LAYERS) {
    for (const object of plan.objects.filter((item) => item.layer === layer).sort((left, right) => left.id.localeCompare(right.id))) {
      const points = footprintPoints(object.footprint, 20).map((point) => ({ x: transform.x(point.x), y: transform.y(point.y) }));
...
export function createEmergencyPlanPdf(plan, evidence = analyzeEmergencyPlan(plan), { exportedAt = new Date().toISOString() } = {}) {
  const emergencyObjects = plan.objects.filter((object) => object.kind === "fire_exit" || object.emergency);
  const allPoints = [...plan.spatial.roomBoundary.outer, ...emergencyObjects.flatMap((object) => footprintPoints(object.footprint, 20))];
  ...
  const scale = Math.min(available.width / (bounds.maxX - bounds.minX), available.height / (bounds.maxY - bounds.minY));
  const transform = { x: (value) => origin.x + (value - bounds.minX) * scale, y: (value) => origin.y + (value - bounds.minY) * scale };
  ...
  page.push(pdfPolygon(plan.spatial.roomBoundary.outer.map((point) => ({ x: transform.x(point.x), y: transform.y(point.y) })), "#ffffff", "#000000", 1.8));
  ...
  for (const object of plan.objects.filter((item) => routeKinds.has(item.kind))) {
    page.push(pdfPolygon(footprintPoints(object.footprint, 12).map((point) => ({ x: transform.x(point.x), y: transform.y(point.y) })), object.kind === "emergency_access_lane" ? "#d9d9d9" : "#f3f3f3", "#4d4d4d", object.kind === "emergency_access_lane" ? 1.5 : .5));
  }
  emergencyObjects.sort((left, right) => left.id.localeCompare(right.id)).forEach((object, index) => {
    const points = footprintPoints(object.footprint, 18).map((point) => ({ x: transform.x(point.x), y: transform.y(point.y) }));
  ...
  readiness.forEach((line, index) => page.push(pdfText(650, 470 - index * 20, line, 8, index === 0 ? "F2" : "F1", "0.1 0.1 0.1")));
...
export function createPortableAuditPackage(state, validation, replay, { exportedAt = new Date().toISOString() } = {}) {
  const payload = {
...
export function createPlanExport(format, { state, plan, validation, replay, exportedAt = new Date().toISOString(), jsonPayload = null, textPayload = null }) {
