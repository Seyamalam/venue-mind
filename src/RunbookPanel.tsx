...
const EMPTY_TASKS: DomainList = Object.freeze([]) as unknown as DomainList;
...
const labelFor = (value: DomainRecord, fallback = "—") => value?.label ?? value?.name ?? value?.kind?.replace("live-event", "live").toUpperCase() ?? value?.id ?? fallback;
const normalizedTaskStatus = (status: string): keyof typeof TASK_STATUS => status === "in-progress" ? "active" : status in TASK_STATUS ? status as keyof typeof TASK_STATUS : "pending";
const taskRoleIds = (task: DomainRecord): string[] => [...new Set([...(task.roleIds ?? []), task.roleId, task.role, task.workstream, task.owner?.roleId].filter(Boolean))] as string[];
const taskEvidence = (task: DomainRecord): DomainList => task.evidence ?? task.evidenceRefs ?? [];
const taskDependencies = (task: DomainRecord): DomainList => task.dependencies ?? task.dependencyIds ?? task.dependencyTaskIds ?? [];
const completedDependencyCount = (task: DomainRecord): number => task.completedDependencyCount ?? task.dependenciesCompleted ?? task.readiness?.completedDependencyIds?.length ?? 0;
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant={definition.variant as BadgeVariant}><Icon aria-hidden="true" />{definition.label}</Badge>;
}

function RunbookSelect({ label, value, options, onValueChange }: { label: string; value: string; options: readonly DomainRecord[]; onValueChange: ValueCallback }) {
...
function RunbookEmptyState({ sourcePlanVersion, sourcePlanStatus, phaseCount, taskCount, onCreate }: { sourcePlanVersion: string | number; sourcePlanStatus: string; phaseCount: number; taskCount: number; onCreate?: (input: DomainRecord) => unknown }) {
...
function RunbookTaskRow({ task, onTaskTransition, onOpenEvidence }: { task: DomainRecord; onTaskTransition?: (input: DomainRecord) => unknown; onOpenEvidence?: ((task: DomainRecord) => void) | null }) {
...
function RunbookEvidenceDialog({ task, open, onOpenChange, onAddEvidence }: { task?: DomainRecord | null; open: boolean; onOpenChange: (open: boolean) => void; onAddEvidence?: (input: DomainRecord) => unknown }) {
...
function RunbookHandoffView({ runbookId, roleOptions, ownerOptions, handoffs, onCreateHandoff, onCopyHandoff, onExportHandoff }: { runbookId: string; roleOptions: readonly DomainRecord[]; ownerOptions: DomainList; handoffs: DomainList; onCreateHandoff?: (input: DomainRecord) => unknown; onCopyHandoff?: (handoff: DomainRecord) => unknown; onExportHandoff?: (handoff: DomainRecord) => unknown }) {
      ...
      <div role="list">{handoffs.length ? handoffs.slice().reverse().map((handoff) => {
        ...
        return <article className="runbook-handoff" key={handoff.id} role="listitem" aria-labelledby={`runbook-handoff-${handoff.id}`}>
          ...
          <section aria-labelledby={`runbook-carry-${handoff.id}`}>
            ...
            {carry.length ? carry.map((item: DomainRecord) => <span key={item.taskId ?? item.id}><code>{item.taskId ?? item.id}</code><em>{String(item.status ?? "open").toUpperCase()}</em><small>{item.ownerLabel ?? item.ownerId ?? "—"}</small></span>) : <span>0</span>}
          </section>
          <section aria-labelledby={`runbook-blockers-${handoff.id}`}>
            <b id={`runbook-blockers-${handoff.id}`}>BLOCKERS</b>
            {blockers.length ? blockers.map((item: DomainRecord) => <span key={item.taskId ?? item.id}><code>{item.taskId ?? item.id}</code><em>{item.code ?? item.status ?? "BLOCKED"}</em><small>{item.locationLabel ?? item.objectId ?? "—"}</small></span>) : <span>0</span>}
          </section>
          <footer>
...
type RunbookPanelProps = {
  open: boolean; runbook?: DomainRecord | null; sourcePlanVersion?: string | number; sourcePlanStatus?: string;
  phaseOptions?: readonly DomainRecord[]; roleOptions?: readonly DomainRecord[]; ownerOptions?: DomainList;
  syncState?: DomainRecord;
  phaseFilter?: string; roleFilter?: string; handoffs?: DomainList; plannedTaskCount?: number; onClose?: VoidCallback;
  onCreate?: (input: DomainRecord) => unknown; onPhaseFilterChange?: ValueCallback; onRoleFilterChange?: ValueCallback;
  onTaskTransition?: (input: DomainRecord) => unknown; onAddEvidence?: (input: DomainRecord) => unknown;
  onCreateHandoff?: (input: DomainRecord) => unknown; onCopyHandoff?: (handoff: DomainRecord) => unknown;
  onExportHandoff?: (handoff: DomainRecord) => unknown; onSync?: VoidCallback; onResolveSyncConflict?: VoidCallback;
};
...
export function RunbookPanel({
  ...
  const [evidenceTask, setEvidenceTask] = useState<DomainRecord | null>(null);
  const activePhaseFilter = phaseFilter ?? internalPhaseFilter;
  const activeRoleFilter = roleFilter ?? internalRoleFilter;
  const tasks: DomainList = runbook?.tasks ?? EMPTY_TASKS;
  const syncDefinition = SYNC_STATUS[String(syncState.state) as keyof typeof SYNC_STATUS] ?? SYNC_STATUS.online;
      ...
      <SheetContent id="event-day-runbook-panel" className="runbook-panel !h-auto !gap-0 !p-0 sm:!max-w-none" side="right" showOverlay={false} showCloseButton={false} aria-label="Event Day Runbook">
        ...
        {!runbook ? <RunbookEmptyState sourcePlanVersion={sourcePlanVersion} sourcePlanStatus={sourcePlanStatus} phaseCount={phaseOptions.length} taskCount={plannedTaskCount} onCreate={onCreate} /> : <>
          <div className={cn("runbook-sync-strip", `is-${syncState.state ?? "online"}`)}>
            <Badge variant={syncDefinition.variant as BadgeVariant} role={syncState.state === "conflict" ? "alert" : "status"} aria-live={syncState.state === "conflict" ? "assertive" : "polite"}><SyncIcon aria-hidden="true" />{syncDefinition.label}</Badge>
