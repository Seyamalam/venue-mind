const EVENT_TYPES: any = ["project.created", "project.updated", "comment.updated", "ledger.appended", "proposal.updated", "approval.committed", "sync.reset", "sync.cursor", "presence.snapshot"];
...
export function createCollaborationClient({
  ...
  let currentPresence: any = { planVersion: "0.0", focusedObjectId: null, viewport: null };
  ...
  return Object.freeze({
    start() {
      ...
      for (const type of EVENT_TYPES) source.addEventListener(type, (event: any) => {
        let data;
