...
export async function registerVenueTools(modelContext, planner, signal, { grantedScopes = DEFAULT_WEBMCP_SCOPES, organizationId = "org-local", projectId = "project-summit-forward", authorization, projectOperations, occupancyOperations, incidentOperations, onLifecycle = () => {} } = {}) {
  ...
  try {
    for (const definition of venueToolDefinitions) {
      ...
      await modelContext.registerTool({
        ...
        execute: async (input = {}, context = {}) => executeVenueWebMcpTool({ contract: definition, planner, projectOperations, occupancyOperations, incidentOperations, input, signal: context.signal ?? signal, grantedScopes, organizationId, projectId, authorization }),
      }, { signal });
      registered += 1;
