...
export async function executeVenueWebMcpTool({ contract, planner, projectOperations, occupancyOperations, incidentOperations, toolService, input = {}, signal, grantedScopes = DEFAULT_WEBMCP_SCOPES, organizationId = "org-local", projectId = "project-summit-forward", authorization: suppliedAuthorization, clock = () => new Date().toISOString(), correlationIdFactory = () => `corr-webmcp-${globalThis.crypto?.randomUUID?.() ?? Date.now()}` }) {
  ...
  try {
    ...
    const service = toolService ?? createVenueToolService({ executeCommand: (command, options) => planner.execute(command, options), projectOperations, occupancyOperations, incidentOperations, recordAuthorizationDenial: (denial) => planner.recordAuthorizationDenial(denial) });
