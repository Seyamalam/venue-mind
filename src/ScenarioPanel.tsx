...
import type { DomainList, DomainRecord, ValueCallback, ValueOption, VoidCallback } from "./ui-types";

const numeric = (value: unknown, fallback: number) => Number.isFinite(Number(value)) ? Number(value) : fallback;

const secondsLabel = (seconds: number) => `${Math.floor(seconds / 60)}:${String(Math.round(seconds % 60)).padStart(2, "0")}`;

type ScenarioSelectProps = { value: string; onValueChange: ValueCallback; disabled?: boolean; label: string; options: ValueOption[] };
...
type ScenarioPanelProps = {
  open: boolean; branches: DomainList; runs: DomainList; onClose: VoidCallback;
  onRun: (input: DomainRecord, branchId: string) => unknown;
  onCompare: (leftId: string, rightId: string) => DomainRecord;
  onExport: ValueCallback; onOverlayChange?: ValueCallback<DomainRecord | null>; onPreviewOption: ValueCallback;
};

export function ScenarioPanel({ open, branches, runs, onClose, onRun, onCompare, onExport, onOverlayChange, onPreviewOption }: ScenarioPanelProps) {
  const [draft, setDraft] = useState<DomainRecord>({ name: "Egress", model: "ingress-egress", mode: "normal", category: "registration", curve: "steady", seed: 73421, minutes: 30, samples: 256, population: 400, arrivals: 28, service: 9, servers: 3, accessShare: 8, branchId: branches.find((branch) => branch.active)?.id ?? branches[0]?.id ?? "" });
  ...
  useEffect(() => {
    onOverlayChange?.(activeFrame && overlayRun ? { runId: overlayRun.id, branchId: overlayRun.branchId, frame: activeFrame, result: overlayRun.result } : null);
  ...
  const comparison = useMemo(() => {
    ...
    try { return left && right ? onCompare(left.id, right.id) : null; } catch { return null; }
  }, [completed, onCompare]);
  const update = (field: string) => (event: ChangeEvent<HTMLInputElement>) => setDraft((current) => ({ ...current, [field]: event.target.value }));
  const updateValue = (field: string) => (value: string) => setDraft((current) => ({ ...current, [field]: value }));
