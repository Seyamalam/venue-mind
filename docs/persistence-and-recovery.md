...
Each authoritative Project record carries a positive `revision` and a strong ETag. Creation sends `If-None-Match: *` plus `X-VenueMind-Create-Only: 1`; updates send the exact current ETag in `If-Match` plus the same revision in `X-VenueMind-Expected-Revision`. The VenueMind headers preserve the conditional-write contract through hosting proxies that reserve standard conditional headers. When both forms arrive, the API requires them to agree. Missing preconditions fail with `428`, invalid or disagreeing preconditions fail with `400`, stale revisions fail with structured `412 PROJECT_REVISION_CONFLICT`, and no conflict is reported as a successful offline save.
