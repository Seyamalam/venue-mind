...
Reconnects resume from `Last-Event-ID`, missing links produce an explicit reset, and all sessions converge through the same optimistic-concurrency boundary. The transport works in the existing Sites Worker without a second state authority. Presence is eventually live at the reconnect interval, while accepted Plan mutation remains serialized by planner commands and conditional Project writes.
