...
const runbook = {
  schemaVersion: 1,
  id: "runbook-project-a",
  versionId: "runbook-v1",
  phases: [],
  tasks: [],
  ledger: [],
};
...
test("Runbook remote scopes create, get, and ordered transition sync requests", async () => {
  ...
  const remote = createRunbookRemote({
    ...
    fetchImpl: async (url, init = {}) => {
      ...
      return response(url.endsWith("transitions:sync") ? { acknowledgements: [], runbook } : { runbook });
    },
  });
  ...
  assert.deepEqual(
    requests.map(({ init }) => init.headers["x-venuemind-organization-id"]),
    ["org-a", "org-a", "org-a"],
  );
  assert.deepEqual(JSON.parse(requests[0].init.body), { runbook });
  assert.deepEqual(
    JSON.parse(requests[2].init.body).commands.map((command) => command.clientSequence),
    [1, 2],
  );
});

test("Runbook remote preserves structured API failures and rejects non-JSON endpoints", async () => {
  const denied = createRunbookRemote({
    organizationId: "org-a",
    fetchImpl: async () =>
      response(
        { error: { code: "AUTHORIZATION_DENIED", message: "Denied", details: { permission: "project.manage" } } },
        { status: 403 },
      ),
  });
  await assert.rejects(
    () => denied.get("project-a", "runbook-v1"),
    (error) =>
      error.code === "AUTHORIZATION_DENIED" && error.status === 403 && error.details.permission === "project.manage",
  );

  const unavailable = createRunbookRemote({
    organizationId: "org-a",
    fetchImpl: async () => response("not found", { status: 404, contentType: "text/html" }),
  });
  await assert.rejects(
    () => unavailable.create("project-a", runbook),
    (error) => error.code === "RUNBOOK_API_UNAVAILABLE" && error.status === 404,
  );
});
