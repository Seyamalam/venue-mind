...
test("accepts mutations only from the configured Vercel origin", async () => {
  ...
  const env = {
    DB: {},
    VENUEMIND_AUTH_MODE: "anonymous-demo",
    VENUEMIND_APP_ORIGINS: "https://venue-mind-jet.vercel.app",
  };
  const allowed = await api.fetch(
    new Request("https://api.example.test/api/session/revoke", {
      method: "POST",
      headers: { origin: "https://venue-mind-jet.vercel.app" },
    }),
    env,
  );
  ...
  const api = createWorker({
    ...
    createProjectRepository: () => ({
      list: async (organizationId) =>
        [...records.values()].filter((record) => record.organizationId === organizationId),
