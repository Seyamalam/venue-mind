...
const projectRepository = {
  async list(organizationId) {
    return [...records.values()].filter((record) => record.organizationId === organizationId);
  },
  async get(organizationId, projectId) {
    return records.get(projectId)?.organizationId === organizationId ? structuredClone(records.get(projectId)) : null;
  },
  async put(organizationId, record, { createOnly = false, expectedRevision = null } = {}) {
    ...
    if ((createOnly && existing) || (!createOnly && (!existing || existing.revision !== expectedRevision)))
...
const identityProvider = {
  authenticate: (request) => {
    ...
    return identity
      ? { provider: "test", subject: identity, email: `${identity}@example.test`, displayName: identity.toUpperCase() }
      : null;
  },
};
...
const env = { DB: {} };
const request = (path, { identity, organizationId, method = "GET", body, headers = {} } = {}) =>
  worker.fetch(
    new Request(`https://example.test${path}`, {
      ...
      headers: {
        ...
        ...(organizationId ? { "x-venuemind-organization-id": organizationId } : {}),
...
test("Cloudflare anonymous demo mode provisions an isolated durable browser identity", async () => {
  ...
  const demoWorker = createWorker({
    secureCookies: false,
    createAccountRepository: () => demoAccounts,
    createProjectRepository: () => projectRepository,
  });
  ...
  const cookieHeader = [...cookies.matchAll(/(venuemind_(?:demo_identity|session)=[^;,]+)/g)]
    .map((match) => match[1])
    .join("; ");
  const resumed = await demoWorker.fetch(
    new Request("https://example.test/api/session", { headers: { cookie: cookieHeader } }),
    demoEnv,
  );
...
test("API membership, invitations, roles, sessions, export, deletion, and Project access remain tenant isolated", async () => {
  ...
  const created = await request("/api/projects/project-alpha", {
    identity: "alice",
    organizationId: alphaId,
    method: "PUT",
    body: projectRecord("project-alpha", "Alpha"),
    headers: { "if-none-match": "*" },
  });
  assert.equal(created.status, 201);
  assert.equal((await created.json()).organizationId, alphaId);
  assert.equal(
    (await request("/api/projects/project-alpha", { identity: "bob", organizationId: bobOrgId })).status,
    404,
  );
  assert.equal((await request("/api/projects", { identity: "bob", organizationId: alphaId })).status, 403);
  assert.equal(
    (
      await request("/api/projects/project-alpha", {
        identity: "alice",
        organizationId: alphaId,
        method: "PUT",
        body: { ...projectRecord("project-alpha", "Forged"), organizationId: bobOrgId },
  ...
  const invitationResponse = await request("/api/invitations", {
    identity: "alice",
    organizationId: alphaId,
    method: "POST",
    body: { email: "bob@example.test", roles: ["viewer"], expiresAt: "2026-08-29T10:00:00.000Z" },
  });
  assert.equal(invitationResponse.status, 201);
  const { token } = await invitationResponse.json();
  assert.equal(
    (await request("/api/invitations/accept", { identity: "bob", method: "POST", body: { token } })).status,
    200,
  );
  assert.equal(
    (await request("/api/projects/project-alpha", { identity: "bob", organizationId: alphaId })).status,
    200,
  );
  assert.equal(
    (
      await request("/api/projects/project-beta", {
        identity: "bob",
        organizationId: alphaId,
        method: "PUT",
        body: projectRecord("project-beta", "Beta"),
        headers: { "if-none-match": "*" },
      })
    ).status,
    403,
  ...
  assert.equal(
    (
      await request(`/api/memberships/${bob.userId}`, {
        identity: "alice",
        organizationId: alphaId,
        method: "PATCH",
        body: { roles: ["planner"] },
  ...
  assert.equal(
    (await request(`/api/memberships/${bob.userId}`, { identity: "alice", organizationId: alphaId, method: "DELETE" }))
      .status,
    200,
  );
  assert.equal(
...
test("a revoked session cookie cannot be replayed", async () => {
  ...
  const revoke = await worker.fetch(
    new Request("https://example.test/api/session/revoke", {
      method: "POST",
  ...
  const replay = await worker.fetch(
    new Request("https://example.test/api/session", { headers: { cookie, accept: "application/json" } }),
    env,
  );
...
test("two API sessions cannot overwrite the same stale Project revision", async () => {
  ...
  const created = await request("/api/projects/project-concurrent", {
    identity: "concurrency-owner",
    organizationId,
    method: "PUT",
    body: projectRecord("project-concurrent", "BASE"),
    headers: { "if-none-match": "*" },
  });
  assert.equal(created.status, 201);
  ...
  const first = await request("/api/projects/project-concurrent", {
    identity: "concurrency-owner",
    organizationId,
    method: "PUT",
    body: { ...base, name: "TAB A" },
    headers: { "if-match": staleEtag },
  });
  assert.equal(first.status, 200);
  assert.equal((await first.clone().json()).revision, 2);
  const stale = await request("/api/projects/project-concurrent", {
    identity: "concurrency-owner",
    organizationId,
    method: "PUT",
    body: { ...base, name: "TAB B" },
    headers: { "if-match": staleEtag },
  });
  ...
  assert.equal(
    (
      await (
        await request("/api/projects/project-concurrent", { identity: "concurrency-owner", organizationId })
      ).json()
    ).name,
    "TAB A",
  );
  assert.equal(
    (
      await request("/api/projects/project-concurrent", {
        identity: "concurrency-owner",
        organizationId,
        method: "PUT",
        body: { ...base, name: "UNCONDITIONAL" },
      })
    ).status,
    428,
  );
...
test("browser recovery state is partitioned by Organization", async () => {
  ...
  await alpha.save(projectRecord("project-shared", "Alpha"));
...
test("stdio MCP repository, session, resources, exports, and logs stay Organization scoped", async () => {
  const alphaRecord = {
    ...projectRecord("project-alpha", "Alpha"),
    organizationId: "org-alpha",
    snapshot: createVenuePlanner(summitForwardPlan).getSnapshot(),
    archivedAt: null,
    deletedAt: null,
    recoveryUntil: null,
    pinned: false,
  ...
  const alphaRepository = createMemoryProjectRepository(
    [alphaRecord, { ...alphaRecord, id: "project-bravo", organizationId: "org-bravo" }],
    { organizationId: "org-alpha" },
  );
  assert.deepEqual(
    (await alphaRepository.list()).map((record) => record.id),
    ["project-alpha"],
  );
  await assert.rejects(
    () => alphaRepository.save({ ...alphaRecord, organizationId: "org-bravo" }),
    /ORGANIZATION_ACCESS_DENIED/,
  );
  const session = createProjectSession({ repository: alphaRepository, organizationId: "org-alpha", clock: () => NOW });
  assert.deepEqual(
    (await session.listProjects()).map((record) => record.id),
    ["project-alpha", "project-summit-forward"],
  );
  await assert.rejects(
    () => session.readProject("project-bravo"),
