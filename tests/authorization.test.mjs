...
const clock = (value = LATER) => () => value;

const grant = (scopes, overrides = {}) => createAgentGrant({
...
test("human authorization matrix grants exactly each documented role permission", () => {
  for (const role of HUMAN_ROLES) {
    ...
    for (const permission of VENUE_PERMISSIONS) {
      const result = evaluateVenuePermission({ permission, principal, projectId: "project-summit-forward", clock: clock() });
...
test("every planner command and published tool resolves through the authorization matrix", () => {
  const commandTypes = venueCommandSchema.oneOf.flatMap((schema) => schema.properties.type.enum ?? [schema.properties.type.const]);
  for (const commandType of commandTypes) assert.ok(VENUE_PERMISSIONS.includes(permissionForCommand(commandType)), commandType);
  ...
  for (const contract of venueToolContracts) {
    ...
    const result = evaluateVenuePermission({ permission, principal, grant: activeGrant, projectId: "project-summit-forward", clock: clock() });
    assert.equal(result.status, "allow", `${contract.name} ${permission}`);
  }
  assert.equal(venueToolContracts.some((contract) => /approve|delete_project/.test(contract.name)), false);
});
...
test("Agent Grants are short-lived and bound to one Organization and Project", () => {
  ...
  assert.equal(evaluateVenuePermission({ permission: "plan.read", principal, grant: activeGrant, organizationId: "org-alpha", projectId: "project-summit-forward", clock: clock() }).status, "allow");
  assert.equal(evaluateVenuePermission({ permission: "plan.read", principal, grant: activeGrant, organizationId: "org-bravo", projectId: "project-summit-forward", clock: clock() }).reason, "agent-grant-organization-mismatch");
...
test("planner denies insufficient roles without changing Plan truth and records sanitized evidence", () => {
  ...
  assert.throws(
    () => planner.execute({ type: "preview_revision", goal: "secret-layout-intent", secret: "never-ledger-this", actor: "human", idempotencyKey: "viewer-preview" }, { authorization, projectId: "project-summit-forward" }),
...
test("Approval policy rejects planners and permits an authenticated approver", () => {
  const planner = createVenuePlanner(summitForwardPlan, { projectId: "project-summit-forward" });
  const plannerAuthorization = { principal: createHumanPrincipal({ id: "planner-1", roles: ["planner"] }), clock: clock() };
  const approverAuthorization = { principal: createHumanPrincipal({ id: "approver-1", roles: ["approver"] }), clock: clock() };
  const proposal = planner.getSnapshot().proposal;
  assert.throws(
    () => planner.execute({ type: "approve_proposal", proposalId: proposal.id, baseVersion: proposal.baseVersion, actor: "human", actorId: "planner-1", idempotencyKey: "planner-approval" }, { authorization: plannerAuthorization, projectId: "project-summit-forward" }),
    (error) => error.code === "AUTHORIZATION_DENIED" && error.details.permission === "approval.approve",
  );
  const approved = planner.execute({ type: "approve_proposal", proposalId: proposal.id, baseVersion: proposal.baseVersion, actor: "human", actorId: "approver-1", idempotencyKey: "approver-approval" }, { authorization: approverAuthorization, projectId: "project-summit-forward" });
...
test("agent proposal scope cannot be converted into Approval authority", () => {
  const planner = createVenuePlanner(summitForwardPlan, { projectId: "project-summit-forward" });
  const authorization = { principal: createAgentPrincipal({ id: "agent-1" }), grant: grant(["venue:read", "venue:propose"]), clock: clock() };
  const preview = planner.execute({ type: "preview_revision", goal: "Improve entrance flow", actor: "agent", idempotencyKey: "agent-preview" }, { authorization, projectId: "project-summit-forward" });
  assert.equal(preview.requiresHumanApproval, true);
  assert.throws(
    () => planner.execute({ type: "approve_proposal", proposalId: preview.proposalId, baseVersion: preview.baseVersion, actor: "agent", idempotencyKey: "agent-approval" }, { authorization, projectId: "project-summit-forward" }),
    (error) => error.code === "AUTHORIZATION_DENIED" && error.details.permission === "approval.approve",
  );
});

test("tool service enforces Agent Grants and filters Project discovery", async () => {
  const authorization = { principal: createAgentPrincipal({ id: "agent-1" }), grant: grant(["venue:read"]), projectId: "project-summit-forward", clock: clock() };
  let executed = false;
  const denials = [];
  const service = createVenueToolService({
    executeCommand: async () => { executed = true; },
    projectOperations: {
      listProjects: async () => ({ source: "test", projects: [{ id: "project-summit-forward" }, { id: "project-secret" }] }),
  ...
  assert.deepEqual(projects.projects.map((project) => project.id), ["project-summit-forward"]);
  await assert.rejects(() => service.execute("venue.preview_revision", { goal: "Try", idempotencyKey: "denied-tool" }), (error) => error.code === "AUTHORIZATION_DENIED");
  assert.equal(executed, false);
  assert.equal(denials.length, 1);
  await assert.rejects(() => service.execute("venue.open_project", { projectId: "project-secret" }), (error) => error.code === "AUTHORIZATION_DENIED" && error.details.reason === "agent-grant-project-mismatch");
});
