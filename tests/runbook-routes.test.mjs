...
const browserRunbook = (overrides = {}) => ({
  ...
  source: {
    ...
    briefFingerprint: "brief-alpha-fingerprint",
    validationId: "validation-alpha",
    validationInputFingerprint: "validation-alpha-fingerprint",
    approvalLedgerEntryId: "approval-alpha",
    sourceLedgerHeadHash: "activity-ledger-alpha-head",
  },
  baseline: {
    acceptedPlan: summitForwardPlan,
    acceptedBrief: summitForwardPlan.brief,
    staffingEvidence: {},
    fingerprint: "baseline-alpha",
  },
...
async function harness() {
  ...
  await db.batch([
    db
      ...
      .bind("user-seyam", "test", "seyam", "seyam@example.test", "active", NOW, NOW),
    db
      .prepare("INSERT INTO organizations (id,name,slug,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?)")
      .bind("org-alpha", "ALPHA", "alpha", "user-seyam", NOW, NOW),
    db
      .prepare("INSERT INTO organizations (id,name,slug,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?)")
      .bind("org-bravo", "BRAVO", "bravo", "user-seyam", NOW, NOW),
    db
      .prepare(
  ...
  const users = {
    ...
    outsider: {
      id: "user-outsider",
      organization: { id: "org-bravo", name: "BRAVO", slug: "bravo", roles: ["planner"] },
    },
  };
  const worker = createWorker({
    ...
    identityProvider: {
      authenticate: (request) => {
        ...
        return identity && users[identity]
          ? {
              provider: "test",
              subject: identity,
              email: `${identity}@example.test`,
              displayName: identity.toUpperCase(),
            }
          : null;
      },
    },
    createAccountRepository: () => ({
      ...
      provision: async (identity) => {
        ...
        return {
          user: { id: fixture.id, email: identity.email, displayName: identity.displayName, status: "active" },
  ...
  const request = (path, { identity, organizationId, method = "GET", body } = {}) =>
    worker.fetch(
...
test("authenticated Runbook routes seed initial state, retry creation, and stay Organization scoped", async (t) => {
  ...
  assert.deepEqual(
    {
      status: created.runbook.tasks[0].status,
      revision: created.runbook.tasks[0].revision,
      evidence: created.runbook.tasks[0].evidence,
    },
    { status: "pending", revision: 0, evidence: [] },
  );

  const retried = await request(collection, {
    identity: "planner",
    organizationId: "org-alpha",
    method: "POST",
    body: browserRunbook(),
  ...
  const conflicting = await request(collection, {
    identity: "planner",
    organizationId: "org-alpha",
    method: "POST",
    body: browserRunbook({ definitionFingerprint: "different" }),
  });
...
test("transition sync returns per-command outcomes and authoritative browser Runbook state", async (t) => {
  ...
  t.after(() => db.close());
  const created = await (
    await request("/api/projects/project-alpha/runbooks", {
      identity: "planner",
      organizationId: "org-alpha",
      method: "POST",
      body: browserRunbook(),
  ...
  const response = await request(syncPath, {
    identity: "planner",
    organizationId: "org-alpha",
    method: "POST",
    body: {
      commands: [
        ...
        command({
          operationId: "operation-no-evidence",
          idempotencyKey: "idempotency-no-evidence",
          clientSequence: 3,
  ...
  assert.deepEqual(
    result.acknowledgements.map((item) => item.status),
    ["applied", "already-applied", "conflict", "rejected"],
  );
  ...
  assert.deepEqual(
    { status: result.runbook.tasks[0].status, revision: result.runbook.tasks[0].revision },
    { status: "in-progress", revision: 1 },
  );
  assert.deepEqual(
    {
      actorType: result.runbook.ledger[0].actorType,
      actorId: result.runbook.ledger[0].actorId,
      source: result.runbook.ledger[0].source,
      sessionId: result.runbook.ledger[0].sessionId,
    },
    { actorType: "human", actorId: "user-seyam", source: "studio", sessionId: "session-user-seyam" },
  );
  assert.deepEqual(
    {
      clientId: result.runbook.transitions[0].clientId,
      clientSequence: result.runbook.transitions[0].clientSequence,
      clientOccurredAt: result.runbook.transitions[0].clientOccurredAt,
    },
    { clientId: "tablet-ops", clientSequence: 1, clientOccurredAt: "2026-09-01T09:59:58.000Z" },
  );
  const domainReceipt = transitionRunbookTask(
    created.runbook,
    { ...command(), actorType: "human", actorId: "user-seyam", source: "studio", sessionId: "session-user-seyam" },
    { committedAt: NOW },
  ).receipt;
  assert.equal(result.runbook.receipts[0].inputFingerprint, domainReceipt.inputFingerprint);
  assert.deepEqual(verifyRunbookLedger(result.runbook), {
    status: "pass",
    entries: 1,
    headHash: result.runbook.ledger[0].hash,
  });

  const blockedResponse = await request(syncPath, {
    identity: "planner",
    organizationId: "org-alpha",
    method: "POST",
    body: {
  ...
  assert.deepEqual(verifyRunbookLedger(blocked.runbook), {
    status: "pass",
    entries: 2,
    headHash: blocked.runbook.ledger[1].hash,
  });
});
...
test("a complete seeded event synchronizes once and exact retries create no duplicate transitions", async (t) => {
  ...
  let local = createEventDayRunbook({
    ...
    validation: {
      validationId: "validation-seeded-event",
      inputFingerprint: "validation-seeded-event-input",
      status: "pass",
    },
  ...
  const first = await (
    await request(syncPath, { identity: "planner", organizationId: "org-alpha", method: "POST", body: { commands } })
  ).json();
  ...
  assert.deepEqual(verifyRunbookLedger(first.runbook), {
    status: "pass",
    entries: 18,
    headHash: first.runbook.ledger.at(-1).hash,
  });
