...
class MemoryR2 {
  ...
  async put(key, value, options) {
    const bytes = new Uint8Array(value instanceof Uint8Array ? value : await new Response(value).arrayBuffer());
    this.objects.set(key, { bytes, ...options });
  }
  async get(key) {
    const item = this.objects.get(key);
    return item ? { body: item.bytes, size: item.bytes.byteLength, httpMetadata: item.httpMetadata, customMetadata: item.customMetadata } : null;
  }
...
const runbook = createEventDayRunbook({
  ...
  validation: { validationId: "validation-incident-routes", inputFingerprint: "validation-input-incident-routes", status: "pass" },
...
async function harness() {
  ...
  await db.batch([
    db.prepare("INSERT INTO users (id,identity_provider,provider_subject,email,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?)").bind("user-seyam", "test", "operator", "operator@example.test", "active", "2026-09-12T10:00:00.000Z", "2026-09-12T10:00:00.000Z"),
  ...
  const request = (path, { identity = "operator", method = "GET", body, form } = {}) => api.fetch(new Request(`https://example.test${path}`, { method, headers: { accept: "application/json", "x-test-identity": identity, "x-venuemind-organization-id": "org-alpha", ...(body ? { "content-type": "application/json" } : {}) }, ...(body ? { body: JSON.stringify(body) } : form ? { body: form } : {}) }), { DB: db, INCIDENT_EVIDENCE: bucket });
  return { bucket, db, request, setNow: (value) => { now = value; } };
}

test("Incident routes create, report, manage, attach, download, reload, and export one auditable register", async (t) => {
  const { bucket, db, request, setNow } = await harness();
  t.after(() => db.close());
  ...
  const report = { type: "report_incident", incidentId: "incident-east-exit", severity: "high", category: "fire-life-safety", summaryCode: "EXIT_OBSTRUCTED", location: { kind: "plan-object", planObjectId: "obj-fire-exit-east" }, relatedRefs: [{ kind: "plan-object", id: "obj-fire-exit-east" }], idempotencyKey: "route-report-001", operationId: "route-operation-001" };
  const reported = await (await request(`${item}/commands:sync`, { method: "POST", body: { commands: [report, report] } })).json();
  assert.deepEqual(reported.acknowledgements.map((value) => value.status), ["applied", "already-applied"]);
  ...
  const managed = await (await request(`${item}/commands:sync`, { method: "POST", body: { commands: [
    { type: "set_incident_owner", incidentId: "incident-east-exit", owner: { roleId: "role-security" }, expectedIncidentRevision: 1, idempotencyKey: "route-owner-001" },
    { type: "acknowledge_incident", incidentId: "incident-east-exit", reasonCode: "OPS_OWNER_CONFIRMED", expectedIncidentRevision: 2, idempotencyKey: "route-ack-001" },
    { type: "escalate_incident", incidentId: "incident-east-exit", level: "venue-command", reasonCode: "EXIT_CAPACITY_AT_RISK", expectedIncidentRevision: 3, idempotencyKey: "route-escalate-001" },
  ] } })).json();
  assert.deepEqual(managed.acknowledgements.map((value) => value.status), ["applied", "applied", "applied"]);
  ...
  const attachmentResponse = await request(`${item}/incidents/incident-east-exit/attachments`, { method: "POST", form });
  assert.equal(attachmentResponse.status, 201);
  const attached = await attachmentResponse.json();
...
test("Incident routes keep privileged response actions human-only and evidence metadata private", async (t) => {
  ...
  const collection = "/api/projects/project-alpha/incident-registers";
  const created = await (await request(collection, { method: "POST", body: { runbookVersionId: runbook.versionId } })).json();
  const item = `${collection}/${encodeURIComponent(created.register.id)}`;
