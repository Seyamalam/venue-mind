...
test("Incident reporting rejects unsafe classification, location, identity fields, and ID reuse", () => {
  ...
  assert.throws(
    () => reportIncident(register, report({ severity: "urgent" })),
    (error) => error.code === "INCIDENT_INVALID",
  );
  assert.throws(
    () => reportIncident(register, report({ location: { kind: "coordinate", point: { x: -100, y: -100 } } })),
    (error) => error.code === "INCIDENT_LOCATION_INVALID",
  );
  assert.throws(
    () => reportIncident(register, report({ attendeeName: "Private Person" })),
    (error) => error.code === "INCIDENT_PRIVACY_REJECTED" && error.details.field === "attendeeName",
  );
  assert.throws(
    () => reportIncident(register, report({ actorType: "robot" })),
    (error) => error.code === "INCIDENT_INVALID" && error.details.reason === "actor-type-invalid",
  );
  const first = reportIncident(register, report(), { committedAt: "2026-09-12T09:05:00.000Z" });
  assert.throws(
    () => reportIncident(first.register, report({ idempotencyKey: "different-key" })),
    (error) => error.code === "INCIDENT_ID_CONFLICT",
  );
  assert.throws(
    () => reportIncident(first.register, report({ severity: "critical" })),
    (error) => error.code === "IDEMPOTENCY_KEY_CONFLICT",
  );
});

test("authorized humans classify, own, acknowledge, escalate, relocate, hand off, act, resolve, close, and export one Incident", () => {
  let register = createIncidentRegister({
    projectId: "project-summit-forward",
    runbook: makeRunbook(),
    createdAt: "2026-09-12T09:00:00.000Z",
    createdBy: "user-ops",
  ...
  const apply = (operation, input, at) => {
    const result = operation(
      register,
      {
        incidentId: "incident-east-exit",
        ...input,
        ...humanMetadata(input.idempotencyKey, register.incidents[0].revision),
      },
      { committedAt: at },
    );
  ...
  apply(
    classifyIncident,
    {
      severity: "critical",
      category: "fire-life-safety",
      summaryCode: "EXIT_BLOCKED_CRITICAL",
      idempotencyKey: "classify-1",
    },
    "2026-09-12T09:06:00.000Z",
  );
  const firstOwner = {
    roleId: "role-security",
    shiftId: "shift-a",
    staffPostObjectId: "obj-post-exit",
    assignmentId: "assignment-security-east",
  };
  apply(setIncidentOwner, { owner: firstOwner, idempotencyKey: "owner-1" }, "2026-09-12T09:07:00.000Z");
  apply(
    acknowledgeIncident,
    { reasonCode: "OPS_OWNER_CONFIRMED", idempotencyKey: "ack-1" },
    "2026-09-12T09:08:00.000Z",
  );
  apply(
    escalateIncident,
    { level: "venue-command", reasonCode: "EXIT_CAPACITY_AT_RISK", idempotencyKey: "escalate-1" },
    "2026-09-12T09:09:00.000Z",
  );
  apply(
    relocateIncident,
    {
      location: { kind: "coordinate", point: { x: 29, y: 10 } },
      reasonCode: "OBSTRUCTION_MOVED",
  ...
  const secondOwner = {
    ...
    shiftId: "shift-b",
    staffPostObjectId: "obj-post-exit",
    assignmentId: "assignment-security-late",
  };
  apply(
    handoffIncident,
    {
      fromOwner: firstOwner,
      toOwner: secondOwner,
      openActionCodes: ["KEEP_EXIT_CLEAR"],
      evidenceRefs: [{ kind: "plan-object", id: "obj-fire-exit-east" }],
      idempotencyKey: "handoff-1",
    },
    "2026-09-12T09:11:00.000Z",
  );
  apply(
    recordIncidentEmergencyAction,
    {
      ...
      authorityRole: "venue-administrator",
      idempotencyKey: "emergency-1",
    },
    "2026-09-12T09:13:00.000Z",
  );
  apply(
    transitionIncidentStatus,
    { toStatus: "mitigating", reasonCode: "CONTROL_IN_PLACE", idempotencyKey: "status-1" },
    "2026-09-12T09:14:00.000Z",
  );
  apply(
    transitionIncidentStatus,
    { toStatus: "resolved", resolutionCode: "EXIT_CLEARED", idempotencyKey: "status-2" },
    "2026-09-12T09:15:00.000Z",
  );
  apply(
    transitionIncidentStatus,
    { toStatus: "closed", reasonCode: "POST_EVENT_REVIEW_COMPLETE", idempotencyKey: "status-3" },
  ...
  assert.equal(incident.revision, 11);
  ...
  assert.equal(register.transitions.length, 11);
  assert.equal(register.receipts.length, 11);
  assert.equal(register.ledger.length, 11);
  assert.equal(
    register.ledger.every((entry) => entry.actorId && entry.committedAt && entry.location && entry.transitionId),
    true,
  );
  assert.equal(verifyIncidentLedger(register).status, "pass");
  assert.deepEqual(
    inspectIncidents(register, { status: "closed", severity: "critical" }).map((item) => item.id),
    ["incident-east-exit"],
  );
  ...
  assert.equal(artifact.transitions.length, 11);
  assert.equal(artifact.ledger.length, 11);
});
...
test("agents cannot exercise human Incident authority", () => {
  ...
  ({ register } = reportIncident(register, report({ actorType: "agent", actorId: "agent-ops", source: "webmcp" }), {
    committedAt: "2026-09-12T09:05:00.000Z",
  }));
  assert.throws(
    () =>
      acknowledgeIncident(register, {
        incidentId: "incident-east-exit",
        reasonCode: "ACK",
        ...humanMetadata("agent-ack", 1),
        actorType: "agent",
        actorId: "agent-ops",
        source: "webmcp",
      }),
    (error) => error.code === "INCIDENT_HUMAN_REQUIRED",
  );
...
test("Incident mutations reject stale revisions and exact retries preserve one transition", () => {
  ...
  assert.throws(
    () =>
      classifyIncident(first.register, {
        incidentId: "incident-east-exit",
        severity: "critical",
        category: "security",
        summaryCode: "SECURITY_RISK",
        ...humanMetadata("stale-classify", 1),
      }),
    (error) => error.code === "INCIDENT_REVISION_CONFLICT" && error.details.currentIncidentRevision === 2,
  );
  assert.throws(
    () =>
      transitionIncidentStatus(first.register, {
        incidentId: "incident-east-exit",
        toStatus: "resolved",
        resolutionCode: "CLEARED",
        ...humanMetadata("resolve-before-ack", 2),
      }),
    (error) =>
      error.code === "INCIDENT_TRANSITION_INVALID" && error.details.reason === "acknowledgement-and-owner-required",
  );
});

test("Incident export fails closed when its global ledger is tampered", () => {
  const initial = createIncidentRegister({
    projectId: "project-summit-forward",
    runbook: makeRunbook(),
    createdAt: "2026-09-12T09:00:00.000Z",
    createdBy: "user-ops",
  ...
  assert.throws(
    () => exportIncidentRecord(tampered, { incidentId: "incident-east-exit", exportedAt: "2026-09-12T09:10:00.000Z" }),
    (error) => error.code === "INCIDENT_LEDGER_INTEGRITY_FAILED",
  );

  for (const mutate of [
    (value) => {
      value.incidents[0].status = "closed";
    },
    (value) => {
      value.transitions[0].location.planFingerprint = "altered";
    },
    (value) => {
      value.receipts.splice(0, 1);
    },
    (value) => {
      value.receipts[0].idempotencyKey = "altered";
    },
    (value) => {
      value.receipts[0].operation = "altered";
    },
    (value) => {
      value.receipts[0].inputFingerprint = "altered";
    },
    (value) => {
      value.source.approvalLedgerEntryId = "altered";
    },
    (value) => {
      value.baseline.acceptedPlan.name = "altered";
    },
    (value) => {
      value.ledger[0].actorType = "robot";
    },
  ]) {
    ...
    assert.throws(
      () => exportIncidentRecord(altered, { incidentId: "incident-east-exit", exportedAt: "2026-09-12T09:10:00.000Z" }),
      (error) => error.code === "INCIDENT_LEDGER_INTEGRITY_FAILED",
    );
  }
});
