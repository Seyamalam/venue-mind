...
test("Incident remote scopes create, get, sync, export, and evidence upload to one Project", async () => {
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    const payload = url.endsWith("/export")
      ? { artifact: { filename: "incident-record.json" } }
      : url.endsWith("/attachments")
        ? { attachment: { id: "incident-evidence-1" } }
        : url.endsWith("commands:sync")
          ? { acknowledgements: [], register: { id: "incident-register-1" } }
          : { register: { id: "incident-register-1" } };
    return new Response(JSON.stringify(payload), { status: url.endsWith("/incident-registers") ? 201 : 200, headers: { "content-type": "application/json" } });
  };
  ...
  assert.deepEqual(calls.map((call) => [call.url, call.init.method ?? "GET"]), [
    ...
    ["/api/projects/project-alpha/incident-registers/incident-register-1/incidents/incident-alpha/attachments", "POST"],
  ]);
...
test("Incident remote preserves structured API failures and rejects invalid upload input", async () => {
  const remote = createIncidentRemote({ organizationId: "org-alpha", fetchImpl: async () => new Response(JSON.stringify({ code: "INCIDENT_REVISION_CONFLICT", error: "Conflict", details: { currentRevision: 4 } }), { status: 409, headers: { "content-type": "application/json" } }) });
  await assert.rejects(() => remote.get("project-alpha", "incident-register-1"), (error) => error.code === "INCIDENT_REVISION_CONFLICT" && error.details.currentRevision === 4 && error.status === 409);
