...
test("database migration 10 installs the tenant-scoped Incident Register aggregate and integrity checks", async (t) => {
  const db = new SqliteD1();
  t.after(() => db.close());
  const migrated = await applyDatabaseMigrations(db);
  assert.equal(DATABASE_SCHEMA_VERSION, 10);
  assert.equal(migrated.currentVersion, 10);
  ...
  await db.prepare("INSERT INTO event_day_incident_registers (id,organization_id,project_id,runbook_id,schema_version,baseline_fingerprint,baseline_json,register_json,revision,ledger_head_hash,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
    .bind("incident-corrupt", "org-bravo", "project-alpha", "runbook-alpha", 1, "baseline-alpha", "{}", "{}", 0, "ledger-alpha", NOW, NOW).run();

  const integrity = await inspectDatabaseIntegrity(db);
  assert.equal(integrity.status, "fail");
  assert.equal(integrity.checks.find((check) => check.id === "incident-register-organization-mismatch")?.count, 1);
  assert.equal(integrity.checks.find((check) => check.id === "incident-register-runbook-scope-mismatch")?.count, 1);
});
