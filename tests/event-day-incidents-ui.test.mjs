...
test("Incident Studio is a non-modal prop-driven operational surface", async () => {
  ...
  for (const component of ["Badge", "Button", "Empty", "Field", "Input", "ScrollArea", "Select", "Separator", "Sheet", "Tabs", "Textarea", "ToggleGroup"]) {
  ...
  assert.match(source, />LIVE · INCIDENTS</);
  for (const view of ["new", "issues", "handoff", "ledger"]) assert.match(source, new RegExp(`<TabsTrigger value="${view}">`));
  for (const field of ["SEVERITY", "CATEGORY", "OWNER", "SUMMARY", "ANCHOR"]) assert.match(source, new RegExp(`>${field}<`));
  for (const action of ["ACK", "ESCALATE", "EMERGENCY", "MITIGATE", "RESOLVE", "CLOSE", "REOPEN", "HANDOFF", "ATTACH", "GET", "DISCARD"]) assert.match(source, new RegExp(`>${action}<`));
  for (const callback of ["onCreate", "onSelectIncident", "onSelectAnchor", "onAcknowledge", "onEscalate", "onEmergencyAction", "onResolve", "onCreateHandoff", "onAttach", "onDownloadAttachment", "onDiscardConflicts", "onSync", "onExport"]) assert.match(source, new RegExp(`\\b${callback}\\b`));
  for (const category of ["accessibility", "crowd-capacity", "fire-life-safety", "production-av", "transport", "weather"]) assert.match(source, new RegExp(`id: "${category}"`));
...
test("Incident Studio is lazy-wired through OPS, recovery, shared tools, and human actions", async () => {
  ...
  for (const command of ["report_incident", "set_incident_owner", "acknowledge_incident", "escalate_incident", "record_incident_emergency_action", "transition_incident_status", "handoff_incident"]) assert.match(source, new RegExp(command));
