...
const panelUrl = new URL("../src/IncidentPanel.jsx", import.meta.url);
...
test("Incident Studio is a non-modal prop-driven operational surface", async () => {
  ...
  for (const action of ["ACK", "ESCALATE", "RESOLVE", "HANDOFF", "ATTACH"]) assert.match(source, new RegExp(`>${action}<`));
  for (const callback of ["onCreate", "onSelectIncident", "onSelectAnchor", "onAcknowledge", "onEscalate", "onResolve", "onCreateHandoff", "onAttach", "onSync", "onExport"]) assert.match(source, new RegExp(`\\b${callback}\\b`));
...
test("Incident Studio is lazy-wired through OPS, recovery, shared tools, and human actions", async () => {
  ...
  for (const command of ["report_incident", "set_incident_owner", "acknowledge_incident", "escalate_incident", "transition_incident_status", "handoff_incident"]) assert.match(source, new RegExp(command));
