...
function Block({ block }: { block: DocBlock }) {
  ...
  if (block.type === "bullets") return <ul>{block.items.map((item) => <li key={item}><CheckCircle size={17} fill="currentColor" />{item}</li>)}</ul>;
  if (block.type === "steps") return <ol>{block.items.map((item, index) => <li key={item}><span>{index + 1}</span>{item}</li>)}</ol>;
  if (block.type === "links") {
    return <div className="doc-link-grid">{block.items.map((item) => <DocumentationLink key={item.href} href={item.href}><span>{item.label}</span><ArrowRight size={16} /></DocumentationLink>)}</div>;
  }
  if (block.type === "table") {
    return (
    ...
    <div className="docs-shell">
        ...
        {navigation.map((group) => {
          ...
          return (
            <section key={group.label}>
              ...
              {group.pages.map((item) => <Link key={item.slug} href={item.href as Route} className={page.slug === item.slug ? "active" : ""} aria-current={page.slug === item.slug ? "page" : undefined}>{item.title}</Link>)}
            </section>
          );
        })}
      </aside>
      <main className="docs-main">
        <div className="docs-hero">
          ...
          {page.slug === "overview" && <Link href="/docs/quickstart" className="docs-cta"><TerminalWindow size={18} />Quickstart<ArrowRight size={16} /></Link>}
        </div>
        <article>
