...
const sharingDeliveryColumns = [
  "lifecycle_state",
  "creation_ledgered_at",
  "revocation_ledgered_at",
  "operation_attempts",
...
async function legacyBaseline(db: D1Database) {
  ...
  if (!hasLifecycle && lifecycleColumns.some((column) => columns.has(column)))
  ...
  if (!hasTenancy && (columns.has("organization_id") || tenancyTables.some((table) => tables.has(table))))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  const hasConcurrency = concurrencyColumns.every((column) => columns.has(column));
  if (!hasConcurrency && concurrencyColumns.some((column) => columns.has(column)))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  const hasCollaboration = collaborationTables.every((table) => tables.has(table));
  if (!hasCollaboration && collaborationTables.some((table) => tables.has(table)))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  const hasSharing = sharingTables.every((table) => tables.has(table));
  if (!hasSharing && sharingTables.some((table) => tables.has(table)))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  let hasSharingDelivery = false;
  if (hasSharing) {
    const { results: shareColumnRows } = await db
      .prepare("PRAGMA table_info(project_share_links)")
      .all<{ name: string }>();
    ...
    if (!hasSharingDelivery && sharingDeliveryColumns.some((column) => shareColumns.has(column)))
      throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  }
  const hasRunbooks = runbookTables.every((table) => tables.has(table));
  if (!hasRunbooks && runbookTables.some((table) => tables.has(table)))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
  const hasOccupancy = occupancyTables.every((table) => tables.has(table));
  if (!hasOccupancy && occupancyTables.some((table) => tables.has(table)))
    throw new Error("MIGRATION_LEGACY_SCHEMA_PARTIAL");
...
export async function planDatabaseMigrations(db: D1Database, { clock = () => new Date().toISOString() } = {}) {
  ...
  let applied = tables.has("schema_migrations")
    ? (
        await db
          .prepare("SELECT version, name, checksum, applied_at, adopted FROM schema_migrations ORDER BY version")
  ...
  if (!applied.length && tables.has("projects")) {
    ...
    if (baseline) {
      ...
      applied = DATABASE_MIGRATIONS.slice(0, baseline).map((migration) => ({
        version: migration.version,
        name: migration.name,
        checksum: migration.checksum,
        applied_at: adoptedAt,
        adopted: 1,
      }));
  ...
  for (const migration of applied) {
    ...
    if (expected.name !== migration.name || expected.checksum !== migration.checksum)
  ...
  if (pending.some((migration, index) => migration.version !== highest + index + 1))
  ...
  return {
    ...
    applied: applied.map((migration) => ({
      ...
      adopted: Boolean(migration.adopted),
    })),
    pending,
  };
}
...
interface ApplyMigrationOptions {
  ...
  readonly projectExport?: (migration: (typeof DATABASE_MIGRATIONS)[number]) => void | Promise<void>;
}

export async function applyDatabaseMigrations(
  db: D1Database,
  { dryRun = false, clock = () => new Date().toISOString(), projectExport }: ApplyMigrationOptions = {},
) {
  ...
  if (report.adoptionRequired) {
    await db.batch(
      report.applied.map((migration) =>
        db
          .prepare(
            "INSERT INTO schema_migrations (version, name, checksum, applied_at, adopted) VALUES (?, ?, ?, ?, 1)",
          )
          .bind(migration.version, migration.name, migration.checksum, migration.applied_at),
      ),
    );
  }
  for (const migration of report.pending) {
    if (migration.destructive || migration.requiresProjectExport) {
      if (!projectExport) throw new Error(`MIGRATION_PROJECT_EXPORT_REQUIRED:${migration.version}`);
      await projectExport(migration);
    }
    await db.batch([
      ...migration.statements.map((sql) => db.prepare(sql)),
      db
        .prepare("INSERT INTO schema_migrations (version, name, checksum, applied_at, adopted) VALUES (?, ?, ?, ?, 0)")
        .bind(migration.version, migration.name, migration.checksum, clock()),
    ]);
  }
...
export async function inspectDatabaseIntegrity(db: D1Database) {
  ...
  for (const [id, sql] of [
    [
      ...
      "SELECT COUNT(*) AS count FROM project_states s LEFT JOIN projects p ON p.id = s.project_id WHERE p.id IS NULL",
    ],
    [
      "project-without-state",
      "SELECT COUNT(*) AS count FROM projects p LEFT JOIN project_states s ON s.project_id = p.id WHERE s.project_id IS NULL",
    ],
    [
      "project-without-organization",
      "SELECT COUNT(*) AS count FROM projects p LEFT JOIN organizations o ON o.id = p.organization_id WHERE p.organization_id IS NULL OR o.id IS NULL",
    ],
    [
      "membership-without-user",
      "SELECT COUNT(*) AS count FROM organization_memberships m LEFT JOIN users u ON u.id = m.user_id WHERE u.id IS NULL",
    ],
    [
      "membership-without-organization",
      "SELECT COUNT(*) AS count FROM organization_memberships m LEFT JOIN organizations o ON o.id = m.organization_id WHERE o.id IS NULL",
    ],
    [
  ...
  for (const [id, sql] of [
    [
      "collaboration-event-without-project",
      "SELECT COUNT(*) AS count FROM project_collaboration_events e LEFT JOIN projects p ON p.id = e.project_id WHERE p.id IS NULL",
    ],
    [
      "presence-without-project",
      "SELECT COUNT(*) AS count FROM project_presence r LEFT JOIN projects p ON p.id = r.project_id WHERE p.id IS NULL",
    ],
    [
      "presence-without-user",
      "SELECT COUNT(*) AS count FROM project_presence r LEFT JOIN users u ON u.id = r.user_id WHERE u.id IS NULL",
    ],
  ] satisfies readonly (readonly [string, string])[]) {
    const { results } = await db.prepare(sql).all<{ count: number }>();
    checks.push({
  ...
  for (const [id, sql] of [
    [
      "share-link-without-project",
      "SELECT COUNT(*) AS count FROM project_share_links l LEFT JOIN projects p ON p.id = l.project_id WHERE p.id IS NULL",
    ],
    [
      "share-link-organization-mismatch",
      "SELECT COUNT(*) AS count FROM project_share_links l JOIN projects p ON p.id = l.project_id WHERE p.organization_id != l.organization_id",
    ],
    [
      "share-link-creator-without-user",
      "SELECT COUNT(*) AS count FROM project_share_links l LEFT JOIN users u ON u.id = l.created_by WHERE u.id IS NULL",
    ],
    [
      "share-link-revoker-without-user",
      "SELECT COUNT(*) AS count FROM project_share_links l LEFT JOIN users u ON u.id = l.revoked_by WHERE l.revoked_by IS NOT NULL AND u.id IS NULL",
    ],
    [
      "notification-without-project",
      "SELECT COUNT(*) AS count FROM notifications n LEFT JOIN projects p ON p.id = n.project_id WHERE p.id IS NULL",
    ],
    [
      "notification-organization-mismatch",
      "SELECT COUNT(*) AS count FROM notifications n JOIN projects p ON p.id = n.project_id WHERE p.organization_id != n.organization_id",
    ],
    [
      "notification-without-user",
      "SELECT COUNT(*) AS count FROM notifications n LEFT JOIN users u ON u.id = n.user_id WHERE u.id IS NULL",
    ],
    [
      "notification-preference-without-user",
      "SELECT COUNT(*) AS count FROM notification_preferences n LEFT JOIN users u ON u.id = n.user_id WHERE u.id IS NULL",
    ],
    [
      "email-outbox-without-notification",
      "SELECT COUNT(*) AS count FROM notification_email_outbox e LEFT JOIN notifications n ON n.id = e.notification_id WHERE n.id IS NULL",
    ],
    [
      "active-share-without-creation-ledger",
      "SELECT COUNT(*) AS count FROM project_share_links WHERE lifecycle_state='active' AND creation_ledgered_at IS NULL",
    ],
    [
      "revoked-share-without-revocation-ledger",
      "SELECT COUNT(*) AS count FROM project_share_links WHERE lifecycle_state='revoked' AND revocation_ledgered_at IS NULL",
    ],
    [
      "pending-revocation-without-actor",
      "SELECT COUNT(*) AS count FROM project_share_links WHERE lifecycle_state='pending-revoke' AND (revoked_at IS NULL OR revoked_by IS NULL)",
    ],
    [
      "email-outbox-inconsistent-delivery",
      "SELECT COUNT(*) AS count FROM notification_email_outbox WHERE delivered_at IS NOT NULL AND (failure_code IS NOT NULL OR lease_token IS NOT NULL)",
    ],
  ] satisfies readonly (readonly [string, string])[]) {
    const { results } = await db.prepare(sql).all<{ count: number }>();
    checks.push({
  ...
  for (const [id, sql] of [
    [
      "incident-register-without-project",
      "SELECT COUNT(*) AS count FROM event_day_incident_registers i LEFT JOIN projects p ON p.id = i.project_id WHERE p.id IS NULL",
    ],
    [
      "incident-register-organization-mismatch",
      "SELECT COUNT(*) AS count FROM event_day_incident_registers i JOIN projects p ON p.id = i.project_id WHERE p.organization_id != i.organization_id",
    ],
    [
      "incident-register-runbook-scope-mismatch",
      "SELECT COUNT(*) AS count FROM event_day_incident_registers i LEFT JOIN event_day_runbooks r ON r.id = i.runbook_id AND r.organization_id = i.organization_id AND r.project_id = i.project_id WHERE r.id IS NULL",
    ],
  ] satisfies readonly (readonly [string, string])[]) {
    const { results } = await db.prepare(sql).all<{ count: number }>();
    checks.push({
  ...
  return {
    status:
      integrity.every((value) => value === "ok") && checks.every((check) => check.status === "pass") ? "pass" : "fail",
    sqlite: integrity,
    checks,
  };
}
