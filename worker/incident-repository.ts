...
export class IncidentRegisterConflict extends Error {
  ...
  constructor(code: "INCIDENT_REGISTER_ID_CONFLICT" | "INCIDENT_REGISTER_REVISION_CONFLICT" | "INCIDENT_REGISTER_BASELINE_IMMUTABLE" | "INCIDENT_REGISTER_SCOPE_INVALID" | "INCIDENT_REGISTER_INTEGRITY_FAILED", details: Record<string, unknown> = {}) {
    super(code === "INCIDENT_REGISTER_ID_CONFLICT"
      ...
      : code === "INCIDENT_REGISTER_INTEGRITY_FAILED" ? "Incident Register integrity verification failed"
...
const assertIntegrity = (register: IncidentRegister, row?: Record<string, unknown>) => {
  ...
  if (integrity.status !== "pass" || rowMismatch) {
    throw new IncidentRegisterConflict("INCIDENT_REGISTER_INTEGRITY_FAILED", {
      ...
      reason: integrity.status === "pass" ? "row-register-mismatch" : integrity.reason ?? "ledger-verification-failed",
    });
  }
  return register;
};
...
export function createD1IncidentRepository(db: D1Database) {
  const get = async (organizationId: string, projectId: string, registerId: string) => {
    await ready(db);
    return map(await db.prepare("SELECT id,project_id,runbook_id,schema_version,baseline_fingerprint,baseline_json,register_json,revision,ledger_head_hash,created_at,updated_at FROM event_day_incident_registers WHERE id=? AND organization_id=? AND project_id=?").bind(registerId, organizationId, projectId).first<Record<string, unknown>>());
  };
  const getByRunbook = async (organizationId: string, projectId: string, runbookId: string) => {
    await ready(db);
    return map(await db.prepare("SELECT id,project_id,runbook_id,schema_version,baseline_fingerprint,baseline_json,register_json,revision,ledger_head_hash,created_at,updated_at FROM event_day_incident_registers WHERE runbook_id=? AND organization_id=? AND project_id=?").bind(runbookId, organizationId, projectId).first<Record<string, unknown>>());
  };
