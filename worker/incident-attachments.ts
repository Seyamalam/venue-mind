...
const INCIDENT_ATTACHMENT_MIME_TYPES = Object.freeze(["image/jpeg", "image/png", "image/webp", "application/pdf"] as const);
...
const safeFilename = (value: unknown) => {
  ...
  const safe = basename.replace(/[";\u0000-\u001f\u007f]/g, "_").trim().slice(0, 120);
...
const validMagic = (mimeType: string, bytes: Uint8Array) => {
  ...
  if (mimeType === "image/webp") return isPrefix(bytes, [0x52, 0x49, 0x46, 0x46]) && String.fromCharCode(...bytes.slice(8, 12)) === "WEBP";
...
export function createIncidentAttachmentService({
  ...
  return Object.freeze({
    async upload(input: AttachmentUpload): Promise<IncidentAttachmentMetadata> {
      ...
      if (!Array.isArray(attachments)) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_INVALID", "Existing attachments must be an array");
      if (attachments.length >= INCIDENT_ATTACHMENT_MAX_COUNT) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_LIMIT_EXCEEDED", "Incident attachment count limit reached", { maximum: INCIDENT_ATTACHMENT_MAX_COUNT });
      const bytes = await toBytes(input.content);
      if (bytes.byteLength === 0 || bytes.byteLength > INCIDENT_ATTACHMENT_MAX_BYTES) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_SIZE_INVALID", "Incident attachment size is invalid", { maximumBytes: INCIDENT_ATTACHMENT_MAX_BYTES, byteLength: bytes.byteLength });
      if (!validMagic(input.mimeType, bytes)) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_TYPE_INVALID", "Incident attachment media type does not match its content", { mimeType: input.mimeType });
      const token = idFactory();
      if (!/^[A-Za-z0-9-]{1,128}$/.test(token)) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_ID_INVALID", "Generated attachment ID is invalid");
      ...
      const metadata: IncidentAttachmentMetadata = Object.freeze({
        ...
        mimeType: input.mimeType as IncidentAttachmentMetadata["mimeType"],
    ...
    async download(metadata: IncidentAttachmentMetadata): Promise<Response> {
      ...
      if (!/^incident-evidence-[A-Za-z0-9-]{1,128}$/.test(id)
        ...
        || !metadata.filename) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_INVALID", "Attachment metadata is invalid");
      const object = await bucket.get(objectKey(id));
      if (!object) throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_NOT_FOUND", "Incident attachment not found", { attachmentId: id });
      const custom = object.customMetadata ?? {};
      if (custom.attachmentId !== id || custom.incidentId !== incidentId || custom.sha256 !== metadata.sha256 || object.size !== metadata.byteLength || object.httpMetadata?.contentType !== metadata.mimeType) {
        throw new IncidentAttachmentError("INCIDENT_ATTACHMENT_INTEGRITY_FAILED", "Incident attachment metadata does not match stored evidence", { attachmentId: id });
      }
